...
$Qry2="select distinct category,part_nm,remark,urut,ISNULL((select sum(qty) from bps_budget_stp_act where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as qty,ISNULL((select sum(price) from bps_budget_stp_act where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as price from bps_budget_stp a where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";	
if($sect_s==""){
	$Qry2="select distinct category,part_nm,urut,ISNULL((select sum(qty) from bps_budget_stp_act where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as qty,ISNULL((select sum(price) from bps_budget_stp_act where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as price from bps_budget_stp a where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";
}		
// echo $qry2;
$baris_s  = 9; //Ini untuk dimulai baris datanya
		...
		$column=4;
				...
				$category=odbc_result($tb_part_s,"category");
				
					$S2->setCellValueByColumnAndRow(0,$baris_s,odbc_result($tb_part_s,"category"));					
			...
			$Qry2="select distinct category,part_nm,sum(qty) as qty,sum(price) as price from bps_budget_stp_act a where term='$term_sum' $sec_sa and not exists (select part_nm from bps_budget_stp where term='$term_s' and category=a.category and part_nm=a.part_nm $sec_sa and revisi='$rev_s') group by part_nm,category";	
...
$mh_stp= "select distinct top 1 mh_s,mh_act,mh_p,revisi from bps_budget_stp_mh where term='$term_s' order by revisi desc";
...
$Qry3="select distinct category,part_nm,remark,urut,ctg,cost_push,cost_reduction,cost_other,nariyuki,ISNULL((select sum(dbo.lp_konprc('$term_s','USD',curr,price*qty)) from bps_budget where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as amt1,ISNULL((select sum(price) from bps_budget_stp_act where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as price from bps_budget_stp a where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";
if($sect_s==""){
	$Qry3="select distinct category,part_nm,urut,ctg,cost_push,cost_reduction,cost_other,nariyuki,ISNULL((select sum(dbo.lp_konprc('$term_s','USD',curr,price*qty)) from bps_budget where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as qty,ISNULL((select sum(price) from bps_budget_stp_act where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as price from bps_budget_stp a where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";
}
...
$baris_a  = 14; //Ini untuk dimulai baris datanya
		...
		$cost_other="0";
				...
				$category=odbc_result($tb_part_a,"category");
				...
				$nariyuki=odbc_result($tb_part_a,"nariyuki");
						...
						$S3->setCellValueByColumnAndRow(0,$baris_a,odbc_result($tb_part_a,"category"));						
...
$Qry3="select distinct category,part_nm,sum(qty) as qty,sum(price) as price,ISNULL((select sum(dbo.lp_konprc('$term_s','USD',curr,price*qty)) from bps_budget where category=a.category and part_nm=a.part_nm and term='$term_sum' $sec_sa),0) as amt1 from bps_budget_stp_act a where term='$term_sum' $sec_sa and not exists (select part_nm from bps_budget_stp where term='$term_s' and category=a.category and part_nm=a.part_nm $sec_sa and revisi='$rev_s') group by part_nm,category";	
		...
		//$column=4;
				...
				$category=odbc_result($tb_part_a,"category");
					...
					if($sect_s!=""){
					$S3->setCellValueByColumnAndRow(11,$baris_a,"Budget di Term $term_s tidak ada, sedangkan di Term $term_sum ada.");
					}
					
					$S3->setCellValueByColumnAndRow(2,$baris_a,"F");
					$S3->setCellValueByColumnAndRow(3,$baris_a,odbc_result($tb_part_a,"amt1")); 	
					$S3->setCellValueByColumnAndRow(4,$baris_a,odbc_result($tb_part_a,"price")); 
					$S3->setCellValueByColumnAndRow(5,$baris_a,'=IF(C'.$baris_a.'="F",E'.$baris_a.',E'.$baris_a.'/$E$12*$F$12)');
					$S3->setCellValueByColumnAndRow(6,$baris_a,'=IF(F'.$baris_a.'<J'.$baris_a.',J'.$baris_a.'-F'.$baris_a.',0)');	
