...
$Qry1="select distinct sect,part_no,part_dtl,uom,term,price,id_proses,jns_budget,lp,pic_updt,tgl_updt,kurs,curr,phase,cccode,part_nm,revisi,fungsi,carline,carmaker,sub_acc,lt_vp,lt_datang,lt_po,lt_pr,lt_Quo,urut from bps_budget_stp where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";

//	echo $Qry1;
//$Qry=$_GET['Qry'];
$baris  = 13; //Ini untuk dimulai baris datanya
//$bar=0;
//$SI->setCellValue('c5', $Qry); //0
//	echo $Qry2;
		$tb_part=odbc_exec($koneksi_lp,$Qry1);
		//$tb_part1=odbc_exec($koneksi,$Qry);
		$row=0;
		$row2=0;
	//	$kat_area="";
...
$peri_stp= "select min(periode) as minperiode, max(periode) as maxperiode from bps_budget_stp where term='$term_s' ";
		$tb_peri_stp=odbc_exec($koneksi_lp,$peri_stp);
		
		while($baris_stp=odbc_fetch_array($tb_peri_stp)){
			$row2++;
			
			$SI->setCellValue('A3', 'PERIODE ' .date("M-y",strtotime(odbc_result($tb_peri_stp,"minperiode")."01")). ' SD ' .date("M-y",strtotime(odbc_result($tb_peri_stp,"maxperiode")."01")) ); //NANTI DIPINDAH KE VARIABEL PERIODE	
		}
		
$peri= "select distinct periode from bps_budget_stp where term='$term_s' order by periode asc";
	
				$tb_peri=odbc_exec($koneksi_lp,$peri);

				$bariscolp=11;
				$bariscol=13;
				$column1=0;
				$column=15;
					while($baris2=odbc_fetch_array($tb_peri)){

					$SI->setCellValueByColumnAndRow($column,$bariscolp,date("M-y",strtotime(odbc_result($tb_peri,"periode")."01")));
				//	$SI->setCellValueByColumnAndRow($column,$bariscolp,date("M-Y",strtotime(odbc_result($tb_peri,"periode"))));
					$SI->setCellValueByColumnAndRow($column+12,$bariscolp,date("M-y",strtotime(odbc_result($tb_peri,"periode")."01")));
					$SI->setCellValueByColumnAndRow($column+24,$bariscolp-1,date("M-y",strtotime(odbc_result($tb_peri,"periode")."01")));
					$SI->setCellValueByColumnAndRow($column+39,$bariscolp-1,date("M-y",strtotime(odbc_result($tb_peri,"periode")."01")));
		
							$peri1= "select distinct part_no,sect,term,qty,price,phase,dbo.lp_konprc(term,'USD',curr,price)
as price_usd,dbo.lp_konprc(term,'USD',curr,price*qty) as amn_usd,(select sum(qty) from bps_budget_stp where part_no='$part_no' $sec_s and term='$term_s' and phase='$category' and fungsi='$fungsi' ) as total_qty,(select sum(dbo.lp_konprc(term,'USD',curr,price*qty)) from bps_budget_stp where part_no='$part_no' $sec_s and term='$term_s' and phase='$category' and fungsi='$fungsi' ) as total_usd,periode from bps_budget_stp where part_no='$part_no' $sec_s and term='$term_s' and periode='".odbc_result($tb_peri,"periode")."' and phase='$category' and fungsi='$fungsi'  order by periode asc ";
			//echo $peri1;
							$tb_peri1=odbc_exec($koneksi_lp,$peri1);
						//	$total_qty=0;
							
								while($baris22=odbc_fetch_array($tb_peri1)){
									
									$SI->setCellValueByColumnAndRow($column,$baris,odbc_result($tb_peri1,"price"));
									$SI->setCellValueByColumnAndRow($column+12,$baris,odbc_result($tb_peri1,"price_usd"));
									$SI->setCellValueByColumnAndRow($column+24,$baris,odbc_result($tb_peri1,"qty"));
									$SI->setCellValueByColumnAndRow($column+25,$baris,odbc_result($tb_peri1,"total_qty"));
									$SI->setCellValueByColumnAndRow($column+26,$baris,(odbc_result($tb_peri1,"total_qty")/12));
									$SI->setCellValueByColumnAndRow($column+39,$baris,odbc_result($tb_peri1,"amn_usd"));
									$SI->setCellValueByColumnAndRow($column+40,$baris,odbc_result($tb_peri1,"total_usd"));
									$SI->setCellValueByColumnAndRow($column+41,$baris,(odbc_result($tb_peri1,"total_usd")/12));
								}	
...
//$SI->setCellValueByColumnAndRow(14,$baris-9,$kurs);
//$SI->setCellValueByColumnAndRow(13,$baris-9,$kurs_code); 

//$SI->setCellValueByColumnAndRow(2,$baris,odbc_result($tb_part,"NIK")); 
//$SI->setCellValueByColumnAndRow(3,$baris,odbc_result($tb_part,"JOB")); 
//$SI->setCellValueByColumnAndRow(105,$baris,date("d-M-Y",strtotime(odbc_result($tb_part,"MASA_BERLAKU")))); 

//set style border
//$SI->getStyle('A10:BA10')->getBorders()->getAllBorders() ->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
//$SI->getStyle('A'.(($baris-3).':BA'.($row)))->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
...
//$excelku->getActiveSheet()->mergeCells('A'.$baris.':W'.$baris);
//$SI->setCellValueByColumnAndRow(0,$baris,"Jumlah Record : ".$row); 
...
/*
$qry_sum_bef= "select distinct t.part_nm,sum(t.QTY) as qty_sum,t.periode as periode FROM
(select distinct a.part_nm,sum(a.qty_tot) AS QTY,a.periode as periode,(select top 1 b.status from bps_approve b where b.no_doc=a.pr_no and b.status='finish') as status_dok from bps_tmpPR a where a.term='83' and a.periode='202309' and a.pr_no like 'PGA-IT%' group by a.part_nm,a.pr_no,a.periode) t where t.status_dok='finish'  group by t.part_nm,t.periode";

$baris_s = 9;

	$tb_sum=odbc_exec($koneksi_lp,$qry_sum_bef);
	$row_s=0;
			while($baris_s2=odbc_fetch_array($tb_sum)){
				$row_s++;
				
				$part_nm_s=odbc_result($tb_sum,"part_nm");
				$qty_sum=odbc_result($tb_sum,"qty_sum");

$S2->setCellValueByColumnAndRow(3,$baris_s,$part_nm_s);
$S2->setCellValueByColumnAndRow(4,$baris_s,$qty_sum); 
*/
$Qry2="select distinct phase,part_nm,urut from bps_budget_stp where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";
...
//echo $Qry2;
	...
	//	$kat_area="";
		...
		//		$sect=odbc_result($tb_part_s,"sect");
		//		$part_no=odbc_result($tb_part_s,"part_no");
				$part_nm=odbc_result($tb_part_s,"part_nm");
		//		$part_dtl=odbc_result($tb_part_s,"part_dtl");
		//		$fungsi=odbc_result($tb_part_s,"fungsi");
		//		$sub_acc=odbc_result($tb_part_s,"sub_acc");
				$category=odbc_result($tb_part_s,"phase");
		//		$cccode=odbc_result($tb_part_s,"cccode"); 
		//		$carline=odbc_result($tb_part_s,"carline");
		//		$carmaker=odbc_result($tb_part_s,"carmaker");
		//		$uom=odbc_result($tb_part_s,"uom");
		//		$curr=odbc_result($tb_part_s,"curr");
			


$peri_sum= "select distinct periode from bps_budget_stp where term='$term_s' order by periode asc";
	
				$tb_peri_sum=odbc_exec($koneksi_lp,$peri_sum);
				$bariscolp=8;
				$column=4;
					while($baris_s22=odbc_fetch_array($tb_peri_sum)){

					$S2->setCellValueByColumnAndRow($column,$bariscolp,date("M-y",strtotime(odbc_result($tb_peri_sum,"periode")."01")));
					$S2->setCellValueByColumnAndRow($column+12,$bariscolp,date("M-y",strtotime(odbc_result($tb_peri_sum,"periode")."01")));

	$year=date("Y",strtotime(odbc_result($tb_peri_sum,"periode")."01"));
	$year2=$year-1;

				$peri1_s= "select distinct phase,part_nm,sect,remark,term,(select sum(qty) from bps_budget_stp where part_nm='$part_nm' $sec_s and periode='".odbc_result($tb_peri_sum,"periode")."' and term='$term_s' and phase='$category' ) as qty_month,(select sum(dbo.lp_konprc(term,'USD',curr,price*qty)) from bps_budget_stp where part_nm='$part_nm' $sec_s and periode='".odbc_result($tb_peri_sum,"periode")."' and term='$term_s' and phase='$category') as usd_month,periode,(select sum(qty) from bps_budget_stp where part_nm='$part_nm' $sec_s and term='$term_s' and phase='$category' ) as total_qty,(select sum(dbo.lp_konprc(term,'USD',curr,price*qty)) from bps_budget_stp where part_nm='$part_nm' $sec_s and term='$term_s' and phase='$category' ) as total_usd
				 from bps_budget_stp where part_nm='$part_nm' $sec_s and term='$term_s' and periode='".odbc_result($tb_peri_sum,"periode")."' and phase='$category' order by phase asc ";
//echo $peri1_s;
								
												$tb_peri1_s=odbc_exec($koneksi_lp,$peri1_s);
											//	$total_qty=0;
												
													while($baris22_s=odbc_fetch_array($tb_peri1_s)){
														
														$S2->setCellValueByColumnAndRow($column,$baris_s,odbc_result($tb_peri1_s,"qty_month"));
														$S2->setCellValueByColumnAndRow($column+12,$baris_s,odbc_result($tb_peri1_s,"usd_month"));
														$S2->setCellValueByColumnAndRow($column+13,$baris_s,odbc_result($tb_peri1_s,"total_qty"));
														$S2->setCellValueByColumnAndRow($column+14,$baris_s,odbc_result($tb_peri1_s,"total_usd"));				
													}	
					$column++;
			}

/*$qry_sum_bef="SELECT isnull(sum(qty_sum),0) as qty_sum,isnull(sum(amt),0) as amt_sum from (select distinct t.part_nm,sum(t.QTY) as qty_sum,t.phase as phase,sum(t.amt) as amt FROM (select distinct a.part_nm,sum(a.qty_act) AS QTY,sum(dbo.lp_konprc('$term_s','USD',curr,price_tot*qty_act)) as amt,a.phase as phase,(select top 1 b.status from bps_approve b where b.no_doc=a.pr_no and b.status='finish') as status_dok
			 from bps_tmpPR a where a.term='$term_sum' and a.pr_no like '$sect_s%' and a.tgl_updt<='".$tahunnya1."-12-31 23:59:59' group by a.part_nm,a.pr_no,a.phase) t where t.status_dok='finish' and t.phase='$category' and t.part_nm='$part_nm' and t.phase='$category' group by t.part_nm,t.phase,t.amt
			 union
select distinct a.part_nm,sum(a.qty) as qty_sum,a.phase,sum(dbo.lp_konprc('$term_s','USD',curr,a.qty*a.price)) as amt from bps_budget a where not exists (select no_ctrl from bps_tmpPR where no_ctrl=a.no_ctrl  and tgl_updt<'".$tahunnya."-01-01') and periode>='".$tahunnya."01' and term='$term_sum' and sect='$sect_s' and part_nm='$part_nm' AND phase='$category' group by a.part_nm,a.phase) X ";
*/

$qry_sum_bef="SELECT isnull(sum(qty_sum),0) as qty_sum,isnull(sum(amt),0) as amt_sum from (select distinct t.part_nm,sum(t.QTY) as qty_sum,t.phase as phase,sum(t.amt) as amt FROM (select distinct q.part_nm,sum(q.qty_act) AS QTY,sum(dbo.lp_konprc('$term_s','USD',curr,price_tot*qty_act)) as amt,q.phase as phase from bps_tmpPR q where q.term='$term_sum' and q.pr_no like '$sect_s%' and q.tgl_updt<='".$tahunnya1."-12-31 23:59:59' and exists (select top 1 b.status from bps_approve b where b.no_doc=q.pr_no and b.status='finish') group by q.part_nm,q.phase) t where t.phase='$category' and t.part_nm='$part_nm' group by t.part_nm,t.phase,t.amt union select distinct a.part_nm,sum(a.qty) as qty_sum,a.phase,sum(dbo.lp_konprc('$term_s','USD',curr,a.qty*a.price)) as amt from bps_budget a where not exists (select no_ctrl from bps_tmpPR where no_ctrl=a.no_ctrl and tgl_updt<'".$tahunnya."-01-01') and periode>='".$tahunnya."01' and term='$term_sum' and sect='$sect_s' and part_nm='$part_nm' AND phase='$category' group by a.part_nm,a.phase) X";

//$qry_sum_bef="SELECT isnull(sum(qty_sum),0) as qty_sum,isnull(sum(amt),0) as amt_sum from (select distinct t.part_nm,sum(t.QTY) as qty_sum,t.phase as phase,sum(t.amt) as amt FROM (select distinct a.part_nm,sum(a.qty_act) AS QTY,sum(dbo.lp_konprc('$term_s','USD',curr,price_tot*qty_act)) as amt,a.phase as phase,(select top 1 b.status from bps_approve b where b.no_doc=a.pr_no and b.status='finish') as status_dok
//			 from bps_tmpPR a where a.term='$term_sum' and a.pr_no like '$sect_s%' and a.periode<='".$tahunnya1."12' group by a.part_nm,a.pr_no,a.phase) t where t.status_dok='finish' and t.phase='$category' and t.part_nm='$part_nm' and t.phase='$category' group by t.part_nm,t.phase,t.amt
//			 union
//select distinct a.part_nm,sum(a.qty) as qty_sum,a.phase,sum(dbo.lp_konprc('$term_s','USD',curr,a.qty*a.price)) as amt from bps_budget a where not exists (select no_ctrl from bps_tmpPR where no_ctrl=a.no_ctrl and tgl_updt<'".$tahunnya."-01-01') and periode>='".$tahunnya."01' and term='$term_sum' and sect='$sect_s' and part_nm='$part_nm' AND phase='$category' group by a.part_nm,a.phase) X ";


//echo $qry_sum_bef;
//			$qry_sum_bef= "select distinct t.part_nm,sum(t.QTY) as qty_sum,t.phase as phase,(select sum(dbo.lp_konprc('$term_s','USD',curr,price_tot*qty_act)) from bps_tmpPR where part_nm='$part_nm' and pr_no like '$sect_s%' and term='$term_sum') as amt_usd FROM (select distinct a.part_nm,sum(a.qty_tot) AS QTY,a.phase as phase,(select top 1 b.status from bps_approve b where b.no_doc=a.pr_no and b.status='finish') as status_dok from bps_tmpPR a where a.term='$term_sum' and a.pr_no like '$sect_s%' group by a.part_nm,a.pr_no,a.phase) t where t.status_dok='finish' and t.phase='$category' and t.part_nm='$part_nm' group by t.part_nm,t.phase";
//	echo $qry_sum_bef;		
//echo $qry_sum_bef;
				$tb_sum=odbc_exec($koneksi_lp,$qry_sum_bef);
				$row_s=0;
						...
						//	$part_nm_s=odbc_result($tb_sum,"part_nm");
							$qty_sum_s=odbc_result($tb_sum,"qty_sum");
							$amt_sum_s=odbc_result($tb_sum,"amt_sum");
			
		//	$S2->setCellValueByColumnAndRow(30,$baris_s,$part_nm_s);
			$S2->setCellValueByColumnAndRow(31,$baris_s,$qty_sum_s); 			
			$S2->setCellValueByColumnAndRow(32,$baris_s,$amt_sum_s); 	
			$S2->setCellValueByColumnAndRow(33,$baris_s,"=IFERROR((AC".$baris_s."-AF".$baris_s.")/AF".$baris_s.",0)");
			$S2->setCellValueByColumnAndRow(34,$baris_s,"=IFERROR((AD".$baris_s."-AG".$baris_s.")/AG".$baris_s.",0)");

						}

$S2->setCellValueByColumnAndRow(0,$baris_s,odbc_result($tb_peri1_s,"phase"));			
//$S2->setCellValueByColumnAndRow(1,$baris_s,odbc_result($tb_peri1_s,"sub_acc"));			
$S2->setCellValueByColumnAndRow(3,$baris_s,odbc_result($tb_peri1_s,"part_nm"));			
$S2->setCellValueByColumnAndRow(35,$baris_s,odbc_result($tb_peri1_s,"remark"));			
...
$Qry3="select distinct phase,part_nm,remark,urut,ctg,cost_push,cost_reduction,cost_other,nariyuki from bps_budget_stp where term='$term_s' $sec_s and revisi='$rev_s' order by urut asc";
$baris_a  = 14; //Ini untuk dimulai baris datanya
		...
		$cost_other="0";
				...
				//$sub_acc=odbc_result($tb_part_a,"sub_acc");
				$category=odbc_result($tb_part_a,"phase");
				$remark=odbc_result($tb_part_a,"remark");
...
$peri_a= "select min(periode) as minperiode, max(periode) as maxperiode from bps_budget_stp where term='$term_s' ";
		$tb_peri_a=odbc_exec($koneksi_lp,$peri_a);
		
		while($baris_stp=odbc_fetch_array($tb_peri_a)){
			$row1_a++;
			
			$S3->setCellValue('B2', 'TERM '.$term_s.' ('.strtoupper(date("F Y",strtotime(odbc_result($tb_peri_a,"minperiode")."01"))). ' - ' .strtoupper(date("F Y",strtotime(odbc_result($tb_peri_a,"maxperiode")."01"))). ')' ); 
			
		}
		
				$peri_s_a= "select distinct periode from bps_budget_stp where term='$term_s' order by periode asc";
	
				$tb_peri_s_a=odbc_exec($koneksi_lp,$peri_s_a);
		
					while($baris_s22=odbc_fetch_array($tb_peri_s_a)){
/*
					$S2->setCellValueByColumnAndRow($column,$bariscolp,date("M-y",strtotime(odbc_result($tb_peri_s_a,"periode")."01")));
					$S2->setCellValueByColumnAndRow($column+12,$bariscolp,date("M-y",strtotime(odbc_result($tb_peri_s_a,"periode")."01")));
*/
	$year_a=date("Y",strtotime(odbc_result($tb_peri_s_a,"periode")."01"));
	$year2_a=$year_a-1; 

				$peri1_s_a= "select distinct phase,part_nm,sect,remark,ctg,cost_push,cost_reduction,cost_other,term,(select sum(qty) from bps_budget_stp where part_nm='$part_nm' $sec_s and periode='".odbc_result($tb_peri_s_a,"periode")."' and term='$term_s' and phase='$category' ) as qty_month,(select sum(dbo.lp_konprc(term,'USD',curr,price*qty)) from bps_budget_stp where part_nm='$part_nm' $sec_s and periode='".odbc_result($tb_peri_s_a,"periode")."' and term='$term_s' and phase='$category') as usd_month,periode,(select sum(qty) from bps_budget_stp where part_nm='$part_nm' $sec_s and term='$term_s' and phase='$category' ) as total_qty,(select sum(dbo.lp_konprc(term,'USD',curr,price*qty)) from bps_budget_stp where part_nm='$part_nm' $sec_s and term='$term_s' and phase='$category' ) as total_usd
				from bps_budget_stp where part_nm='$part_nm' $sec_s and term='$term_s' and periode='".odbc_result($tb_peri_s_a,"periode")."' and phase='$category' order by phase asc ";

								
												$tb_peri1_s_a=odbc_exec($koneksi_lp,$peri1_s_a);
											//	$total_qty=0;
												
													while($baris22_s_a=odbc_fetch_array($tb_peri1_s_a)){
														
													//	$S3->setCellValueByColumnAndRow($column,$baris_s,odbc_result($tb_peri1_s_a,"qty_month"));
													//	$S3->setCellValueByColumnAndRow($column+12,$baris_s,odbc_result($tb_peri1_s_a,"usd_month"));
													//	$S3->setCellValueByColumnAndRow($column+13,$baris_s,odbc_result($tb_peri1_s_a,"total_qty"));
													$cost_other=odbc_result($tb_peri1_s_a,"cost_other");
										$S3->setCellValueByColumnAndRow(9,$baris_a,odbc_result($tb_peri1_s_a,"total_usd"));				
													}	
	
			}

/*
$qry_sum_bef_a="SELECT isnull(sum(qty_sum),0) as qty_sum,isnull(sum(amt),0) as amt_sum from (select distinct t.part_nm,sum(t.QTY) as qty_sum,t.phase as phase,sum(t.amt) as amt FROM (select distinct a.part_nm,sum(a.qty_act) AS QTY,sum(dbo.lp_konprc('$term_s','USD',curr,price_tot*qty_act)) as amt,a.phase as phase,(select top 1 b.status from bps_approve b where b.no_doc=a.pr_no and b.status='finish') as status_dok
			 from bps_tmpPR a where a.term='$term_sum' and a.pr_no like '$sect_s%' and a.tgl_updt<='".$tahunnya1."-12-31 23:59:59' group by a.part_nm,a.pr_no,a.phase) t where t.status_dok='finish' and t.phase='$category' and t.part_nm='$part_nm' and t.phase='$category' group by t.part_nm,t.phase,t.amt
			 union
select distinct a.part_nm,sum(a.qty) as qty_sum,a.phase,sum(dbo.lp_konprc('$term_s','USD',curr,a.qty*a.price)) as amt from bps_budget a where not exists (select no_ctrl from bps_tmpPR where no_ctrl=a.no_ctrl  and tgl_updt<'".$tahunnya."-01-01') and periode>='".$tahunnya."01' and term='$term_sum' and sect='$sect_s' and part_nm='$part_nm' AND phase='$category' group by a.part_nm,a.phase) X ";
*/
$qry_sum_bef_a="SELECT isnull(sum(qty_sum),0) as qty_sum,isnull(sum(amt),0) as amt_sum from (select distinct t.part_nm,sum(t.QTY) as qty_sum,t.phase as phase,sum(t.amt) as amt FROM (select distinct q.part_nm,sum(q.qty_act) AS QTY,sum(dbo.lp_konprc('$term_s','USD',curr,price_tot*qty_act)) as amt,q.phase as phase from bps_tmpPR q where q.term='$term_sum' and q.pr_no like '$sect_s%' and q.tgl_updt<='".$tahunnya1."-12-31 23:59:59' and exists (select top 1 b.status from bps_approve b where b.no_doc=q.pr_no and b.status='finish') group by q.part_nm,q.phase) t where t.phase='$category' and t.part_nm='$part_nm' group by t.part_nm,t.phase,t.amt union select distinct a.part_nm,sum(a.qty) as qty_sum,a.phase,sum(dbo.lp_konprc('$term_s','USD',curr,a.qty*a.price)) as amt from bps_budget a where not exists (select no_ctrl from bps_tmpPR where no_ctrl=a.no_ctrl and tgl_updt<'".$tahunnya."-01-01') and periode>='".$tahunnya."01' and term='$term_sum' and sect='$sect_s' and part_nm='$part_nm' AND phase='$category' group by a.part_nm,a.phase) X";
				...
				$tb_sum_bef_a=odbc_exec($koneksi_lp,$qry_sum_bef_a);
				$row_s_a=0;
						while($baris_s2_a=odbc_fetch_array($tb_sum_bef_a)){
							$row_s_a++;
							
						//	$qty_sum_s=odbc_result($tb_sum_bef_a,"qty_sum");
							$amt_sum_s=odbc_result($tb_sum_bef_a,"amt_sum");
							
		//	$S3->setCellValueByColumnAndRow(31,$baris_s,$qty_sum_s); 			
			$S3->setCellValueByColumnAndRow(4,$baris_a,$amt_sum_s); 	
...
//$ews2->setCellValue('b3', '=IF(C'.$baris_a.'="F",E'.$baris_a.',E'.$baris_a.'/$E$'.$baris_a.'*$F$'.$baris_a.')');
...
$sql_bud_s= "select distinct part_nm,(select sum(dbo.lp_konprc(term,'USD',curr,price*qty)) from bps_budget_FA where part_nm='$part_nm' $sec_s and term='$term_sum') as total_amt_a from bps_budget_FA where part_nm='$part_nm' $sec_s and term='$term_sum' group by part_nm";
		$tb_sql_bud_s=odbc_exec($koneksi_lp,$sql_bud_s);
		
		while($baris_bud_s=odbc_fetch_array($tb_sql_bud_s)){
			
			$S3->setCellValueByColumnAndRow(3,$baris_a,odbc_result($tb_sql_bud_s,"total_amt_a"));
//echo $sql_bud_s;
		}
		

$S3->setCellValueByColumnAndRow(0,$baris_a,odbc_result($tb_peri1_s_a,"phase"));						
$S3->setCellValueByColumnAndRow(1,$baris_a,odbc_result($tb_peri1_s_a,"part_nm"));	
$S3->setCellValueByColumnAndRow(11,$baris_a,odbc_result($tb_peri1_s_a,"remark"));
$S3->setCellValueByColumnAndRow(2,$baris_a,odbc_result($tb_peri1_s_a,"ctg"));
...
//$S3->setCellValueByColumnAndRow(6,$baris_a,odbc_result($tb_peri1_s_a,"cost_push"));
//$S3->setCellValueByColumnAndRow(7,$baris_a,odbc_result($tb_peri1_s_a,"cost_reduction"));
...
//set tinggi row
/*
$excelku->getActiveSheet(3)->getRowDimension('1')->setrowHeight(16.25);
$excelku->getActiveSheet(3)->getRowDimension('2')->setrowHeight(16.25);
$excelku->getActiveSheet(3)->getRowDimension('4')->setrowHeight(16.25);
$excelku->getActiveSheet(3)->getRowDimension('4')->setrowHeight(16.25);
$excelku->getActiveSheet(3)->getRowDimension('5')->setrowHeight(16.25);
$excelku->getActiveSheet(3)->getRowDimension('6')->setrowHeight(16.25);

// Mergecell, menyatukan beberapa kolom
$excelku->getActiveSheet(3)->mergeCells('A4:A7');  
$excelku->getActiveSheet(3)->mergeCells('B4:B7');  
$excelku->getActiveSheet(3)->mergeCells('C4:C7');   */
/*
$excelku->getActiveSheet(3)->mergeCells('I2:I4');  
$excelku->getActiveSheet(3)->mergeCells('J2:J4');  
$excelku->getActiveSheet(3)->mergeCells('K2:K4');  
$excelku->getActiveSheet(3)->mergeCells('B7:B10');  
$excelku->getActiveSheet(3)->mergeCells('C7:C10');  
$excelku->getActiveSheet(3)->mergeCells('F7:J7');  
$excelku->getActiveSheet(3)->mergeCells('K7:K9');  
$excelku->getActiveSheet(3)->mergeCells('L7:L10'); 

//SET fill

$S4->getStyle('C1:D2')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('ccffff');
$S4->getStyle('C7')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('ccffff');
$S4->getStyle('B11:L12')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('9bc2e6');

//set style font
$S4->getStyle('B1:B4')->getFont()->setBold(true)->setName('Calibri')->setSize(11)->getColor()->setRGB('000000');
$S4->getStyle('B7:L12')->getFont()->setBold(true)->setName('Calibri')->setSize(11)->getColor()->setRGB('000000');  */
/*
//SET border
$S4->getStyle('I1:K5')->getBorders()->getAllBorders() ->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
$S4->getStyle('F7:J7')->getBorders()->getAllBorders() ->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$S4->getStyle('B7:L12')->getBorders()->getVertical() ->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$S4->getStyle('B11:L12')->getBorders()->getOutline() ->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
$S4->getStyle('B7:L10')->getBorders()->getOutline() ->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);

//SET STYLE
$S4->getStyle('B7:L10')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$S4->getStyle('B7:L10')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$S4->getStyle('I1:K5')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$S4->getStyle('I1:K5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); */
...
/*
$S4->setCellValue('B3', 'Expense');
$S4->setCellValue('C2', 'STP 83'); 
$S4->setCellValue('D2', 'ACT 83'); 
$S4->setCellValue('E2', 'Nariyuki'); 
$S4->setCellValue('F2', 'Cost Push'); 
$S4->setCellValue('G2', 'Cost Red.'); 
$S4->setCellValue('H2', 'Other'); 
$S4->setCellValue('I2', 'STP '.$term_s); 
$S4->setCellValue('A8', 'TAKI Graph Expense Term '.$term_sum); 
*/
/*
$S4->setCellValue('B1', 'Refleksi Term '.$term_sum);
$S4->setCellValue('B2', 'Additional Budget');
$S4->setCellValue('B3', 'Item Tidak Dipakai');
$S4->setCellValue('D1', 'STP Term '.$term_s);
$S4->setCellValue('D2', 'Cost Push Harga Naik');
$S4->setCellValue('D3', 'Cost Push Qty Naik');
$S4->setCellValue('D4', 'Cost Push Item Baru');
$S4->setCellValue('D5', 'Cost Reduction');
*/
$S4->setCellValue('B1', 'Refleksi Term '.$term_sum);
...
/*
$S4->setCellValue('L1', 'Refleksi Term '.$term_sum);
$S4->setCellValue('V1', 'STP Term '.$term_s);
$S4->setCellValue('L2', 'Additional Budget');
$S4->setCellValue('P2', 'Item Tidak Dipakai');
$S4->setCellValue('T2', 'Cost Push');
$S4->setCellValue('X2', 'Cost Reduction');
$S4->setCellValue('AB2', 'Qty Naik');
$S4->setCellValue('AD2', 'Item Baru');
$S4->setCellValue('L4', 'No');
$S4->setCellValue('M4', 'Item');
$S4->setCellValue('N4', 'Amount');
$S4->setCellValue('O4', 'Remark');
$S4->setCellValue('P4', 'No');
$S4->setCellValue('Q4', 'Item');
$S4->setCellValue('R4', 'Amount');
$S4->setCellValue('S4', 'Remark');
$S4->setCellValue('T4', 'No');
$S4->setCellValue('U4', 'Item');
$S4->setCellValue('V4', 'Amount');
$S4->setCellValue('W4', 'Remark');
$S4->setCellValue('X4', 'No');
$S4->setCellValue('Y4', 'Item');
$S4->setCellValue('Z4', 'Amount');
$S4->setCellValue('AA4', 'Remark');
$S4->setCellValue('AB4', 'No');
$S4->setCellValue('AC4', 'Item');
$S4->setCellValue('AD4', 'No');
$S4->setCellValue('AE4', 'Item');
*/
/*
//$S4->setCellValue('B2', 'TERM '.$term_s.' (JULY 2024 - JUNE 2025)'); 
/* $S4->setCellValue('B3', 'DEPART/SECT : '.$sect_s);
$S4->setCellValue('C1', 'F'); 
$S4->setCellValue('C2', 'V'); 
$S4->setCellValue('D1', 'FIXED COST'); 
$S4->setCellValue('D2', 'VARIABLE COST'); 
$S4->setCellValue('I1', 'CHECKED 2'); 
$S4->setCellValue('I5', 'MGR/SM'); 
$S4->setCellValue('J1', 'CHECKED 1'); 
$S4->setCellValue('J5', 'CO SPV/ASMAN'); 
$S4->setCellValue('K1', 'PREPARED'); 
$S4->setCellValue('K5', 'SPV');  
$S4->setCellValue('A4', 'DEPT'); 
$S4->setCellValue('B4', 'SECT'); 
$S4->setCellValue('C4', 'CATEGORY'); 
$S4->setCellValue('C8', 'MH/th'); 
$S4->setCellValue('C9', 'Average MH'); 
$S4->setCellValue('D4', 'Term '.$term_sum); 
$S4->setCellValue('D5', 'Plan'); 
$S4->setCellValue('D6', 'Amount'); 
$S4->setCellValue('D7', 'A'); 
$S4->setCellValue('E4', 'Term '.$term_sum); 
$S4->setCellValue('E5', 'Act+Forecast'); 
$S4->setCellValue('E6', 'Amount'); 
$S4->setCellValue('E7', 'B'); 
$S4->setCellValue('F4', 'Term '.$term_s);
$S4->setCellValue('F5', 'Nariyuki'); 
$S4->setCellValue('F6', 'Amount'); 
$S4->setCellValue('F7', 'C'); 
$S4->setCellValue('G5', 'Cost Push'); 
$S4->setCellValue('G6', 'Amount'); 
$S4->setCellValue('G7', 'D'); 
$S4->setCellValue('H5', 'Cost Reduction'); 
$S4->setCellValue('H7', 'Amount'); 
$S4->setCellValue('H7', 'E');
$S4->setCellValue('I5', 'Other'); 
$S4->setCellValue('I6', 'Amount'); 
$S4->setCellValue('I7', 'F'); 
$S4->setCellValue('J5', 'STP'); 
$S4->setCellValue('J6', 'Amount'); 
$S4->setCellValue('J7', 'G'); */
...
//echo $qry_t;
		...
		$row1_at=0;
			...
			//	$cost_reduction=odbc_result($tb_part_at,"cost_reduction");
			//	$cost_other=odbc_result($tb_part_at,"cost_other");
			//	$dept=odbc_result($tb_part_at,"dept");
			//	$sec_det=odbc_result($tb_part_at,"sect_detail");
...
/*
$excelku->getActiveSheet(4)->mergeCells('I2:I4');  
$excelku->getActiveSheet(4)->mergeCells('J2:J4');  
$excelku->getActiveSheet(4)->mergeCells('K2:K4');  
$excelku->getActiveSheet(4)->mergeCells('B7:B10');  
$excelku->getActiveSheet(4)->mergeCells('C7:C10');  
$excelku->getActiveSheet(4)->mergeCells('F7:J7');  
$excelku->getActiveSheet(4)->mergeCells('K7:K9');  
$excelku->getActiveSheet(4)->mergeCells('L7:L10');  */
...
/* $S5->setCellValue('B3', 'DEPART/SECT : '.$sect_s);
$S5->setCellValue('C1', 'F'); 
$S5->setCellValue('C2', 'V'); 
$S5->setCellValue('D1', 'FIXED COST'); 
$S5->setCellValue('D2', 'VARIABLE COST'); 
$S5->setCellValue('I1', 'CHECKED 2'); 
$S5->setCellValue('I5', 'MGR/SM'); 
$S5->setCellValue('J1', 'CHECKED 1'); 
$S5->setCellValue('J5', 'CO SPV/ASMAN'); 
$S5->setCellValue('K1', 'PREPARED'); 
$S5->setCellValue('K5', 'SPV');  */
...
/*
$Qry4="select distinct phase,(select sum(cost_push) from bps_budget_stp where phase='$category' and term='$term_s' ) as cost_push1,dept,sect_detail from bps_budget_stp where term='$term_s' and revisi='$rev_s' group by dept,sect_detail,phase,cost_push order by dept,sect_detail,phase";
$baris_af  = 11; //Ini untuk dimulai baris datanya
		...
		$row1_af=0;
			...
			//	$part_nm=odbc_result($tb_part_af,"part_nm");
			//	$sub_acc=odbc_result($tb_part_af,"sub_acc");
				$category=odbc_result($tb_part_af,"phase");
			//	$remark=odbc_result($tb_part_af,"remark");
			//	$ctg=odbc_result($tb_part_af,"ctg");
				$cost_push=odbc_result($tb_part_af,"cost_push1");
			//	$cost_reduction=odbc_result($tb_part_af,"cost_reduction");
			//	$cost_other=odbc_result($tb_part_af,"cost_other");
...
$S5->setCellValueByColumnAndRow(2,$baris_af,$category);
$S5->setCellValueByColumnAndRow(3,$baris_af,$cost_push);
...
$total_usd="0";
				 ...
				 from bps_budget_stp where sect='$deptsect' and term='$term_s' and periode='".odbc_result($tb_peri_s_a,"periode")."' and phase like '$kateg%' group by phase,sect,term,periode order by phase asc ";
											...
											//	$total_qty=0;
													...
													//	$S3->setCellValueByColumnAndRow($column,$baris_s,odbc_result($tb_peri1_s_fa,"qty_month"));
													//	$S3->setCellValueByColumnAndRow($column+12,$baris_s,odbc_result($tb_peri1_s_fa,"usd_month"));
													//	$S3->setCellValueByColumnAndRow($column+13,$baris_s,odbc_result($tb_peri1_s_fa,"total_qty"));
...
select distinct sum(a.qty) as qty_sum,a.phase,sum(dbo.lp_konprc('$term_s','USD',curr,a.qty*a.price)) as amt from bps_budget a where not exists (select no_ctrl from bps_tmpPR where no_ctrl=a.no_ctrl and tgl_updt<'".$tahunnya."-01-01') and periode>='".$tahunnya."01' and term='$term_sum' and sect='$deptsect' and phase like '$kateg%' group by a.phase) X ";
				...
				$row_s_fa=0;
						...
						//	$qty_sum_s=odbc_result($tb_sum_bef_fa,"qty_sum");
							$amt_sum_s=odbc_result($tb_sum_bef_fa,"amt_sum");
							
		//	$S3->setCellValueByColumnAndRow(31,$baris_s,$qty_sum_s); 			
			$S5->setCellValueByColumnAndRow(4,$baris_af,$amt_sum_s); 	
		//	$S5->setCellValueByColumnAndRow(10,$baris_af,"=IFERROR(IF(F".$baris_af.">0,(J".$baris_af."-F".$baris_af.")/F".$baris_af.",100%),0)");
//$S5->setCellValueByColumnAndRow(5,$baris_af,'=IF(C'.$baris_af.'="F",E'.$baris_af.',E'.$baris_af.'/$E$12*$F$12)');
...
//$S5->setCellValueByColumnAndRow(0,$baris_af,odbc_result($tb_peri1_s_fa,"phase"));						
//$S5->setCellValueByColumnAndRow(1,$baris_af,odbc_result($tb_peri1_s_fa,"part_nm"));	
//$S5->setCellValueByColumnAndRow(11,$baris_af,odbc_result($tb_peri1_s_fa,"remark"));
//$S5->setCellValueByColumnAndRow(2,$baris_af,odbc_result($tb_peri1_s_fa,"ctg"));
...
//$blk_range="A250:BS250".$baris;
//$excelku->getActiveSheet()->getStyle("F2:I7")->applyFromArray($styleArray);
