...
//$Mnu=strtoupper($_SESSION['akses']);
    ...
    if ($Mnu == 'ALL') {
        $Mnu1 = " ";
        if ($lokweb == 'TF' || $lokweb == 'JF') {
            $Sql_menu = "SELECT * FROM tbl_menu WHERE app_nm = '$website' AND (lokasi = 'TF-JF' OR lokasi = '$lokweb') ORDER BY grop, no_urut ASC";
        } else {
            $Sql_menu = "SELECT * FROM tbl_menu WHERE app_nm = '$website' AND lokasi = '$lokweb' ORDER BY grop, no_urut ASC";
        }
    } else {
        $Mnu1 = str_replace(",", "','", $Mnu);
        if ($lokweb == 'TF' || $lokweb == 'JF') {
            $Sql_menu = "SELECT * FROM tbl_menu WHERE app_nm = '$website' AND (lokasi = 'TF-JF' OR lokasi = '$lokweb') AND EXISTS (SELECT code_menu FROM grp_akses WHERE code_menu = tbl_menu.code_menu AND ID_AKSES IN('$Mnu1')) ORDER BY grop, no_urut ASC";
			
        } else {
            $Sql_menu = "SELECT * FROM tbl_menu WHERE app_nm = '$website' AND lokasi = '$lokweb' AND EXISTS (SELECT code_menu FROM grp_akses WHERE code_menu = tbl_menu.code_menu AND ID_AKSES IN('$Mnu1')) ORDER BY grop, no_urut ASC";
        }
    }
	//echo $Sql_menu."<>".$Mnu;
