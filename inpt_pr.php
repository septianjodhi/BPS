...
$q_update2="insert into bps_tmpPR(periode,no_ctrl,part_no,part_nm,part_dtl,part_desc,uom,qty_plan,curr,price_plan,Qty_add,Price_add,exp_pr,penawaran,pemakaian,price_quo,qty_tot,price_tot,amount,sisa_bud,pic_updt,tgl_updt,id_tmp_pr,pr_date,pr_no,term,kode_supp,no_quo,cccode,lp) ";
$q_update2val="SELECT periode,no_ctrl,part_no,part_nm,part_dtl,part_desc,uom,qty as qty_plan,curr,price as price_plan,dbo.lp_cr_QPadd('Qty',no_ctrl,'OPEN') as Qty_add,dbo.lp_cr_QPadd('Price',no_ctrl,'OPEN') as Price_add,dbo.cr_waktulp('pr',no_ctrl) as exp_pr,dbo.cr_proseslp('quo',id_proses) as penawaran,dbo.lp_sumactbud(no_ctrl,'amount') as pemakaian,dbo.lp_minmaxquo(no_ctrl,dbo.cr_waktulp('po',no_ctrl),'rekom') as price_quo,'".$np2[1]."' as qty_tot,'".$np2[2]."' as price_tot,'".$np2[3]."' as amount,'".$np2[4]."' as sis_bud,'".$pic."' as pic_updt,getdate() as tgl_updt,'".$idtmp."' as id_tmp_pr,'$pr_date' as pr_date,'$pr_no' as pr_no,term,'$supkod' as kode_supp,'$noquo' as no_quo,cccode,lp from bps_budget where no_ctrl='".$np2[0]."'";
	//	echo $q_update2."<br>";
...
$qry_adpr="insert into bps_pr(pr_no,remark,no_ctrl,pr_date,qty,price,curr,term,periode,sect,cccode,pic_updt,tgl_updt) select pr_no as pr_no,'$rmk_pr' as remark,no_ctrl,pr_date,qty_tot as qty,(case when price_quo=null then price_tot else price_quo end) as price,curr,term,periode,'$sect' as sect,cccode,'$pic' as pic_updt,getdate() as tgl_updt from bps_tmpPR where id_tmp_pr='$idtmp' and periode='$per_tmp' and part_nm='$partnm_tmp'";
//echo $qry_adpr;
...
$pchsec=explode("-",$sect);
$dept=$pchsec[0];
$qry_delaprv="delete from bps_approve where jns_doc in($dokaprv) and no_doc='$pr_no'";
$tb_delaprv=odbc_exec($koneksi_lp,$qry_delaprv);
$qry_adaprv="insert into bps_approve(pic_plan,email_plan,no_doc,tgl_prepaire,jns_doc,sect,initial,approve,no_aprv) SELECT nama as pic_plan,email as email_plan,'$pr_no' as no_doc,getdate() as tgl_prepaire,jns_dok as jns_doc,'$sect' as sect,initial,approve,no_aprv  FROM bps_setApprove where status_akun='aktif' and jns_dok in($dokaprv) and (sect='$sect' or sect='$dept-ALL' or (sect='SAMI-ALL' AND (max_amount='0' or (min_amount<'$amoun_USD' and max_amount>='$amoun_USD'))))";
//$amoun_USD
...
echo '<option value="'.$peri.'">'.$peri.'</option>';
	...
	<label>Curr</label>
	  <select class="selectpicker" data-live-search="true" style="width: 100%;"  name="dt_cur" id="dt_cur" required>
			...
			<?php
			$tb_kurs_code=odbc_exec($koneksi_lp,"select distinct curr from bps_budget where sect='$sect'");
...
echo '<option value="'.$kurs_code.'">'.$kurs_code.'</option>';
	...
	<label>Phase</label>
	  <select class="selectpicker" data-live-search="true" style="width: 100%;"  name="dt_phs" id="dt_phs" required>
			...
			<?php
			$tb_phs=odbc_exec($koneksi_lp,"select distinct phase from bps_budget where sect='$sect'");
...
$qrycrdt="select count(*) as jmc from bps_budget where $whrfix $whr";
...
$whr=" and replace($cmd_cari,' ','') like '%$txt_cari%'";}
	*/								   
$sq_acc="SELECT periode,phase,lp,no_ctrl,part_no,part_nm,part_dtl,part_desc,uom,qty,dbo.lp_cr_QPadd('Qty',no_ctrl,'OPEN') as Qty_add,curr,price,dbo.lp_cr_QPadd('Price',no_ctrl,'OPEN') as Price_add,dbo.cr_waktulp('pr',no_ctrl) as exp_pr,dbo.cr_proseslp('quo',id_proses) as penawaran, dbo.lp_sumactbud(no_ctrl,'amount') as act_bud,dbo.lp_minmaxquo(no_ctrl,dbo.cr_waktulp('po',no_ctrl),'rekom') as price_quo,(select distinct carline from lp_cv where cost_center_code=bps_budget.cccode) as cccode from bps_budget where $whrfix $whr order by periode,part_nm asc";
//
...
$Qadd=number_format(odbc_result($tb_acc,"qty_add"),0);
if($Qadd==0){$tot_qty=$Qplan;}else{$tot_qty=$Qplan+$Qadd;}
$prcplan=odbc_result($tb_acc,"price");
$prcadd=odbc_result($tb_acc,"price_add");
if($prcadd==''){$tot_prc=$prcplan;}else{$tot_prc=$prcplan+$prcadd;}
...
//if(odbc_result($tb_acc,"part_no")=="ELC112"){echo "ketquo= ".$ketquo." and price_quo= ".$price_quo." sis_bud=".$sis_bud." tot_prc=".$tot_prc;}
//if($ketquo=="NO" or $price_quo>0){
	if($sis_bud>0){
		$row++;
$plh="on";$rmx_x="";
if($tot_prc<$price_quo){$plh="off";}//$rmx_x="Harga Kurang";}
...
<td >
<?php if($plh=="on"){ ?>
<div class="switch" ><label><input type="checkbox" name="plh[]" id="plh" value="<?php echo $no_ctrl.'|'.$tot_qty.'|'.$tot_prc.'|'.$amount.'|'.$sis_bud; ?>" onclick="dipilih(this.form);"><span class="lever"></span></label></div>
...
<td><?php echo number_format($act_bud,2,".",","); ?></td>
<td><?php echo number_format($amount-$act_bud,2,".",","); ?></td>
