...
// convert(nvarchar(10), a.tgl_updt,23) as tgl_po,eta,a.pic_updt,a.ppn,isnull(b.no_dok,'-') from bps_podtl a
// left join lp_supp c on a.kode_supp=c.supp_code
// left join bps_kontrak_supp b on c.SUPP_CODE=b.kode_supp
// where a.po_no='$pono' and b.tgl_berakhir>=getdate()";
$qry_supp="SELECT distinct a.kode_supp,c.SUPP_NAME,c.ALAMAT,c.kota,c.kode_pos,c.CP_HP,c.TELP,c.fax,c.EMAIL,c.CP_NAME,a.po_no,
...
//$dtl_po="select bps_podtl.*,(select ppn from lp_supp where supp_code=bps_podtl.kode_supp) as ppn from bps_podtl where po_no='$pono'";
	...
	// if($lp!='LD'){$ppn=floor($t_Amo/odbc_result($tb_dtlpo,"ppn"));}else{$ppn=0;}
			...
			if(odbc_result($tb_qry_supp,"ppn")=="12"){
				$ppn=round(round($t_Amo*11/12)*(odbc_result($tb_qry_supp,"ppn")/100));
				$amn_dpp=$amn_dpp+(round($t_Amo*11/12));
			}else{
		...
		<tr class="odd gradeX">
			...
			<td align="center" valign="middle" nowrap="nowrap"><?php echo number_format(sprintf("%.2f",$qt));if($uo<>"0"){ echo " ".$uo;}?></td>
	...
	<tr>
		<td colspan="4" align="Center" height="21">DDP Nilai Lain (11/12 * Total) </td>
		<td align="center" valign="middle"></td>
		<td align="center" valign="middle"></td>
		<td align="center" valign="middle"><?php echo $curr." ".number_format(sprintf("%.2f",round($amn_dpp)),2,'.',',');?></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td align="center" valign="middle"></td>
		<td align="center" valign="middle">
			<strong><?php echo $curr." ".number_format(sprintf("%.2f",$GT),2,'.',',');?></strong>
		</td>
	</tr>
</tbody>
</table>
