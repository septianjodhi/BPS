...
<table border="1" style="border-collapse: collapse; font-size:10px; width:800px; font-weight: bold;">
	...
	<tbody>
		<?php
		$sql_stk31="select no_ctrl,part_no,part_nm,part_dtl,part_desc,uom,
		(select cv_code from lp_cv where cost_center_code=a.cccode) as cv_code,cccode,curr,price_tot,(case when exists (select top 1 no_ctrl from bps_budget_add where no_ctrl=a.no_ctrl) 
		...
		while(odbc_fetch_array($tb_stk31))
		{
			...
			<tr class="odd gradeX">
				...
				<td align="center" valign="middle" nowrap="nowrap">
					<?php echo number_format(sprintf("%.2f",$prc_tot),3,'.',',');?>
					</td>
			...
			<tr align="center" valign="middle">
				...
				<td align="center" valign="middle" nowrap="nowrap"><?php  if($curr=='IDR'){
					...
					$tb_pln=odbc_exec($koneksi_lp,"select sum(dbo.lp_konprc (term,'USD',curr,price*qty))
					...
					$tbact=odbc_exec($koneksi_lp,"select isnull(sum(dbo.lp_konprc (term,'USD',curr,
						(case when price_tot>=price_plan then price_plan else price_tot end)*
						(case when qty_act>=qty_plan then qty_plan else qty_act end))),0) as 
						act_stp from bps_tmpPR a 
						where periode='$per' and pr_no like '%$sec%' and 
						tgl_updt<=DATEADD(n,1,'$updtpr') and
						no_ctrl in (select no_ctrl from bps_budget where periode=a.periode and no_ctrl=a.no_ctrl) ");
					$amnact=odbc_result($tbact,"act_stp");

					$qry_bal="select sum(isnull(dbo.lp_konprc (term,'USD',curr,price*qty),0)) 
					as act_bud from bps_PR a where sect='$sec' and periode='$per' and
					tgl_updt<='$updtpr' and no_ctrl in ( select no_ctrl from mstr_stp 
					...
					$qry_add=odbc_exec($koneksi_lp,"select sum(a.qty*dbo.lp_konprc (a.term,'USD',a.curr,b.price)) as price_usd from bps_budget_add a 
						left join bps_PR b on a.no_ctrl=b.no_ctrl and a.sect=b.sect and
						a.periode=b.periode 
						where a.periode='$per' and a.sect='$sec' and doc_no is not null and
						b.tgl_updt<='$updtpr' ");
					$amn_add=odbc_result($qry_add,"amn_add");
					...
					<tr>
						...
						<td colspan="3" rowspan="3">
							<ul>
								<li>PLAN STP	(USD) : <?php echo number_format(sprintf("%.2f",$amnpln),2,'.',','); ?></li>
								<li>ACT STP 	(USD) :<?php echo number_format(sprintf("%.2f",$amnact),2,'.',','); ?></li>
								<li>ACT ADD 	(USD) :<?php echo number_format(sprintf("%.2f",$amn_add),2,'.',','); ?></li>
								<li><?= $amn_add) ;?></li>
								<li>BALANCE 	(USD) :<?php echo number_format(sprintf("%.2f",$bal_stp),2,'.',','); ?></li>
								<?php	
								$crbal="select sum(dbo.lp_konprc(term,'USD',curr,qty*price)) as ambal from bps_budget_FA a
	...
	//echo $crbal;
								?>
								<li>UNREQ		(USD) :<?php echo number_format(sprintf("%.2f",$ambal),2,'.',','); ?></li>
							</ul>
						</td>
