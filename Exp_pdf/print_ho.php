...
<table width="780" border="1" style="border-collapse: collapse; font-size:10px; width:790px;">
	...
	<?php
	$sql_stk3="SELECT distinct part_no, part_nm, part_dtl,part_desc,min(tgl_terima), sum(qty) AS qty,uom from bps_serahterima where ho_no='$kd' group by part_no,part_nm,part_dtl,part_desc,uom";
	$tb_stk1=odbc_exec($koneksi_lp,$sql_stk3);
	echo $sql_stk3;
	$row=0;
	while(odbc_fetch_array($tb_stk1))
		{$row++;
			...
			<tr>
				...
				<td><?php echo $part_no ." ".$part_no." ".$part_dtl."".$part_desc;?></td>
