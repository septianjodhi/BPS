...
$sql_stk2="select doc_no,isnull(count(doc_no),0) as jml from bps_budget_add a inner join bps_pr b on a.no_ctrl=b.no_ctrl where pr_no='$kd' and doc_no is not null group by doc_no order by doc_no desc";
...
<table border="1" style="border-collapse: collapse; font-size:10px; width:800px; font-weight: bold;">
		...
		while(odbc_fetch_array($tb_stk31))
		{
			...
			<tr align="center" valign="middle">
				...
				<td align="center" valign="middle" nowrap="nowrap"><?php  if($curr=='IDR'){
					...
					$tbact=odbc_exec($koneksi_lp,"select sum(dbo.lp_konprc (term,'USD',curr,
						(case when price_tot>=price_plan then price_plan else price_tot end)*(case when
						qty_act>=qty_plan then qty_plan else qty_act end))) as act_stp from bps_tmpPR a 
						where periode='$per' and pr_no like '%$sec%' and tgl_updt<=DATEADD(n,1,'$updtpr') and
					...
					$qry_add=odbc_exec($koneksi_lp,"select sum(isnull(dbo.lp_konprc
					(a.term,'USD',a.curr,price*qty),0)) as amn_add from bps_budget_add a
					where sect='$sec' and periode='$per' and doc_no is not null and 
					tgl_updt<='$updtpr' and no_ctrl in ( select no_ctrl from bps_pr 
					...
					<tr>
						...
						<td colspan="3" rowspan="3">
							<ul>
								...
								<li>ACT BUD 	(USD) :<?php echo number_format(sprintf("%.2f",$act_bud),2,'.',','); ?></li>
	...
	//echo $crbal;
								...
								<li>UN-USED		(USD) :<?php echo number_format(sprintf("%.2f",$amnpln-$amnact-$ambal),2,'.',','); ?></li>
							</ul>
						</td>
