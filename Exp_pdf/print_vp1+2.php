...
$sql_stk1="select *,(tot_bayar+(tot_bayar*a.ppn/100)-(tot_bayar*pph/100)) as byr,(select top 1 account from bps_kedatangan where inv_no=a.inv_no) as acc from bps_vp a inner join LP_SUPP b on a.kode_supp=b.SUPP_CODE where vp_no='$kd' order by inv_no asc";
...
while($baris=odbc_fetch_array($tb_stk1)){
	...
	$tot_bayar=$amn+$prc_ppn-$prc_pph-$discount;
...
<table width="782" border="1" style="font-family: Arial Narrow, Helvetica, sans-serif; border-collapse: collapse;">
	<tr>
		...
		<td colspan="10" rowspan="2">
			...
			echo $kd_curr.number_format($total+2,2,",",".");?></strong></p>
			<br/>
			<p>Say  : <em><u><strong><?php echo ucwords(terbilang($total+2)).$id_curr;?></strong></u></em></p>
		</td>
	</tr>
	<tr>
...
<table width="750" height="163" border="1"  frame="box" style="font-family: Arial Narrow, Helvetica, sans-serif; border-collapse: collapse; font-size:14p;">
	<tr>
        ...
        QRCode::png($isi_teks1,$tempdir.$namafile1,$quality1,$ukuran1,$padding1);
				...
				while($baris=odbc_fetch_array($tb_stk1)){
					...
					$discount=odbc_result($tb_stk1,"discount");
					$tot_bayar=floor($t_amn+$prc_ppn-$prc_pph)-$discount;
					$po=odbc_result($tb_stk1,"inv_no");
					$amn_inv=$amn+floor($amntik*($ppn/100))-floor($amntik*($pph/100));
					if($i>1){
						$amt_adj=0;
					}
					echo "Invoice : ".$po." = ".$curr." ".number_format(floor($amn_inv+2)+$amt_adj,2)."<br>";
				}
				echo $gpo;
				if($discount!=0){
	...
	<table border="1" style="font-family: Arial Narrow, Helvetica, sans-serif; border-collapse: collapse; font-size:14p;">
		<?php
		$sql_aprv="select * from bps_approve where jns_doc='VP' and no_doc='$kd' and initial<>'FA' AND initial<>'RMU' order by no_aprv desc";
		$tb_aprv=odbc_exec($koneksi_lp,$sql_aprv);
		$bar1="";$bar2="";$bar3="";
		while(odbc_fetch_array($tb_aprv)){
			$bar1=$bar1.'<td width="200" colspan="2" align="center">'.odbc_result($tb_aprv,"approve").'</td>';
