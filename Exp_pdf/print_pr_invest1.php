...
$sql_stk1="select distinct a.PR_NO,a.PR_DATE,a.no_ctrl,a.Qty,a.price,a.term,a.periode,
a.remark,a.pic_updt,a.tgl_updt,a.sect,a.curr,a.cccode,a.carline_code,a.equipment_no,
b.no_ctrl,b.sub_term,b.cip_no,b.bud_group,b.qty,b.price,b.order_plan,b.del_plan,
b.carline,b.expired 
from bps_PR a inner join bps_budget_invest b on a.equipment_no=b.no_ctrl
where pr_no='$kd'";
...
$sql_stk2="select top 1 doc_no,isnull(count(doc_no),0) as jml from bps_budget_add a inner join bps_pr b on a.no_ctrl=b.no_ctrl and a.periode=b.periode where pr_no='$kd' and doc_no is not null and a.doc_date <=b.tgl_updt group by doc_no order by doc_no desc";
...
<table border="1" style="border-collapse: collapse; font-size:10px; width:800px; font-weight: bold;">
	...
	<tbody>
		...
		(select cv_code from lp_cv where cost_center_code=a.cccode) as cv_code, cccode, curr,
		price_tot,(case when exists (select top 1 no_ctrl from bps_budget_add where
		no_ctrl=a.no_ctrl) then (select top 1 expaired from bps_budget_add where no_ctrl=a.no_ctrl
		and periode=a.periode) else dbo.cr_waktulp('vp',no_ctrl) end) as esti,qty_act as qty,
		dbo.lp_konprc(term,'USD',curr,sum(price_tot*qty_act)) as amoUSD,(select cip_no from
		bps_budget_invest where no_ctrl=a.equipment) as cip_no
		from bps_tmpPR a where pr_no='$kd'
		group by part_no,exp_pr,part_nm,part_dtl,part_desc,uom,cccode,curr,price_quo,price_tot,no_ctrl,qty_act,term,periode,req_date,equipment order by part_no";
		echo $sql_stk31;
		...
		{
			$no++;
			$pn=odbc_result($tb_stk31,"part_no");
			$p_dtl=odbc_result($tb_stk31,"part_dtl");
			$pnm=odbc_result($tb_stk31,"part_desc");
			...
			$cr_car=odbc_exec($koneksi_lp,"select Carline_code,carline from lp_cv 
				where cost_center_code='$cccode'");
				...
				<tr>
					...
					<td colspan="3" rowspan="3">
						<ul>
							...
							<?php
							$crbal="select isnull(sum(dbo.lp_konprc(term,'USD',curr,qty*price)),0) as sct_stp from bps_budget_FA a
							where sect='$sec' and periode='$per' and no_ctrl not in ( select no_ctrl from bps_pr b where
							sect=a.sect and periode=a.periode and tgl_updt<='$updtpr')";
		...
		<table border="1" style="border-collapse: collapse; font-size:10px; width:800px; font-weight: bold;">
			...
			<tbody>
				<?php
			// $sql_stk31="select no_ctrl,part_no,part_nm,part_dtl,part_desc,uom,req_date,
			// (select cv_code from lp_cv where cost_center_code=a.cccode) as cv_code,cccode,curr,price_tot,(case when exists (select top 1 no_ctrl from bps_budget_add where no_ctrl=a.no_ctrl) then (select top 1 expaired from bps_budget_add where no_ctrl=a.no_ctrl and periode=a.periode) else dbo.cr_waktulp('vp',no_ctrl) end) as esti,
