	...
	<div class="container-fluid">
						...
						<div class="row clearfix">
							...
							<div class="col-sm-4">
								<div class="form-group">
									<label>Periode</label>
									<div class="form-line">
										<input type="text" readonly class="periodemn form-control" id="periode" name="periode" placeholder="Periode Plan" required>
									</div>
								</div>
							</div>
							<div class="col-md-4">
								<div class="form-group">
									<label>Expired</label>
									<div class="form-line">
										<input type="text" class="form-control date-min" id="exp_date" name="exp_date" placeholder="Expired Date" required>
									</div>
								</div>
							</div>
						</div>
		...
		// $sq_crpart="select * from bps_setperiode";
					if(isset($_POST['smpn']) or isset($_POST['del'])){	
						...
						$expired=$_POST['exp_date'];

						$term=$_POST['term'];
						$pic=$_SESSION['nama'];
						$qry_del="delete from bps_setperiode where periode='$periode'";
						$qry_add="insert into bps_setperiode (term,periode,expaired,pic_updt,tgl_updt) values('$term','$periode','$expired','$pic',getdate())";
						$tb_del=odbc_exec($koneksi_lp,$qry_del);
					}
					if(isset($_POST['smpn']) ){	
//echo $qry_add;
						$tb_add=odbc_exec($koneksi_lp,$qry_add);
						$sq_crpart="select * from bps_setperiode where periode='$periode'";
					}
					?>
							...
							<div class="col-sm-3">	
								<div class="form-group">
									<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="cmd_cari" id="cmd_cari" >
										...
										<option value="kurs_code">Periode</option>
									</select>
								</div>
							</div>
							<div class="col-sm-6">	
								...
								<tbody>
									...
									if(isset($_POST['smpn']) or isset($_POST['cr_b']))
									{
										...
										if($txt_cari==""){$whr="periode is not null"; }else{
											...
											$sq_crpart="select * from bps_setperiode where $whr";
        								// }
        								// if(isset($_POST['smpn']) or isset($_POST['cr_b']))
        								// {	
											...
											while($baris1=odbc_fetch_array($tb_acc)){ $row++;
												...
												<tr onclick="javascript:pilih(this);">
													...
													<td><?php echo date("Ym",strtotime(odbc_result($tb_acc,"periode"))); ?></td>
													<td><?php echo date("Y-m-d",strtotime(odbc_result($tb_acc,"expaired"))); ?></td>
													<td></td>
												</tr>	
												<?php 
											}
	...
	<script>
		function pilih(row){
			...
			document.form1.periode.value=kd_pel2;
			document.form1.exp_date.value=kd_pel3;
		}
	</script>
	<script type="text/javascript">
