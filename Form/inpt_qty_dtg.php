...
/*$del="delete from bps_podtl where po_no not like '%GA%' and kode_supp='".$np2[1]."' and part_no='".$np2[0]."' and pr_no='".$np2[2]."' and lp='$sec'";
...
$updt_po="update bps_podtl set po_no='$po_no',tgl_updt=getdate(),eta='$eta',status_po='OPEN'
where po_no not like '%$sec%' and kode_supp='$np2[1]' and part_no='$np2[0]' and pr_no='$np2[2]' and lp='$sec'";
$tbupdt_po=odbc_exec($koneksi_lp,$updt_po);
echo "<script>alert('$updt_po -  $po_no - $eta - $sec - $np2[1] - $np2[0] - $np2[2] ');</script>";
}
...
<div class="row clearfix">		
	<div class="header">
	<h2>Record<small>Cari Purchase Order ( PO ) Number</small></h2>
	</div>
	<div class="body">
	...
	<label>Purchase Order ( PO ) No.</label>
	  ...
	  <select class="selectpicker" data-live-search="true" style="width: 100%;"  name="ksupp" id="ksupp" required>
			...
			<?php
			$tb_ksupp=odbc_exec($koneksi_lp,"select distinct kode_supp from bps_podtl where po_no not like '%$sec%' and lp='$sec'");
...
<tbody>
<?php
$sq_acc="select a.term,a.periode,a.kode_supp,a.pr_no,a.part_no,a.part_nm,a.part_dtl,a.part_desc,a.uom,sum(qty_act) as qty,a.curr,price_tot,min(dateadd(DD,3,pr_date)) as min_po
...
where po_no not like '%$sec%' and penawaran='YES' and no_aprv=1 and status='FINISH' and a.kode_supp='$kdsuppi' and a.lp='$sec'
group by a.term,a.periode,a.kode_supp,a.pr_no,a.part_no,a.part_nm,a.part_dtl,a.part_desc,a.uom,a.curr,price_tot order by a.pr_no,a.part_no";
...
inner join lp_supp b on a.kode_supp=b.supp_code where a.lp='$sec' and po_no like '%$sec%' group by a.po_no,a.kode_supp,b.SUPP_NAME,a.lp order by min(a.tgl_updt) desc";
...
$tb_del1=odbc_exec($koneksi_lp,"update bps_podtl set po_no=REPLACE('$pono','$sec-$supp_code',''),eta=null,status_po=null where po_no='$pono'");
