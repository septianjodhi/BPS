	...
	<div class="row clearfix">
		<form action="" id="frmcari" name="frmcari" method="post"  enctype="multipart/form-data">
			...
			if(isset($_POST['smpn']) ){
				...
				$tb_inv=odbc_exec($koneksi_lp,"select inv_no from bps_vp where inv_no='$inv_no'");
											$inv_noa="";
											while($tb_inv_no=odbc_fetch_array($tb_inv)){ 
												$inv_noa=odbc_result($tb_inv,"inv_no");												
											}
											if($inv_noa != ""){
												
												//$inv_no=$inv_noa;
												echo "<script>alert('No Invoice = $inv_no  Kembar! ');</script>"; 
												//break;
											} else {
				...
				foreach ($plh as $_boxValue2){
					...
					pic_updt,getdate() as tgl_updt,lp,price,curr,'$pjk' as pph,account,cccode,'$PPNA',uom,'$tgl_rcv' as rcv_inv_date,'$nobc' as no_bc,
