...
?>
 ...
 <div class="container-fluid">
    ...
    <div class="card">				
	<div class="header">
	 <h2>Record Document Approve<small>Lihat data Approve</small></h2>
	</div>
	...
	<form role="form"  name="frm" id="frm" method="post" action="" enctype="multipart/form-data">
		...
		<label>Section</label>
		<div class="form-line">
		<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="sectu" id="sectu" required>
		...
		<option value='NONCHECKED'>TIDAK CHECK</option>	
		</select>
		</div>
...
$whrfil=" and status='FINISH' and (no_doc in (select no_doc from bps_approve where status='close' and no_aprv=1) or no_doc like '%FIN%')";}
elseif($status=='BLMCHECKED'){
$whrfil=" and status is NULL and (no_doc in (select no_doc from bps_approve where status='close' and no_aprv=1) or no_doc like '%FIN%')";}
...
$whrtgl=" tgl_prepaire BETWEEN '".$rg_tgl0." 00:00:00' AND '".$rg_tgl1." 23:59:59'";

if($_GET['prv']=='verified' and $user='RMU'){
	$sq_acc="SELECT distinct jns_doc,status,no_doc,'Rofiul Muayati' as pic_plan,'RMU' as initial,4 as no_aprv,'VERIFIED' as approve from bps_approve where $whrtgl $crsect $whrfil order by jns_doc,no_doc asc";
}else{
...
<td><?php echo $stts; ?></td>
<td>
<?php if($stts=='' and $status=='BLMCHECKED' ){?>
<button type="button" class="btn bg-green waves-effect" onclick="open_child('select/aprv_<?php echo $jns_doc?>.php?nomor=<?php echo $row;?>&nodoc=<?php echo $no_doc;?>&jnsdok=<?php echo $jns_doc;?>&noaprv=<?php echo $no_aprv;?>&initial=<?php echo $initial;?>&picplan=<?php echo $pic_plan;?>&area=<?php echo $sect;?>','Lihat Detail <?php echo $no_doc;?>','800','500'); return false;"><i class="material-icons">visibility</i>
</button>
<?php 
}else{}
?>
