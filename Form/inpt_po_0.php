...
if(isset($_POST['smpn_po']) ){
	...
	$qry_nopo="select max(RIGHT(po_no,3)) as jj from bps_podtl where lp='$pch_sect[1]' and kode_supp='$kd_spp' and convert(nvarchar(6),tgl_updt,112)='$blnth'";
	//periode='$per_tmp'";
...
<div class="row clearfix">		
	...
	<label>Supplier</label>
	  ...
	  <select class="selectpicker" style="width: 100%;"  name="ksupp" id="ksupp" required>
			...
			<?php
			$tb_ksupp=odbc_exec($koneksi_lp,"select distinct kode_supp from bps_podtl where po_no not like 'PO%' and lp='$sec'");
...
?>
	...
	<label>PPN</label>
	<div class="form-line">
	<select class="selectpicker" style="width: 100%;"  name="ppn" id="ppn" required>
	<option selected="selected" value="">---Pilih Nominal---</option>
...
<tbody>
<?php
$sq_acc="select a.term,a.periode,a.kode_supp,a.pr_no,a.part_no,a.part_nm,a.part_dtl,a.part_desc,a.uom,qty_act as qty,a.curr,price_tot,min(dateadd(DD,3,pr_date)) as min_po
...
where po_no not like 'PO-$sec%' and penawaran='YES' and (select status from bps_approve where no_doc=a.pr_no and no_aprv=1) ='FINISH'  and a.kode_supp='$kdsuppi' and a.lp='$sec'
group by a.term,a.periode,a.kode_supp,a.pr_no,a.part_no,a.part_nm,a.part_dtl,a.part_desc,a.uom,a.curr,price_tot,qty_act order by a.pr_no,a.part_no";
...
inner join lp_supp b on a.kode_supp=b.supp_code where a.lp='$sec' and po_no like 'PO%' group by a.po_no,a.kode_supp,b.SUPP_NAME,a.lp order by min(a.tgl_updt) desc";
...
<td>
<a href="##" class="btn bg-green waves-effect"><i onclick="open_child('Exp_pdf/print_po.php?nomor=<?php echo $i;?>&po_no=<?php echo $pono;?>','Print PO <?php echo $pono;?>','800','500'); return false;" class="material-icons">print</i></a>
<a href="#" class="btn bg-red waves-effect" onClick="deletepo()" ><i class="material-icons">delete</i></a>
...
<div class="modal-header"><h4 class="modal-title" id="defaultModalLabel">HAPUS ADDITIONAL BUDGET</h4></div>
...
$tb_del1=odbc_exec($koneksi_lp,"update bps_podtl set po_no=REPLACE(po_no,'$podel','$revpono'),eta=null,status_po=null where po_no='$podel'");
//echo $tb_del1;
