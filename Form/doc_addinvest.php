...
//if(lstctrl==""){alert('ANDA BELUM MEMILIH DATA');}else{
	jQuery.ajax({
		type: 'GET',
		url: 'select/list_noctrl_addinv.php',
		dataType:"text", 
...
if(isset($_POST['smpn_doc']) ){
	...
	$sec=$pchsec[1];
	//$rmk_ad=$_POST['rmk_ad'];
	$qry_nodoc="select max(RIGHT(doc_no,3)) as nn from bps_budget_invest_add where sect='$sect' and convert(nvarchar(4),doc_date,12)='$bln1'";
	...
	$add_docno=$sect."-ADDINVEST-".$bln1."-".$nodoc3;	

	foreach ($_POST['plh'] as $_boxValue2)
	{
		$np2=explode("|",$_boxValue2);	
		$updt_add_budget="update bps_budget_invest_add set doc_no='$add_docno',doc_date=getdate() where no_ctrl_add='$np2[0]'";
	...
	$amoun_IDR=0;
	$sql_amount="select distinct sum(dbo.lp_konprc(term,'IDR','USD',qty*price)) as idr 
	from bps_budget_invest_add where doc_no='$add_docno' group by doc_no";
	$tb_amount=odbc_exec($koneksi_lp,$sql_amount);
	while($bar_moun=odbc_fetch_array($tb_amount)){
		$amoun_IDR=odbc_result($tb_amount,"idr");
	}

	$cr_kdchg=odbc_exec($koneksi_lp, "select distinct kode_chg from bps_budget_invest_add where doc_no='$add_docno' ");
	...
	if ($kode_chg=='4' and $jml_data=='1') {
		$qry_addaprv="insert into bps_approve(pic_plan,email_plan,no_doc,tgl_prepaire,jns_doc,sect,
		initial,  approve,no_aprv)
		SELECT nama as pic_plan,email as email_plan,'$add_docno' as no_doc,getdate() as
		tgl_prepaire,jns_dok as jns_doc,sect,initial,approve,no_aprv  FROM bps_setApprove where jns_dok
		in('ADD') and (sect='$sect' or sect='$dept-ALL' or (sect='SAMI-ALL' AND no_aprv=4))";
	}else{
		$qry_addaprv="insert into bps_approve(pic_plan,email_plan,no_doc,tgl_prepaire,jns_doc,sect,
		initial,  approve,no_aprv)
		SELECT nama as pic_plan,email as email_plan,'$add_docno' as no_doc,getdate() as
		tgl_prepaire,jns_dok as jns_doc,sect,initial,approve,no_aprv  FROM bps_setApprove where jns_dok
		in('ADD') and (sect='$sect' or sect='$dept-ALL' or (sect='SAMI-ALL' AND (max_amount='0' or (min_amount<='$amoun_IDR' and max_amount>'$amoun_IDR'))))";
	}
...
<section class="content">
	<div class="container-fluid">
		<div class="block-header">
			<h2>Additional Budget Invest</h2>
		</div>
		<div class="row clearfix">	
			<div class="card">
				<div class="row clearfix">
					<div class="header">
						<h2>Record<small>Cari Additional Budget Invest</small></h2>
					</div>
					<div class="body">
							...
							<div class="col-sm-3">	
								<div class="form-group">
									...
									<select class="selectpicker" style="width: 100%;"  name="peri" id="peri" required>
										...
										<?php
										$tb_peri=odbc_exec($koneksi_lp,"select distinct periode from bps_budget_invest_add where sect='$sect' and doc_date is NULL  order by periode asc");
		...
		if(isset($_POST['cr_b']) ){	
			$peri=$_POST['peri'];
			$qrycrdt="select count(*) as jmc from bps_budget_invest_add where periode='$peri' and sect='$sect'";
...
//echo $qrycrdt;
				...
				<div class="row clearfix">
					<div class="card">
						<form action="" id="frmcari" name="frmcari" method="post"  enctype="multipart/form-data">
							<div class="row clearfix">				
								<div class="header">
									<h2>Buat Dokumen Additional Invest Section <?php echo $sect; ?></h2>
								</div>
								<div class="body">
									<div class="col-sm-2">	
										<div class="form-group">
								...
								<div class="body">
									<div class="table-wrapper-scroll-y my-custom-scrollbar">
										<table id="dtVerticalScroll_vvvv" class="table table-striped table-bordered " cellspacing="0" width="100%">								
											...
											<tbody>
												<?php
												$sq_acc="select * from bps_budget_invest_add where periode='$peri' and sect='$sect' and doc_date is NULL order by no_ctrl asc";
												...
												while($baris1=odbc_fetch_array($tb_acc)){ 
													...
													if($peri<=$period){
														...
														<tr >				
															...
															<td><?php echo odbc_result($tb_acc,"expired"); ?></td>
														</tr>	
														<?php 
													}
												}
		...
		<div class="row clearfix">
			<div class="card">
				<div class="header">
					<h2>Print Dokumen Additional Invest Section <?php echo $sect; ?></h2>
				</div>
							...
							<div class="table-responsive">
								<table id="example" class="table table-bordered table-striped table-hover dataTable js-exportable tabel2">
									...
									<tbody>
										...
										from bps_budget_invest_add a inner join bps_approve b on a.doc_no=b.no_doc and a.sect=b.sect
										where a.sect='$sect' and doc_no is not NULL and a.periode>=convert(nvarchar(6),getdate(),112)
										...
										while($bar_pr=odbc_fetch_array($tb_docadd)){ $i++;
											...
											<tr onclick="javascript:pilih(this);" >
												...
												<td>
													<button type="button" class="btn bg-green waves-effect" onclick="open_child('Exp_pdf/print_add_invest.php?nomor=<?php echo $i;?>&nodoc=<?php echo $docno;?>','Print Add Budget <?php echo $docno;?>','800','500'); return false;"><i class="material-icons">print</i></button>
													<?php
													$jml_data=odbc_exec($koneksi_lp,"select isnull(count(no_ctrl),0) as jm from bps_PR where no_ctrl in (select no_ctrl from bps_budget_invest_add where doc_no='$docno' )" );
													$jml=odbc_result(	$jml_data, "jm");
													$status_doc=odbc_result($tb_docadd,"status");
													if ($status_doc=='' or $status_doc=='Close') {?>
														<a href="##"><i onclick="open_child('select/rev_add.php?sec=<?php echo $sect;?>&nodoc=<?php echo $docno;?>','Edit ADD <?php echo $docno;?>','800','500'); return false;" class="material-icons">edit</i></a>
														<a href="#" class="btn bg-red waves-effect" onClick="del_addinv()" ><i class="material-icons">delete</i></a>
														<?php 
													}
													?>
												</td>	
...
if(isset($_POST['deladd']) ){	
	$no_add=$_POST["no_add"];
	$tb_del1=odbc_exec($koneksi_lp,"update bps_budget_invest_add set doc_no=null,doc_date=NULL where doc_no='$no_add' ");
	$tb_del=odbc_exec($koneksi_lp,"delete from bps_approve where jns_doc='ADD' and no_doc='$no_add'");

	echo "update bps_budget_invest_add set doc_no=0,doc_date=NULL where doc_no='$no_add'";
}
?>
