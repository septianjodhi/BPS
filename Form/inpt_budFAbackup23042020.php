...
$kolM=$data->val($i,13);//UOM
$kolN=$data->val($i,14);//Currency
$kolAY=$data->val($i,51);//SUB ACCOUNT
$kolAZ=$data->val($i,52);//KODE PROSES
$kolBA=$data->val($i,53);//PURCHASING
$kolBB=$data->val($i,54);//PENAWARAN
$kolBC=$data->val($i,55);//PR
$kolBD=$data->val($i,56);//PO
$kolBE=$data->val($i,57);//KEDATANGAN
$kolBF=$data->val($i,58);//VP
...
$sql_updt="insert into bps_budget(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode,lt_Quo,lt_pr,lt_po,lt_datang,lt_vp) values('$sect_u','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$ctrln','$kolBA','$kolAZ','$kolG','$kolAY','$kolC','$kolD','$kolE','$peri','$qty','$kolM','$kolN','$prc','$kolI','$kolJ','$kolBB','$kolBC','$kolBD','$kolBE','$kolBF')";	
$sql_updtFA="insert into bps_budget_FA(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode) values('$sect_u','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$ctrln','$kolBA','$kolAZ','$kolG','$kolAY','$kolC','$kolD','$kolE','$peri','$qty','$kolM','$kolN','$prc','$kolI','$kolJ')";
$hasil = odbc_exec($koneksi_lp,$sql_updt);	
$hasilFA = odbc_exec($koneksi_lp,$sql_updtFA);
//echo "<br>lht ".$i.$sql_updt;
 if(!$hasilFA){
...
?>	 
	...
	<div class="col-md-2">	
		 <div class="form-group">
				<div class="input-group">
				<select class="selectpicker" data-live-search="true" style="width: 50%;"  name="term_cr" id="term_cr"  required>
				<option selected="selected" value="">---Pilih Term---</option>
				<?php echo $opt_term; ?>
		...
		<div class="form-group">
				<div class="input-group">
				<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="sect_cr" id="sect_cr"  required>
				<option selected="selected" value="">---Pilih Section---</option>
...
<td><?php echo number_format(odbc_result($tb_acc,"qty"),0,".",","); ?></td>
<td><?php echo odbc_result($tb_acc,"uom"); ?></td>
<td><?php echo odbc_result($tb_acc,"curr"); ?></td>
<td><?php echo number_format(odbc_result($tb_acc,"price"),2,".",","); ?></td>
<td><?php echo number_format(odbc_result($tb_acc,"amount"),2,".",","); ?></td>
