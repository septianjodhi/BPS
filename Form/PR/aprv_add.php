...
while(odbc_fetch_array($tb_aprv)){
	if(odbc_result($tb_aprv,"approve")=="REQUEST"){
			...
			$picaprv=odbc_result($tb_aprv,"initial");
	}
...
<?php 
//$listinitial=array("008381","MBM");
		if(in_array('APR_ADD',$ak)){	
			//echo "test tampil";
 ?>
...
<?php }else{echo "TIDAK MENDAPATKAN AKSES VERIFIKASI ADDITIONAL";

} ?>
<?php
if(isset($_POST['apprv']) ){

	$update="update bps_approve set pic_act='$pic_updt',tgl_updt=getdate(),status='FINISH' where no_doc='$kd' and jns_doc='$jnsdok'";
...
if(isset($_POST['unapprv'])){
	$squpdtapp=odbc_exec($koneksi_lp,"update bps_approve set pic_act=NULL,tgl_updt=NULL,status=NULL where no_doc='$kd' and jns_doc='$jnsdok' ");
