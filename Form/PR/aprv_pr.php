...
<div class="card">
	<div class="container-fluid">
		...
		$stts=$_GET['stts'];
		$prv=$_GET['prv'];
//echo "<script>alert('$no_aprv');</script>";
		$aprv_dok=strpos($akses,'APP_PR');
        $kd_akses=explode(",",$akses);
$notenya="";
if($prv=="mgr" && $jnsdok=="PR"){
	
	$notenya="Lanjutkan Proses";
}


	if(isset($_POST['apprv'])){
		$catatan=$_POST['txtcatat'];
		$insnote="insert into bps_noteaprv(pic_aprv,no_doc,jns_doc,approve,status,catatan,pic_updt,tgl_updt) values('$pic_plan','$kd','$jnsdok','$prv','Approve','$catatan','$pic_updt',getdate())";
		
			$tb_acc=odbc_exec($koneksi_lp,$insnote);
			if($_SESSION['lok']=="JF" and  ($_SESSION["area"]=="PGA-IT" or substr($kd,0,6)=="PGA-IT" or substr($kd,0,6)=="PGA-IT" or $_SESSION["area"]=="PGA-GA" or substr($kd,0,6)=="PGA-GA")){
						
						
					$qryhistnote="select max(no_aprv) as noaprv from bps_approve where jns_doc='$jnsdok' and no_doc='$kd'";
					$tb_histnote=odbc_exec($koneksi_lp,$qryhistnote);							
					$noaprv3=0;
						while(odbc_fetch_array($tb_histnote))
						{
							$noaprv3=odbc_result($tb_histnote,"noaprv");
						}
					...
					$noaprv3=0;
						...
						$qryhistnote="select no_aprv from bps_approve where jns_doc='$jnsdok' and no_doc='$kd'  and pic_plan='$pic_updt'  order by no_aprv asc";
					$tb_histnote=odbc_exec($koneksi_lp,$qryhistnote);							
					$actnoaprv3=0;
						while(odbc_fetch_array($tb_histnote))
						{
							$actnoaprv3=odbc_result($tb_histnote,"no_aprv");
						}
						
						if($noaprv3==$actnoaprv3){
							$squpdtapp="update bps_approve set pic_act='$pic_updt',tgl_updt=getdate() where no_doc='$kd' and jns_doc='$jnsdok' and pic_plan='$pic_updt' ";//and sect='$sec' 
							$tb_acc=odbc_exec($koneksi_lp,$squpdtapp);
						
							$squpdtapp="update bps_approve set status='FINISH' where no_doc='$kd' and jns_doc='$jnsdok' ";//and sect='$sec' 
							$tb_acc=odbc_exec($koneksi_lp,$squpdtapp);
							
						}else{
							$squpdtapp="update bps_approve set pic_act='$pic_updt',tgl_updt=getdate(),status='CLOSE' where no_doc='$kd' and jns_doc='$jnsdok' and pic_plan='$pic_updt' ";//and sect='$sec' 
							$tb_acc=odbc_exec($koneksi_lp,$squpdtapp);
				...
				$squpdtapp="update bps_approve set pic_act='$pic_updt',tgl_updt=getdate(),status='FINISH' where no_doc='$kd' and jns_doc='$jnsdok' ";//and sect='$sec' 
				$tb_acc=odbc_exec($koneksi_lp,$squpdtapp);
			}
		...
		$email=odbc_result($tbnoapr,"email_plan");
			...
			if($prv=="mgr" && $jnsdok=="PR"){
				...
				echo '<script>
				alert("APPROVED");document.getElementById("frmcback").submit();	
				</script>';
	...
	if(isset($_POST['unapprv'])){
		$catatan=$_POST['txtcatat'];
		$insnote="insert into bps_noteaprv(pic_aprv,no_doc,jns_doc,approve,status,catatan,pic_updt,tgl_updt) values('$pic_plan','$kd','$jnsdok','$prv','UnApprove','$catatan','$pic_updt',getdate())";
		
			$tb_acc=odbc_exec($koneksi_lp,$insnote);
			
				if($_SESSION['lok']=="JF" and  ($_SESSION["area"]=="PGA-IT" or substr($kd,0,6)=="PGA-IT" or $_SESSION["area"]=="PGA-GA" or substr($kd,0,6)=="PGA-GA")){
						$squpdtapp="update bps_approve set pic_act=NULL,tgl_updt=NULL,status=NULL where no_doc='$kd' and jns_doc='$jnsdok'  and pic_plan='$pic_updt'";//and sect='$sec'
						$tb_acc=odbc_exec($koneksi_lp,$squpdtapp);
					...
					$actnoaprv3=0;
						...
						if($noaprv3==$actnoaprv3){
								$squpdtapp="update bps_approve set status='CLOSE' where no_doc='$kd' and jns_doc='$jnsdok'  and pic_plan<>'$pic_updt'";//and sect='$sec'
								$tb_acc=odbc_exec($koneksi_lp,$squpdtapp);
						}
	...
	if(isset($_POST['rjk'])){
		$catatan=$_POST['txtcatat'];
		$insnote="insert into bps_noteaprv(pic_aprv,no_doc,jns_doc,approve,status,catatan,pic_updt,tgl_updt) values('$pic_plan','$kd','$jnsdok','$prv','Reject','$catatan','$pic_updt',getdate())";
			...
			$noapr="select no_aprv,email_plan from bps_approve where no_doc='$kd' and jns_doc='$jnsdok'";
			$tbnoapr=odbc_exec($koneksi_lp,$noapr);
			$email=odbc_result($tbnoapr,"email_plan");
			echo "<script>
		alert('REJECT');
...
//$tambah="";										   
	...
	//	if($tambah!=""){echo '<h3 style="text-align:right;"><i>ADDITIONAL</i></h3>';}
		...
		<table width="781" border="0">
			<?php if($tambah!=""){echo '<h3 style="text-align:right;"><i>ADDITIONAL</i></h3>';}?>
			<tr>
				...
				<td width="456" align="right">
					<table width="388" border="1" style="border-collapse: collapse; font-family: Arial, Helvetica, sans-serif; font-size: 10px;">
						...
						while(odbc_fetch_array($tb_aprv)){
							...
							if(odbc_result($tb_aprv,"approve")=="VERIFIED"){
								$aprvhdr="REQUEST";
							}else{
								$aprvhdr=odbc_result($tb_aprv,"approve");
							}
							$bar1=$bar1.'<td width="90" align="center">'.$aprvhdr.'</td>';
		...
		<!-- <form action="" id="frmcari" method="post"  enctype="multipart/form-data">
			<div class="col-sm-2" align="left">
				<?php if($stts=='BLMCHECKED'){?>
					...
					<button type="button" align="right"  class="btn bg-purple" onclick="modrejectpr();">REJECT</button>
...
//echo $squpdtapp;
	...
	<form action="" id="frmcari" method="post"  enctype="multipart/form-data">
		<div class="form-group">
			...
			<textarea id="txtcatat" name="txtcatat" style="width:800px;height: 20%;" required><?php echo $notenya;?></textarea>
		</div>
			<div class="col-sm-2" align="left">
				<?php if($stts=='BLMCHECKED'){?>
					<button type="submit" align="right" name="apprv" id="apprv" class="btn bg-purple">APPROVE</button>
					<button type="submit" align="right"  class="btn bg-purple" id="rjk" name="rjk">REJECT</button>
				<?php }else{ ?><button type="submit" align="left" name="unapprv" id="unapprv" class="btn bg-purple">UNAPPROVE</button><?php }?>
		...
		if($prv=="mgr" && $jnsdok=="PR"){
			...
			<form action="../../index.php?page=60002&prv=mgr" id="frmcback" method="post">
			
				<input type="hidden" class="form-control" id="jnsdok" name="jnsdok" value="<?php echo $jnsdok; ?>" required>
				<input type="hidden" class="form-control" id="cr_status" name="cr_status" value="<?php echo $stts; ?>" required>
				<input type="hidden" class="form-control" id="sectu" name="sectu" value="<?php echo $sectu; ?>" required>
				
				<input type="hidden" class="form-control" id="cr_dt" name="cr_dt" value="<?php echo $sectu; ?>" required>
				
				
				
				
				<button type="submit" align="right"  class="btn bg-purple" id="cr_dta" name="cr_dta" value="Send">BACK</button>
			</form>
			<?php
...
<p><h3>History Approval <?=$jnsdok;?> dengan No Dokumen <?=$kd;?></h3></p>
			<table border="1" style="border-collapse: collapse;  width:800px; font-weight: bold;">
			<thead>
				<tr>
					<th width="3%">NO.</th>
					<th width="5%">Status</th>
					<th width="15%">Pic Approve</th>
					<th width="20%">Tanggal Approve</th>
			...
			<tbody>
					...
					<?php 
					$qryhistnote="select * from bps_noteaprv where jns_doc='$jnsdok' and no_doc='$kd'  order by no asc";
					$tb_histnote=odbc_exec($koneksi_lp,$qryhistnote);							
					$barhis=0;
						while(odbc_fetch_array($tb_histnote))
						{
					...
					<tr>
						<td><?=$barhis;?></td>
						<td><?=odbc_result($tb_histnote,"status");?></td>
						<td><?=odbc_result($tb_histnote,"pic_updt");?></td>
						<td><?= date("d M Y H:i:s",strtotime(odbc_result($tb_histnote,"tgl_updt")));?></td>
						<td><?=odbc_result($tb_histnote,"catatan");?></td>
					</tr>
					<?php
						}
					?>
