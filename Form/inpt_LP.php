...
function open_child(url,title,w,h){
 ...
 <div class="container-fluid">
    ...
    <div class="card">
		<div class="header">
             <h2>INPUT<small>Manual Input LP</small></h2>
        ...
        <div class="row clearfix">
				...
				<label>LP Code</label>
				<div class="form-line">
					<input type="text" class="form-control" id="lp_code" name="lp_code" placeholder="LP Code" required>
				</div>
                </div>
				</div>
				<div class="col-md-4">
				...
				<label>Sect.</label>
				<div class="form-line">
					<input type="text" class="form-control" id="sect" name="sect" placeholder="sect" required>
				</div>
                </div>
				</div>
...
$lp_code=$_POST['lp_code'];	
$sect=$_POST['sect'];
$pic=$_SESSION['nama'];
$qry_del="delete from bps_lp where kd_lp='$lp_code'";
$qry_add="insert into bps_lp(kd_lp,dept_lp,pic_updt,tgl_updt) values('$lp_code','$sect','$pic',getdate())";
$tb_del=odbc_exec($koneksi_lp,$qry_del);
}
if(isset($_POST['smpn']) ){	
$tb_add=odbc_exec($koneksi_lp,$qry_add);
$sq_crpart="select * from bps_lp where kd_lp='$lp_code'";
}
?>	 
	...
	<div class="row clearfix">				
	<div class="header">
	 <h2>Record<small>Cari LP</small></h2>
	</div>
	<div class="body">
...
<option value="kd_lp">Purchasing</option>
<option value="dept_lp">Section</option>
       </select>
    </div>
	</div>
	<div class="col-sm-6">	
...
if($txt_cari==""){$whr="kd_lp is not null"; }else{
$whr="replace($cmd_cari,' ','') like '%$txt_cari%'";}
$sq_crpart="select * from bps_lp where $whr";
}
...
<td><?php echo $row; ?></td>
<td><?php echo odbc_result($tb_acc,"kd_lp"); ?></td>
<td><?php echo odbc_result($tb_acc,"dept_lp"); ?></td>
