...
if(isset($_POST['smpn_po']))
{
	...
	$cek_po=odbc_exec($koneksi_lp,"SELECT count(*) as jml_data from $tbl_po where pr_no='$np2[0]' and kode_supp='$supp_code'");
	$cekdatapo=odbc_result($cek_po,"jml_data");

	if ($cekdatapo>0) {
		// code...
		echo "<script>alert('Sudah di buat PO');'</script>";
	} else {
		...
		$data_pr=" SELECT no_ctrl,a.part_no,qty_act,max(b.prioritas) as prioritas ,
		(select prioritas from bps_suppcapacity where periode=a.periode and part_no=a.part_no and supp_code='$supp_code') as no_urut,
		convert (int,qty_act)%convert (int,max(b.prioritas)) as sisa
		from bps_tmpPR a 
		inner join bps_suppcapacity b 
		on a.periode=b.periode and a.part_no=b.part_no and a.lp=b.lp
		where pr_no='$np2[0]' and a.lp='$sec'
		group by no_ctrl,a.part_no,qty_act,a.pr_no,a.periode";

		$qry_pr=odbc_exec($koneksi_lp,$data_pr);
		// echo $data_pr."<br>";

		while($baris_part=odbc_fetch_array($qry_pr)){
			$no_ctrl=$baris_part["no_ctrl"];
			$part_no=$baris_part["part_no"];
			$qty_act=$baris_part["qty_act"];
			$jml_sup=$baris_part['prioritas'];
			$no_urut=$baris_part['no_urut'];
			$sisa=$baris_part['sisa'];

			$qty_order=($qty_act-$sisa)/$jml_sup;

			if($sisa<=$no_urut && $sisa>0 )
			{
				$qty_po=$qty_order+1;
			}else{
				$qty_po=$qty_order;
			...
			a.part_desc,'$po_no' as po_no,'$eta' as eta, '$pic' as pic_updt,
			getdate() as tgl_updt,$qty_po as qty,
			price_tot as price,a.lp,a.term, 
			a.periode,curr,uom, account,cccode,no_ctrl,'OPEN' as status_po, 0 as ppn,0 as pph from bps_tmpPR a inner join bps_suppcapacity b
			on a.periode=b.periode and a.part_no=b.part_no and a.lp=b.lp and a.term=b.term
			where pr_no='$np2[0]' and a.lp='$sec' and a.part_no='$part_no'
			and penawaran='YES' and a.periode='$periode' and no_ctrl='$no_ctrl' and b.supp_code='$supp_code' ";

			$tb_insert=odbc_exec($koneksi_lp,$insert_po." ".$query_pr2);
			//echo $insert_po." ".$query_pr2."</br>";

			$cek_part=odbc_exec($koneksi_lp,"SELECT Top 1 qty,(select sum(qty) from $tbl_po 
				where pr_no=a.pr_no and no_ctrl=a.no_ctrl) as qty_po 
			from bps_PR a where pr_no='$np2[0]' and no_ctrl='$no_ctrl' ");
			$qty_pr=odbc_result($cek_part,"qty");
			$qty_totpo=odbc_result($cek_part,"qty_po");
			$selisih_qty=$qty_totpo-$qty_pr;

			if ($qty_totpo>$qty_pr) {
				// code...
				$update_qty=odbc_exec($koneksi_lp,"UPDATE $tbl_po set qty=qty-$selisih_qty where po_no='$po_no' and pr_no='$np2[0]' and no_ctrl='$no_ctrl' and part_no='$part_no' ");
			}
							...
							<div class="body">
								...
								<div class="col-sm-3">
									<div class="form-group">
										...
										<div class="form-line">
											<select class="selectpicker" name="supp_code" id="supp_code" required>
												<option selected="selected" value="">-Pilih Supplier-</option>
												<?php 
												$tb_supp=odbc_exec($koneksi_lp,"SELECT distinct supp_code from bps_suppcapacity where lp='$sec' and periode='$period' ");
												$row=0;
												while($bar_supp=odbc_fetch_array($tb_supp)){
													$row++;
													$supp_code=odbc_result($tb_supp,"supp_code");
													echo '<option value="'.$supp_code.'">'.$supp_code.'</option>';
												}
												?>
											</select>
										</div>
								...
								<input type="hidden" name="periode" id="periode" value="<?php echo $period; ?>" class="form-control"  >
									...
									<div class="form-group">
										...
										<div class="form-line">
											<select class="selectpicker" style="width: 100%;"  name="ppn" id="ppn" >
												<option selected="selected" value="0">---PPN---</option>
									...
									<div class="form-group">
										...
										<div class="form-line">
											<select class="selectpicker" style="width: 100%;"  name="pph" id="pph" >
												<option selected="selected" value="0">---PPH---</option>
												<option value="0">0%</option>
							...
							<div class="table-responsive">
								<table id="example" class="table table-bordered table-striped table-hover dataTable js-exportable tabel2">						
									...
									<tbody>
										...
										$sq_po="SELECT distinct a.po_no,min(a.tgl_updt) as tgl,a.kode_supp,b.SUPP_NAME,a.lp from $tbl_po a
