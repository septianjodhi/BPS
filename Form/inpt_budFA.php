<script type="text/javascript">
	function open_child(url,title,w,h){
		var left = (screen.width/2)-(w/2);
		var top = (screen.height/2)-(h/2);
		w = window.open(url, title, 'toolbar=no, location=no, directories=no, \n\
			status=no, menubar=no, scrollbar=no, resizabel=no, copyhistory=no,\n\
			width='+w+',height='+h+',top='+top+',left='+left);
	};
	
	function cr_nm(url,title,w,h){
		var nik=document.form1.account.value;
		var pno=document.form1.term.value;
		var pnd=document.form1.periode.value;
		var pernow = new Date();
		var nperi=pernow.getFullYear()+''+("0"+(pernow.getMonth()+1)).slice(-2);
		var balp=parseInt(pnd)-parseInt(nperi);
	 // alert(balp);
	 if(pno=="" || balp<0){alert("Term Belum dipilih Atau Periode Kurang dari Bulan Sekarang");}else{
	 	var left = (screen.width/2)-(w/2);
	 	var top = (screen.height/2)-(h/2);
	 	w = window.open(url+'&ac='+nik, title, 'toolbar=no, location=no, directories=no, \n\
	 		status=no, menubar=no, scrollbar=no, resizabel=no, copyhistory=no,\n\
	 		width='+w+',height='+h+',top='+top+',left='+left);		
	 }
	};
	function cr_part(url,title,w,h){
		var pno=document.form1.part_no.value;
		var pnd=document.form1.part_desc.value;
		var pri=document.form1.periode.value;
		if(pri==""){alert("Periode Belum Diisi");}else{
			var left = (screen.width/2)-(w/2);
			var top = (screen.height/2)-(h/2);
			w = window.open(url+'&pno='+pno+'&pnd='+pnd+'&p='+pri, title, 'toolbar=no, location=no, directories=no, \n\
				status=no, menubar=no, scrollbar=no, resizabel=no, copyhistory=no,\n\
				width='+w+',height='+h+',top='+top+',left='+left);	
		}		
	};
	function cekctrl(url,title,w,h){
		var pno=document.form1.term.value;
		var pnd=document.form1.periode.value;
		var sec=document.form1.sect.value;
		var jns_bud=document.form1.jns_bud.value;
		var lp=document.form1.lp.value;
		var pernow = new Date();
		var nperi=pernow.getFullYear()+''+("0"+(pernow.getMonth()+1)).slice(-2);
		var balp=parseInt(pnd)-parseInt(nperi);
	 // alert(balp);
	 if(pno=="" ||sec=="" || balp<0){alert("Term-Section Belum dipilih Atau Periode Kurang dari Bulan Sekarang");}else{
	 	var left = (screen.width/2)-(w/2);
	 	var top = (screen.height/2)-(h/2);
	 	w = window.open(url+'&t='+pno+'&p='+pnd+'&s='+sec+'&k='+jns_bud+'&lp='+lp, title, 'toolbar=no, location=no, directories=no, \n\
	 		status=no, menubar=no, scrollbar=no, resizabel=no, copyhistory=no,\n\
	 		width='+w+',height='+h+',top='+top+',left='+left);		
	 };
	 
	/*function chgpart(){
		 alert("The text has been changed.");
	
  };
  */
};

function downstp(url,title,w,h){
			
			var term_s=document.form4.term_s.value;
			var sect_s=document.form4.sect_s.value;
			var rev_s=document.form4.for_review.value;
			var akses=document.form4.site.value;
//var term_s='82';
/*
if("<?php echo $admin1 ; ?>"=='admin'){
	  ur="Exp_xls/exp_planbud_stp.php?t="+term_s;
}else{
	  ur="Exp_xls/exp_planbud_stp.php?t="+term_s+"&s="+sect_s;
} */

            var left = (screen.width/2)-(w/2);
            var top = (screen.height/2)-(h/2);
            w = window.open(url+'?t='+term_s+'&s='+sect_s+'&r='+rev_s+'&f='+akses, title, 'toolbar=no, location=no, directories=no, \n\
                status=no, menubar=no, scrollbar=no, resizabel=no, copyhistory=no,\n\
                width='+w+',height='+h+',top='+top+',left='+left);

        };

</script>
<?php
$sect= $_SESSION["area"]; 
$akses=$_SESSION["akses"];
$admin_FA=strpos($akses,'_FA');
$kd_akses=explode(",",$akses);
if(in_array('ADM_FA',$kd_akses)){
	$adm1="admin";
}else{
	$adm1="user";
}

$crlok='<option selected="selected" value="'.$_SESSION['lok'].'">SAMI-'.$_SESSION['lok'].'</option>';
if($_SESSION['lok']=="TF"){
	$crlok=$crlok.'<option value="'.$_SESSION['lok'].'">SAMI-'.$_SESSION['lok'].'</option>';
}
?>
<section class="content">
	<div class="container-fluid">
		<div class="block-header"><h2>BUDGET</h2></div>

		<div class="row clearfix">
			<div class="card">
				<div class="header">
					<ul class="nav nav-tabs tab-nav-right" role="tablist">
						<li role="presentation" class="active"><a href="#manual" data-toggle="tab">Manual Input</a></li>
						<li role="presentation"><a href="#by_upload" data-toggle="tab">Upload</a></li>
						<li role="presentation"><a href="#upload" data-toggle="tab">Upload Baru</a></li>
						<li role="presentation"><a href="#uploadstp" data-toggle="tab">Upload STP</a></li>
						<li role="presentation"><a href="#inputmh" data-toggle="tab">Input Manhour</a></li>
					</ul>	
				</div>
				<div class="tab-content">
					<div role="tabpanel" class="tab-pane fade in active" id="manual">
						<div class="header"><h2>INPUT PLAN BUDGET</h2></div>
						<div class="body">
							<form name="form1" id="form1"  class="step_with_validation" method="POST">
								<h3>STEP 1</h3>
								<fieldset>
									<div class="col-sm-3">	 
										<div class="form-group">
											<label>Term</label>
											<div class="input-group form-float">
												<select class="selectpicker" style="width: 100%;"  name="term" id="term" required>
													<option selected="selected" value="">-Pilih Term-</option>
													<?php 
													//$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm where start_prepaire<=getdate() and finish_term >=getdate() order by term desc");
													$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm order by term desc");
													$opt_term="";
													while($bar_term=odbc_fetch_array($tb_term)){
														$opt_trm=odbc_result($tb_term,"term");
														$opt_term= $opt_term.'<option value="'.$opt_trm.'">TERM '.$opt_trm.'</option>';
													}
													echo $opt_term;
													?>
												</select>
											</div>
										</div>
										<div class="form-group form-float">
											<label>Periode</label>
											<div class="form-line">
												<input type="number" class="periode form-control" id="periode" name="periode" value="<?php echo date("Ym"); ?>" placeholder="Periode" required>
											</div> 
										</div>
									</div>
									<div class="col-sm-3">
										<div class="form-group form-float">
											<label>Section</label>
											<div class="input-group form-float">
												<select class="selectpicker" data-live-search="true" style="width: 70%;"  name="sect" id="sect"  required>
													<option selected="selected" value="">---Pilih Section---</option>
													<?php

													$qsect="select distinct dept_code,sect_code from tbl_sect";
													$tb_acc=mysql_query($qsect);
													while($baris1=mysql_fetch_array($tb_acc)){
														$grpdept=$baris1["dept_code"]."-".$baris1["sect_code"];	
														$opt_sect=$opt_sect.'<option value="'.$grpdept.'">'.$grpdept.'</option>';
													}
													echo $opt_sect;
													?>

												</select>
											</div>
										</div>
										<div class="form-group">
											<label>Cost Center</label>
											<div class="input-group form-float">
												<div class="form-line">
													<input type="text" readonly class="form-control bg-grey" id="cccode" name="cccode" placeholder="Cost Center" required>
												</div>
												<span class="input-group-addon">
													<button type="button" class="btn bg-purple waves-effect"  onclick="open_child('template.php?plh=select/plh_cccode.php&o=cccode&k=1','Data Conveyor','800','500'); return false;"><i class="material-icons">search</i> </button>
												</span>
											</div></div>
										</div>
										<div class="col-sm-3"> 
											<div class="form-group form-float">
												<label>Account</label>
												<div class="input-group">
													<div class="form-line">
														<input type="text"  readonly class="form-control bg-grey" id="account" name="account" placeholder="Account" required>
													</div>
													<span class="input-group-addon">
														<button type="button" class="btn bg-purple waves-effect"  onclick="cr_nm('template.php?plh=select/plh_acc.php&acc=account','Data Conveyor','800','500'); return false;"><i class="material-icons">search</i> </button>
													</span>

												</div>
											</div>
											<div class="form-group form-float">
												<label>Sub Account</label>
												<div class="input-group">
													<div class="form-line">
														<input type="text"  readonly class="form-control bg-grey" id="sub_acc" name="sub_acc" placeholder="Sub Account" required>
													</div>
													<span class="input-group-addon">
														<button type="button" class="btn bg-purple waves-effect"  onclick="cr_nm('template.php?plh=select/plh_acc.php&acc=sub_acc','Data Conveyor','800','500'); return false;"><i class="material-icons">search</i> </button>
													</span>

												</div>
											</div> 
										</div>
										<div class="col-sm-3"> 
											<div class="form-group form-float">
												<div class="form-group">
													<label>Purchasing</label>
													<div class="input-group">
														<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="lp" id="lp"  required>
															<option selected="selected" value="">-Pilih Purchasing-</option>
															<?php
															$qlp="select * from bps_lp order by kd_lp";
															$tb_lp=odbc_exec($koneksi_lp,$qlp);
															while($bar_qlp=odbc_fetch_array($tb_lp)){
																$dt_lp=odbc_result($tb_lp,"kd_lp");
																echo '<option value="'.$dt_lp.'">'.$dt_lp.'</option>';
															}				
															?>
														</select>
													</div>
												</div>
											</div>
										</div>

									</fieldset>

									<h3>STEP 2</h3>
									<fieldset>
										<div class="col-sm-3">
											<div class="form-group form-float">
												<label>Part No</label>
												<div class="form-line">
													<input type="text"  class="form-control bg-grey" id="part_no" name="part_no" placeholder="Part No" required>
												</div>
											</div>
											<div class="form-group form-float">
												<label>Part Name</label>
												<div class="input-group">
													<div class="form-line">
														<input type="text" class="form-control" id="part_nm" name="part_nm"  placeholder="Nama Part" required>
													</div>
													<span class="input-group-addon">
														<button type="button" class="btn bg-purple waves-effect"  onclick="cr_part('template.php?plh=select/plh_partbud.php','Data Conveyor','800','500'); return false;"><i class="material-icons">search</i> </button>
													</span>
												</div>
											</div>
										</div>
										<div class="col-sm-9"> 
											<div class="form-group form-float">
												<label>Detail Part</label>
												<div class="form-line">
													<input type="text"  class="form-control bg-grey" id="part_dtl" name="part_dtl" placeholder="Detail Part" required>
												</div>
											</div>
											<div class="form-group form-float">
												<label>Remark Part</label>
												<div class="form-line">
													<input type="text" class="form-control" id="part_desc" name="part_desc" placeholder="Remark Part">
												</div>
											</div>
										</div>
									</fieldset>

									<h3>STEP 3</h3>
									<fieldset>
										<div class="col-sm-3"> 
											<div class="form-group">
												<label>Phase</label>
												<div class="input-group">
													<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="phase" id="phase"  required>
														<option selected="selected" value="">---Pilih Phase---</option>
														<?php
														$qphase="select distinct phase from bps_phase";
														$tb_phase=odbc_exec($koneksi_lp,$qphase);
														while($bar_qphase=odbc_fetch_array($tb_phase)){
															$dt_phase=odbc_result($tb_phase,"phase");
															echo '<option value="'.$dt_phase.'">'.$dt_phase.'</option>';
														}				
														?>
													</select>
												</div>
											</div>
											<div class="form-group">
												<label>Type</label>
												<div class="input-group">
													<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="jns_bud" id="jns_bud"  required>
														<option selected="selected" value="NORMAL">Normal</option>
														<option value="ADDITIONAL">ADD</option>
													</select>
												</div>
											</div>
										</div>
										<div class="col-sm-3"> 
											<div class="form-group">
												<label>Process Code</label>
												<div class="input-group">
													<div class="form-line">
														<input type="text" readonly class="form-control bg-grey" id="id_proses" name="id_proses" placeholder="Kode Proses" required>
													</div> <span class="input-group-addon">
														<button type="button" class="btn bg-purple waves-effect"  onclick="open_child('template.php?plh=select/plh_pros.php','Data Conveyor','800','500'); return false;"><i class="material-icons">search</i> </button>
													</span>
												</div>
											</div>
										</div>
										<div class="col-sm-6"> 
											<div class="form-group">
												<label>Control No</label>
												<div class="input-group">
													<div class="form-line">
														<input type="text" readonly class="form-control bg-grey" id="no_ctrl" name="no_ctrl" placeholder="No Contro" required>
													</div>
													<span class="input-group-addon">
														<button type="button" class="btn bg-purple waves-effect"  onclick="cekctrl('template.php?plh=select/cek_noctrl.php&o=no_ctrl','Data Conveyor','800','500'); return false;"><i class="material-icons">search</i> </button>
													</span>
												</div>
											</div>
											<div class="col-sm-3">	 
												<div class="form-group">
													<label>Qty</label>
													<div class="form-line">
														<input type="number" class="form-control" id="qty" name="qty" placeholder="Qty" required>
													</div>
												</div>
											</div>
											<div class="col-sm-3">  
												<div class="form-group">
													<label>UOM</label>
													<div class="form-line">
														<input type="text" readonly class="form-control bg-grey"  id="uom" name="uom" placeholder="Uom" required>
													</div>
												</div>
											</div>
											<div class="col-sm-3"><div class="form-group">
												<label>Currency</label>
												<div class="form-line">
													<input type="text" readonly class="form-control bg-grey"  id="curr" name="curr" placeholder="Currency" required>
												</div>
											</div>
										</div>
										<div class="col-sm-3"><div class="form-group">
											<label>Price</label>
											<div class="form-line">
												<input type="number" class="form-control" id="price" name="price" placeholder="Price" required>
											</div>
										</div>
									</div>
								</div>
							</fieldset>
							<h3>STEP 4</h3>
							<fieldset>
								<div class="row clearfix">	
									<div class="col-sm-2"> 
										<div class="form-group">
											<label>LT Quotation</label>
											<div class="form-line">
												<input type="number" class="form-control" id="lt_Quo" name="lt_Quo" min="0"  placeholder="LT Penawaran" required>
											</div>
										</div>
									</div>
									<div class="col-sm-2"> 
										<div class="form-group">
											<label>LT PR</label>
											<div class="form-line">
												<input type="number" class="form-control" id="lt_pr" name="lt_pr" min="0"  placeholder="LT PR" required>
											</div>
										</div>
									</div> 
									<div class="col-sm-2">
										<div class="form-group">
											<label>LT PO</label>
											<div class="form-line">
												<input type="number" class="form-control" id="lt_po" name="lt_po" min="0" placeholder="LT PO" required>
											</div>
										</div>
									</div>
									<div class="col-sm-2"> 
										<div class="form-group">
											<label>LT Arrival</label>
											<div class="form-line">
												<input type="number" class="form-control" id="lt_datang" name="lt_datang"  min="0"  placeholder="LT Arrival" required>
											</div>
										</div>
									</div>
									<div class="col-sm-2">    
										<div class="form-group">
											<label>LT VP</label>
											<div class="form-line">
												<input type="number" class="form-control" id="lt_vp" name="lt_vp" min="0"  placeholder="LT VP" required>
											</div>
										</div>
									</div>
								</div>    
								<div class="row clearfix">		 
									<button type="submit" id="smpn_acc" name="smpn" class="btn bg-green waves-effect"><i class="material-icons">saves</i>SAVE</button>		 
									<button type="submit" id="del_acc" name="del" class="btn bg-red waves-effect"><i class="material-icons">delete</i>Delete</button>

								</div>
							</fieldset>
						</form>
					</div>
				</div>

				<div role="tabpanel" class="tab-pane fade" id="by_upload">

					<div class="header">
						<h2>INPUT<small>Upload Data Budget</small></h2>
						<ul class="header-dropdown m-r--5">
							<li class="dropdown">
								<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
									<i class="material-icons">more_vert</i>
								</a>
								<ul class="dropdown-menu pull-right">
									<li><a href="javascript:void(0);">Action</a></li>
								</ul>
							</li>
						</ul>
					</div>
					<form role="form" enctype="multipart/form-data" name="form2" id="form2" method="post" action="">
						<div class="body">
							<div class="col-lg-12">
								<div class="col-md-3">	
									<div class="form-group">
										<label>Term</label>
										<div class="input-group">
											<select class="selectpicker"  style="width: 50%;"  name="term_u" id="term_u"  required>
												<option selected="selected" value="">---Pilih Term---</option>
												<?php echo $opt_term; ?>

											</select>
										</div>
									</div>
								</div>
								<div class="col-md-3">	
									<div class="form-group">
										<label>Section</label>
										<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="sect_u" id="sect_u"  required>
											<option selected="selected" value="">---Pilih Section---</option>
											<?php 
											echo $opt_sect;
											?>
										</select>
									</div>
								</div>
								<div class="col-md-3">
									<div class="form-group">
										<label>Type</label>
										<div class="input-group">
											<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="jns_bud_u" id="jns_bud_u"  required>
												<option selected="selected" value="NORMAL">Normal</option>
												<option value="ADDITIONAL">ADD</option>
											</select>
										</div>
									</div>
								</div>
								<div class="col-md-3">	
									<div class="form-group">
										<label>Open File</label>
										<div class="form-line">
											<input type="file" class="form-control" id="file" name="file" placeholder="Cari File" required>
										</div>
									</div>
								</div>
							</div>
							<div class="row clearfix">	
								<button type="submit" id="upld" name="upld" class="btn bg-orange waves-effect">
									<i class="material-icons">saves</i>Send Format 1</button>
									<button type="submit" id="upld2" name="upld2" class="btn bg-orange waves-effect">
										<i class="material-icons">saves</i>Send Format 2</button> 
									<button type="submit" id="upldnew" name="upldnew" class="btn bg-orange waves-effect">
										<i class="material-icons">saves</i>Send Format New</button> 
									</div>
								</div>
							</form>	
						</div>
						<!-- Start Upload Format Baru -->
						<div role="tabpanel" class="tab-pane fade" id="upload">

							<div class="header">
								<h2>INPUT<small>Upload Data Budget by Section</small></h2>
								<ul class="header-dropdown m-r--5">
									<li class="dropdown">
										<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
											<i class="material-icons">more_vert</i>
										</a>
										<ul class="dropdown-menu pull-right">
											<li><a href="javascript:void(0);">Action</a></li>
										</ul>
									</li>
								</ul>
							</div>
							<form role="form" enctype="multipart/form-data" name="form3" id="form3" method="post" action="">
								<div class="body">
									<div class="col-lg-12">
										<div class="col-md-3">	
											<div class="form-group">
												<label>Term</label>
												<div class="input-group">
													<select class="selectpicker"  style="width: 50%;"  name="term_u" id="term_u" required>
														<option selected="selected" value="">---Pilih Term---</option>
														<?php 
													//$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm where start_prepaire<=getdate() and finish_term >=getdate() order by term desc");
													$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm order by term desc");
													$opt_term="";
													while($bar_term=odbc_fetch_array($tb_term)){
														$opt_trm=odbc_result($tb_term,"term");
														$opt_term= $opt_term.'<option value="'.$opt_trm.'">TERM '.$opt_trm.'</option>';
													}
													echo $opt_term;
													?>
													</select>
												</div>
											</div>
										</div>									
										<div class="col-md-3">	
											<div class="form-group">
												<label>Open File</label>
												<div class="form-line">
													<input type="file" class="form-control" id="file" name="file" placeholder="Cari File" required>
												</div>
											</div>
										</div>
									</div>
									<div class="row clearfix">	
										<button type="submit" id="upld3" name="upld3" class="btn bg-orange waves-effect">
											<i class="material-icons">saves</i>Upload
										</button>
									</div>
								</div>
							</form>	
						</div>
						<!-- End Upload Format Baru -->
						
						<!--Upload STP-->
					<div role="tabpanel" class="tab-pane fade" id="uploadstp">

					<div class="header">
						<h2>INPUT<small>Upload Plan Budget</small></h2>
						<ul class="header-dropdown m-r--5">
							<li class="dropdown">
								<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
									<i class="material-icons">more_vert</i>
								</a>
								<ul class="dropdown-menu pull-right">
									<li><a href="javascript:void(0);">Action</a></li>
								</ul>
							</li>
						</ul>
					</div>
					<form role="form" enctype="multipart/form-data" name="form4" id="form4" method="post" action="">
						<div class="body">
							<div class="col-lg-12">
								<div class="col-md-3">
<input hidden type="text" id="site" name="site" value='<?php echo $_SESSION['lok']; ?>'>							
									<div class="form-group">
										<label>Term</label>
										<div class="input-group">
											<select class="selectpicker"  style="width: 50%;"  name="term_s" id="term_s"  required>
												<option selected="selected" value="">---Pilih Term---</option>
												<?php 
													//$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm where start_prepaire<=getdate() and finish_term >=getdate() order by term desc");
													$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm order by term desc");
													$opt_term="";
													while($bar_term=odbc_fetch_array($tb_term)){
														$opt_trm=odbc_result($tb_term,"term");
														$opt_term= $opt_term.'<option value="'.$opt_trm.'">TERM '.$opt_trm.'</option>';
													}
													echo $opt_term;
													?>

											</select>
										</div>
									</div>
								</div>
								<div class="col-md-3">	
									<div class="form-group">
										<label>Section</label>
										<div class="input-group"><!-- data-live-search="true" -->
										<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="sect_s" id="sect_s"  required>
											<option selected="selected" value="">---Pilih Section---</option>
											<?php if($kd_akses[1]=='ADM_FA'){?>
											 <option value="ALL">All Section</option>
											 <?php 													
													$tb_section=odbc_exec($koneksi_lp,"select distinct sect from bps_budget_FA order by sect asc");
													//$opt_fra="";
													while($bar_section=odbc_fetch_array($tb_section)){
														$opt_section=odbc_result($tb_section,"sect");
														echo '<option value="'.$opt_section.'">'.$opt_section.'</option>';
													}
													
													?>
											 <?php } ?>
											<?php 
											
											 if($_GET['prv']=='verified' and $adm1== 'admin' ){
												 echo $opt_sect;
											 }else{
												echo '<option value="'.$sect.'">'.$sect.'</option>';
												 
											 }
											
											?>
											<option value="PGA-QS">PGA-QS</option>
											<option value="QA-CAL">QA-CAL</option>
											<option value="NYS-NYS">NYS-NYS</option>
										</select>
									</div>
									</div>
								</div>
								<div class="col-md-3">
									<div class="form-group">
										<label>For Review</label>
										<div class="input-group">
											<select class="selectpicker" data-live-search="true" style="width: 100%;"  name="for_review" id="for_review"  required>
													<option selected="selected" value="">---Pilih For Review---</option>
													<?php if($kd_akses[1]=='ADM_FA' || $sect == 'PGA-IT'){?>
											
													<option value="5">Review 5</option>
											 <?php } ?>
											 
											
													
											 
													<?php 
													
													$tb_term=odbc_exec($koneksi_lp,"select review from bps_stpperiode where start_period<='".date("Y-m-d")."' and finish_period>='".date("Y-m-d")."' order by start_period asc");
													$opt_fra="";
													while($bar_term=odbc_fetch_array($tb_term)){
														$opt_fr=odbc_result($tb_term,"review");
														$opt_fra= $opt_fra.'<option value="'.$opt_fr.'">For Review '.$opt_fr.'</option>';
													}
													echo $opt_fra;
													?>
											</select>
										</div>
									</div>
								</div>
								<div class="col-md-3">	
									<div class="form-group">
										<label>Open File</label>
										<div class="form-line">
											<input type="file" class="form-control" id="file_s" name="file_s" placeholder="Cari File" required>
										</div>
									</div>
								</div>
							</div>
							<div class="row clearfix">	
								<button type="submit" id="upld_s" name="upld_s" class="btn bg-orange waves-effect">
									<i class="material-icons">saves</i>Upload Plan Budget</button>

									<button type="button" class="btn bg-blue waves-effect" onclick="downstp('Exp_xls/exp_planbud_stp.php','Data Conveyor','800','500'); return false;"><i class="material-icons">file_download</i>Download STP</button>
									
									<button type="button" class="btn bg-blue waves-effect" onclick="downstp('Exp_xls/exp_planbud_stp_detail.php','Data Conveyor','800','500'); return false;"><i class="material-icons">file_download</i>Download STP Detail</button>
									
									<button type="submit" id="upld_s_r" name="upld_s_r" class="btn bg-green waves-effect">
									<i class="material-icons">saves</i>Upload Remark Plan Budget</button>
									
									<button type="button" class="btn bg-blue waves-effect" onclick="downstp('Exp_xls/exp_planbud_stp_atable.php','Data Conveyor','800','500'); return false;"><i class="material-icons">file_download</i>Download Budget for A Table</button>
							
									</div>
								</div>
									
								
							</form>
<div class="container-fluid">
    <div class="row clearfix">
<?php 
 include('form/dashboard_stp.php'); 
             ?> 		
</div>
</div>		 
						</div>
	
						<!--Upload STP-->
						
						<!--Input Manhour-->
						
					<div role="tabpanel" class="tab-pane fade" id="inputmh">

					<div class="header">
						<h2>INPUT<small>Input ManHour</small></h2>
						<ul class="header-dropdown m-r--5">
							<li class="dropdown">
								<a href="javascript:void(0);" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
									<i class="material-icons">more_vert</i>
								</a>
								<ul class="dropdown-menu pull-right">
									<li><a href="javascript:void(0);">Action</a></li>
								</ul>
							</li>
						</ul>
					</div>
					<form role="form" enctype="multipart/form-data" name="form5" id="form5" method="post" action="">
						<div class="body">
							<div class="col-lg-12">
								<div class="col-md-3">
									<div class="form-group form-float">
											<label>Term</label>
										<div class="input-group form-float">
											<select class="selectpicker"  style="width: 50%;"  name="term_mh_s" id="term_mh_s"  required>
												<option selected="selected" value="">---Pilih Term---</option>
												<?php 
													//$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm where start_prepaire<=getdate() and finish_term >=getdate() order by term desc");
													$tb_term=odbc_exec($koneksi_lp,"select distinct term from bps_setterm order by term desc");
													$opt_term="";
													while($bar_term=odbc_fetch_array($tb_term)){
														$opt_trm=odbc_result($tb_term,"term");
														$opt_term= $opt_term.'<option value="'.$opt_trm.'">TERM '.$opt_trm.'</option>';
													}
													echo $opt_term;
													?>

												</select>
											</div>
										</div>										
									</div>
									
									
									
								<div class="col-md-12">
									<div class="col-md-3">
										<div class="form-group">
											<label>Input ManHour</label>
											<div class="input-group form-float">
												<div class="form-line">
													<input type="text" class="form-control" id="mh_s" name="mh_s" placeholder="ManHour Term Sebelum" required>
												</div>
											</div>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label>Input ManHour Actual</label>
											<div class="input-group form-float">
												<div class="form-line">
													<input type="text" class="form-control" id="mh_act" name="mh_act" placeholder="ManHour Term Act sd Desember + Plan Januari" required>
												</div>
											</div>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label>Input ManHour Term Plan</label>
											<div class="input-group form-float">
												<div class="form-line">
													<input type="text" class="form-control" id="mh_p" name="mh_p" placeholder="ManHour Term Plan" required>
											</div>
												</div>
											</div>
										</div>
									</div>
								</div>
						<div class="row clearfix">	
								<button type="submit" id="sv_mh_s" name="sv_mh_s" class="btn bg-blue waves-effect">
									<i class="material-icons">saves</i>Simpan</button>
									</div>
								</div>
							</form>	
						</div>
	
						<!-- END Input manHour-->
						
					</div>
				</div>
			</div>

			<?php
			if(isset($_POST['smpn']) or isset($_POST['del'])){	
				$sect=$_POST['sect'];
				$term=$_POST['term'];
				$jns_budget=$_POST['jns_bud'];
				$pic_updt=$_SESSION['nama'];
//$tgl_updt=$_POST['tgl_updt']
				$no_ctrl=$_POST['no_ctrl'];
				$lp=$_POST['lp'];
				$id_proses=$_POST['id_proses'];
				$account=$_POST['account'];
				$sub_acc=$_POST['sub_acc'];
				$part_no=$_POST['part_no'];
				$part_nm=$_POST['part_nm'];
				$part_dtl=$_POST['part_dtl'];
				$part_desc=$_POST['part_desc'];
				$periode=$_POST['periode'];
//$cvcode=$_POST['cvcode'];
				$cccode=$_POST['cccode'];
				$phase=$_POST['phase'];
				$qty=$_POST['qty'];
				$uom=$_POST['uom'];
				$price=$_POST['price'];
				$curr=$_POST['curr'];
				$lt_vp=$_POST['lt_vp'];
				$lt_datang=$_POST['lt_datang'];
				$lt_po=$_POST['lt_po'];
				$lt_pr=$_POST['lt_pr'];
				$lt_Quo=$_POST['lt_Quo'];
				$expaired=date("Y-m-d",strtotime($periode."28"));

				$qry_del=" where no_ctrl='$no_ctrl'";
				$qry_add="insert into bps_budget(sect,term,jns_budget,pic_updt,tgl_updt,expaired,
				no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_desc,part_dtl,periode,qty,uom,price,curr,cccode,phase,lt_Quo,lt_pr,lt_po,lt_datang,lt_vp) values('$sect','$term','$jns_budget','$pic_updt',getdate(),'$expaired','$no_ctrl','$lp','$id_proses','$account','$sub_acc','$part_no','$part_nm','$part_desc','$part_dtl','$periode','$qty','$uom','$price','$curr','$cccode','$phase','$lt_Quo','$lt_pr','$lt_po','$lt_datang','$lt_vp')";
				$qry_addFA="insert into bps_budget_FA(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_desc,part_dtl,periode,qty,uom,price,curr,cccode,phase) values('$sect','$term','$jns_budget','$pic_updt',getdate(),'$expaired','$no_ctrl','$lp','$id_proses','$account','$sub_acc','$part_no','$part_nm','$part_desc','$part_dtl','$periode','$qty','$uom','$price','$curr','$cccode','$phase')";
				$tb_del=odbc_exec($koneksi_lp,"delete from bps_budget ".$qry_del);
				$tb_delFA=odbc_exec($koneksi_lp,"delete from bps_budget_FA ".$qry_del);
				if(isset($_POST['smpn']) ){	
					$tb_add=odbc_exec($koneksi_lp,$qry_add);
					$tb_addFA=odbc_exec($koneksi_lp,$qry_addFA);
					$sq_acc="select *,(qty * price) as amount from bps_budget_FA where no_ctrl='$no_ctrl'";
				}}

				if(isset($_POST['upld']) ){
					require_once "excel_reader2.php";
	echo "<script>alert('upload data');</script>"; 
		$file_name = $_FILES['file']['name']; //nama file (tanpa path)
		$tmp_name  = $_FILES['file']['tmp_name']; //nama local temp file di server
		$file_size = $_FILES['file']['size']; //ukuran file (dalam bytes)
		$file_type = $_FILES['file']['type']; //tipe filenya (langsung detect MIMEnya)
		$fp1 = fopen($tmp_name, 'r'); // open file (read-only, binary)
		$fp = fopen($tmp_name, 'r');		
		$pecah=explode(".",$file_name);
		$ekstensi=$pecah[1];
		$extensionList=array("xls","XLS");
		if(in_array($ekstensi,$extensionList)){		
			$target = basename($_FILES['file']['name']) ;
			move_uploaded_file($_FILES['file']['tmp_name'], $target);			 
			$data = new Spreadsheet_Excel_Reader($_FILES['file']['name'],false);  
			$baris = $data->rowcount($sheet_index=0);
			$fixedbaris = $baris;	 
			$term_u=$_POST['term_u'];
			$sect_u=$data->val(5,2)."-".$data->val(5,4); //$sect_u=$_POST['sect_u'];
			$jns_bud_u=$_POST['jns_bud_u'];
			$pic=$_SESSION['nama'];
			$jmrow=0;
			if($sect_u = $_POST['sect_u']){
				$sql_del=" where term='$term_u' and sect='$sect_u'";
			//echo $sql_del;
				//$hilang=odbc_exec($koneksi_lp,"Delete from bps_budget".$sql_del);
				//$hilangFA=odbc_exec($koneksi_lp,"Delete from bps_budget_FA".$sql_del);
				for ($i=8; $i<=$fixedbaris; $i++){		
//$kolA=$data->val($i,1);//no
$kolB=$data->val($i,2);//CONTROL NO	
$kolC=$data->val($i,3);//PURCHASING
$kolD=$data->val($i,4);//KODE PROSES
$kolE=$data->val($i,5);//ACCOUNT
$kolF=$data->val($i,6);//SUB ACCOUNT
//$kolG=$data->val($i,7);//COUNTER
$kolH=$data->val($i,8);//PART NO
$kolI=$data->val($i,9);//PART NAME
$kolJ=$data->val($i,10);//DETAIL PART
$kolK=$data->val($i,11);//PERIODE
$kolL=str_replace(",","",$data->val($i,12));//QTY
$kolM=$data->val($i,13);//UOM
$kolN=$data->val($i,14);//Currency
$kolO=number_format(str_replace(",","",$data->val($i,15)),10,".","");//PRICE

//$kolP=$data->val($i,16);//AMOUNT
$kolQ=$data->val($i,17);//PHASE
$kolR=$data->val($i,18);//Cv code/cccode
$kolS=$data->val($i,19);//PENAWARAN
$kolT=$data->val($i,20);//PR
$kolU=$data->val($i,21);//PO
$kolV=$data->val($i,22);//KEDATANGAN
$kolW=$data->val($i,23);//VP
$experied=date("Y-m-d",strtotime($kolK."28"));

if($kolB!=""){
	$sql_updt="insert into bps_budget(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode,lt_Quo,lt_pr,lt_po,lt_datang,lt_vp) values('$sect_u','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$kolB','$kolC','$kolD','$kolE','$kolF','$kolH','$kolI','$kolJ','$kolK','$kolL','$kolM','$kolN','$kolO','$kolQ','$kolR','$kolS','$kolT','$kolU','$kolV','$kolW')";	
	$sql_updtFA="insert into bps_budget_FA(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode) values('$sect_u','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$kolB','$kolC','$kolD','$kolE','$kolF','$kolH','$kolI','$kolJ','$kolK','$kolL','$kolM','$kolN','$kolO','$kolQ','$kolR')";
	$hasil = odbc_exec($koneksi_lp,$sql_updt);	
	$hasilFA = odbc_exec($koneksi_lp,$sql_updtFA);
	echo "<br>lht ".$i.$sql_updt;
	if(!$hasilFA){
		echo "<br>Error ".$i.$sql_updt."<br>".$sql_updtFA;
		print(odbc_error());
	}else{ $jmrow++; }
}	}		
unlink($_FILES['file']['name']);	
echo "<script>alert('$jmrow DATA SELESAI DI UPLOAD ');</script>";
$sq_acc="select *,(qty * price) as amount from bps_budget_FA where tgl_updt=getdate() and pic_updt='$pic'";
}else{		
	unlink($_FILES['file']['name']);	
	echo "<script>alert('GAGAL Upload, Dept - Sect Tidak sesuai');</script>";}
}else{ echo "<script>alert('Format file harus XLS'); window.location = '?page=form/inpt_budFA.php'</script>"; }

}


//Start Upload STP

		if(isset($_POST['upld_s']) ){
					require_once "excel_reader2.php";
	
		$file_name = $_FILES['file_s']['name']; //nama file (tanpa path)
		$tmp_name  = $_FILES['file_s']['tmp_name']; //nama local temp file di server
		$file_size = $_FILES['file_s']['size']; //ukuran file (dalam bytes)
		$file_type = $_FILES['file_s']['type']; //tipe filenya (langsung detect MIMEnya)
		$fp1 = fopen($tmp_name, 'r'); // open file (read-only, binary)
		$fp = fopen($tmp_name, 'r');		
		$pecah=explode(".",$file_name);
		$ekstensi=$pecah[1];
		$extensionList=array("xls","XLS");
		$errnya=0;
		if(in_array($ekstensi,$extensionList)){		
			$target = basename($_FILES['file_s']['name']) ;
			move_uploaded_file($_FILES['file_s']['tmp_name'], $target);			 
			$data = new Spreadsheet_Excel_Reader($_FILES['file_s']['name'],false);  
			 
			$term_s=$_POST['term_s'];
			$term=$_POST['term_s'];
			$year=$term-82+2022;
			$year2=$year+1;
			
			$period=date("Ym",strtotime($year."-07-01"));
			$periode=date("Y-m-01",strtotime($year."-07-01"));
				
			$sect_s=$_POST['sect_s'];
			$sect_ss=explode("-",$_POST['sect_s']);
			$dept=$sect_ss[0];
			$sect_det=$sect_ss[1];
			
			$for_review=$_POST['for_review'];
			$pic=$_SESSION['nama'];
			$tgl=date("Y-m-d H:i:s");
			$jmrow=0;
			$ny=0;
			
			$jmlerr=0;
			$jmlerid=0;
			$jmler=0;
				$sql_del=" where term='$term_s' and sect='$sect_s' and revisi='$for_review'";
			//echo $sql_del;
				$hilang=odbc_exec($koneksi_lp,"Delete from bps_budget_stp".$sql_del);
			
			for($j=1;$j<=4;$j++){
				$baris = $data->rowcount($sheet_index=$j);
				$fixedbaris = $baris;
				
				if($j==1){
				$kategori="RUTIN";
				}else if($j==2){
					$kategori="INSIDENTAL";
				}
				else if($j==3){
					$kategori="PROJECT";
				}else if($j==4){
					$kategori="IMPROVEMENT";
				}
				//echo $j."fixedbaris".$fixedbaris."<br>";			
				for ($i=12; $i<=$fixedbaris; $i++){		
//echo $i."<br>";
$kolA=$data->val($i,1,$j);
$kolC=$data->val($i,3,$j);//PN

//$kolD=$data->val($i,4,$j);//PART DESC
//$kolE=$data->val($i,5,$j);//PART DTL
$kolF=str_replace("'"," ",$data->val($i,6,$j));//FUNSGI
$kolG=$data->val($i,7,$j);//OPEX/FOH
$kolJ=$data->val($i,10,$j);//CATEGORY
$kolK=$data->val($i,11,$j);//CC
$kolL=$data->val($i,12,$j);//CARLINE

$kolM=$data->val($i,13,$j);//CARMAKER
$kolBQ=$data->val($i,68,$j);//KATEGORI SUB ACCOUNT

$kolAN=str_replace(",","",$data->val($i,40,$j));//QTY 7
$kolAO=str_replace(",","",$data->val($i,41,$j));//QTY 8
$kolAP=str_replace(",","",$data->val($i,42,$j));//QTY 9
$kolAQ=str_replace(",","",$data->val($i,43,$j));//QTY 10
$kolAR=str_replace(",","",$data->val($i,44,$j));//QTY 11
$kolAS=str_replace(",","",$data->val($i,45,$j));//QTY 12
$kolAT=str_replace(",","",$data->val($i,46,$j));//QTY 1
$kolAU=str_replace(",","",$data->val($i,47,$j));//QTY 2
$kolAV=str_replace(",","",$data->val($i,48,$j));//QTY 3
$kolAW=str_replace(",","",$data->val($i,49,$j));//QTY 4
$kolAX=str_replace(",","",$data->val($i,50,$j));//QTY 5
$kolAY=str_replace(",","",$data->val($i,51,$j));//QTY 6

$kolAN=str_replace("*","",$kolAN);//QTY 7

$kolAO=str_replace("*","",$kolAO);//QTY 8
$kolAP=str_replace("*","",$kolAP);//QTY 9
$kolAQ=str_replace("*","",$kolAQ);//QTY 10
$kolAR=str_replace("*","",$kolAR);//QTY 11
$kolAS=str_replace("*","",$kolAS);//QTY 12
$kolAT=str_replace("*","",$kolAT);//QTY 1
$kolAU=str_replace("*","",$kolAU);//QTY 2
$kolAV=str_replace("*","",$kolAV);//QTY 3
$kolAW=str_replace("*","",$kolAW);//QTY 4
$kolAX=str_replace("*","",$kolAX);//QTY 5
$kolAY=str_replace("*","",$kolAY);//QTY 6

$kolAN=str_replace(" ","",$kolAN);//QTY 7

$kolAO=str_replace(" ","",$kolAO);//QTY 8
$kolAP=str_replace(" ","",$kolAP);//QTY 9
$kolAQ=str_replace(" ","",$kolAQ);//QTY 10
$kolAR=str_replace(" ","",$kolAR);//QTY 11
$kolAS=str_replace(" ","",$kolAS);//QTY 12
$kolAT=str_replace(" ","",$kolAT);//QTY 1
$kolAU=str_replace(" ","",$kolAU);//QTY 2
$kolAV=str_replace(" ","",$kolAV);//QTY 3
$kolAW=str_replace(" ","",$kolAW);//QTY 4
$kolAX=str_replace(" ","",$kolAX);//QTY 5
$kolAY=str_replace(" ","",$kolAY);//QTY 6

$kolAN=str_replace("-","",$kolAN);//QTY 7

$kolAO=str_replace("-","",$kolAO);//QTY 8
$kolAP=str_replace("-","",$kolAP);//QTY 9
$kolAQ=str_replace("-","",$kolAQ);//QTY 10
$kolAR=str_replace("-","",$kolAR);//QTY 11
$kolAS=str_replace("-","",$kolAS);//QTY 12
$kolAT=str_replace("-","",$kolAT);//QTY 1
$kolAU=str_replace("-","",$kolAU);//QTY 2
$kolAV=str_replace("-","",$kolAV);//QTY 3
$kolAW=str_replace("-","",$kolAW);//QTY 4
$kolAX=str_replace("-","",$kolAX);//QTY 5
$kolAY=str_replace("-","",$kolAY);//QTY 6

$kolAN=str_replace("?","",$kolAN);//QTY 7

$kolAO=str_replace("?","",$kolAO);//QTY 8
$kolAP=str_replace("?","",$kolAP);//QTY 9
$kolAQ=str_replace("?","",$kolAQ);//QTY 10
$kolAR=str_replace("?","",$kolAR);//QTY 11
$kolAS=str_replace("?","",$kolAS);//QTY 12
$kolAT=str_replace("?","",$kolAT);//QTY 1
$kolAU=str_replace("?","",$kolAU);//QTY 2
$kolAV=str_replace("?","",$kolAV);//QTY 3
$kolAW=str_replace("?","",$kolAW);//QTY 4
$kolAX=str_replace("?","",$kolAX);//QTY 5
$kolAY=str_replace("?","",$kolAY);//QTY 6


if($kolAN=="" || $kolAN=="-"){
	$kolAN="0";
}
if($kolAO=="" || $kolAO=="-"){
	$kolAO="0";
}
if($kolAP=="" || $kolAP=="-"){
	$kolAP="0";
}
if($kolAQ=="" || $kolAQ=="-"){
	$kolAQ="0";
}
if($kolAR=="" || $kolAR=="-"){
	$kolAR="0";
}
if($kolAS=="" || $kolAS=="-"){
	$kolAS="0";
}
if($kolAT=="" || $kolAT=="-"){
	$kolAT="0";
}
if($kolAU=="" || $kolAU=="-"){
	$kolAU="0";
}
if($kolAV=="" || $kolAV=="-"){
	$kolAV="0";
}
if($kolAW=="" || $kolAW=="-"){
	$kolAW="0";
}
if($kolAX=="" || $kolAX=="-"){
	$kolAX="0";
}

if($kolAY=="" || $kolAY=="-"){
	$kolAY="0";
}
//	echo "CHECK";
$qty[0]=$kolAN;
$qty[1]=$kolAO;
$qty[2]=$kolAP;
$qty[3]=$kolAQ;
$qty[4]=$kolAR;
$qty[5]=$kolAS;
$qty[6]=$kolAT;
$qty[7]=$kolAU;
$qty[8]=$kolAV;
$qty[9]=$kolAW;
$qty[10]=$kolAX;
$qty[11]=$kolAY;
//echo $kolF."-F-".$kolG."-G-".$kolJ."-J-".$kolK."-K-".$kolL."-L-".$kolM."-M-".$kolC."<br>";
//START
		//	if($kolC!="" && $kolF!="" && $kolG!="" && $kolJ!="" && $kolK!="" && $kolL!="" && $kolM!=""){
	if($kolC!=""){
	if($kolF!="" && $kolG!="" && $kolJ!="" && $kolK!="" && $kolL!="" && $kolM!="" &&  $kolBQ!=""){
				//echo $kolC."<br>";
			//	echo $i."<br>";
		//	echo $kolF.$kolG.$kolJ.$kolK.$kolL.$kolM."<br>";
				
													$tb_term=odbc_exec($koneksi_lp,"select distinct part_nm,part_dtl,uom,price1,price2,price3,price4,price5,price6,price7,price8,price9,price10,price11,price12,curr,lp,kat_tax,kat_tax,kd_proses,kd_lt,status_part,id_proses,lt_quo,lt_po,lt_pr,lt_datang,lt_vp from bps_part where part_no='$kolC'");
													$cekp=0;
													$kd_proses="";
													while($bar_term=odbc_fetch_array($tb_term)){
														$part_nm=odbc_result($tb_term,"part_nm"); //ok
													//	$part_grp=odbc_result($tb_term,"part_grp");
													//	$part_kat=odbc_result($tb_term,"part_kat");
														$part_dtl=odbc_result($tb_term,"part_dtl"); //ok
														$uom=odbc_result($tb_term,"uom"); //ok
														$price[0]=odbc_result($tb_term,"price1"); //ok
														$price[1]=odbc_result($tb_term,"price2");
														$price[2]=odbc_result($tb_term,"price3");
														$price[3]=odbc_result($tb_term,"price4");
														$price[4]=odbc_result($tb_term,"price5");
														$price[5]=odbc_result($tb_term,"price6");
														$price[6]=odbc_result($tb_term,"price7");
														$price[7]=odbc_result($tb_term,"price8");
														$price[8]=odbc_result($tb_term,"price9");
														$price[9]=odbc_result($tb_term,"price10");
														$price[10]=odbc_result($tb_term,"price11");
														$price[11]=odbc_result($tb_term,"price12");
														$curr=odbc_result($tb_term,"curr");
														$lp=odbc_result($tb_term,"lp");
														$kat_tax=odbc_result($tb_term,"kat_tax");
														$kd_proses=odbc_result($tb_term,"kd_proses");
														$kd_lt=odbc_result($tb_term,"kd_lt");
														$status_part=odbc_result($tb_term,"status_part");
														$id_proses=odbc_result($tb_term,"id_proses");
														
															$lt_quo=odbc_result($tb_term,"lt_quo");
															$lt_po=odbc_result($tb_term,"lt_po");
															$lt_pr=odbc_result($tb_term,"lt_pr");
															$lt_datang=odbc_result($tb_term,"lt_datang");
															$lt_vp=odbc_result($tb_term,"lt_vp");
															
														$tb_termA=odbc_exec($koneksi_lp,"select * from bps_kurs where kurs_code='$curr' and term='$term_s'");
														$kurs=0;
														while($bar_termA=odbc_fetch_array($tb_termA)){
															$kurs=odbc_result($tb_termA,"kurs");
														}
														
														
														$tb_termB=odbc_exec($koneksi_lp,"select distinct kode_lt,lt_vp,lt_po,lt_pr,lt_dtg,lt_Quo from bps_leadtime where kode_lt='$kd_lt' ");
														$termB=0;
														while($bar_termB=odbc_fetch_array($tb_termB)){
															$kd_lt=odbc_result($tb_termB,"kode_lt");
															$lt_quo=odbc_result($tb_termB,"lt_quo");
															$lt_po=odbc_result($tb_termB,"lt_po");
															$lt_pr=odbc_result($tb_termB,"lt_pr");
															$lt_datang=odbc_result($tb_termB,"lt_dtg");
															$lt_vp=odbc_result($tb_termB,"lt_vp");
															//$lp=odbc_result($tb_termB,"lp");
														}
														
														
														$cekp++;
													}
													
													if($lt_vp==""){
														$lt_vp="0";
													}
													if($lt_datang==""){
														$lt_datang="0";
													}
													if($lt_po==""){
														$lt_po="0";
													}
													if($lt_pr==""){
														$lt_pr="0";
													}
													if($lt_quo==""){
														$lt_quo="0";
													}
													
	/*							
	$sql_updt="insert into bps_budget_stp(no_ctrl,sect,part_no,part_desc,part_dtl,uom,periode,term,price,account,acc_desc,expaired,id_proses,lt_vp,lt_datang,lt_po,lt_pr,lt_Quo,jns_budget,lp,pic_updt,tgl_updt,qty,kurs,sub_acc,curr,phase,cccode,cv_code,part_nm,qty_fcs,price_fcs,revisi,fungsi,carline,carmaker) values('$ctrl_n','$sect_s','$term_s','$jns_bud_u','$pic',getdate(),'$experied','$kolB','$kolC','$kolD','$kolE','$kolF','$kolH','$kolI','$kolJ','$kolK','$kolL','$kolM','$kolN','$kolO','$kolQ','$kolR','$kolS','$kolT','$kolU','$kolV','$kolWs')";	
	*/

	//echo $price[0].$price[1];
//	echo htmlspecialchars_decode($qty[0].$qty[1].$qty[2]);
//echo $kd_proses;	

	if($cekp>0 && $kd_proses!="" ){
	
	for ($k=0; $k<12; $k++){

	$period2=date("Ym",strtotime($periode." +$k month"));
	$expired=date("Y-m-28",strtotime($periode." +$k month"));
//	echo $period2."xxxx".$price[0];

	$cek=0;
	$cekPN="select part_no,fungsi,phase,sub_acc,cccode,carline,carmaker,periode,term,revisi,qty from bps_budget_stp where part_no='$kolC' and sect='$sect_s' and term='$term_s' and revisi='$for_review' and phase='$kolJ' and sub_acc='$kolG' and fungsi='$kolF' and cccode='$kolK' and carline='$kolL' and carmaker='$kolM' and periode='$period2' and urut='$j' and qty='$qty[$k]' ";
	$tb_cekPN=odbc_exec($koneksi_lp,$cekPN);
	echo $cekPN;
		while(odbc_fetch_array($tb_cekPN)) {
			$cek++;
			$pncek=odbc_result($tb_cekPN,"part_no");
			$fungsicek=odbc_result($tb_cekPN,"fungsi");
			$phasecek=odbc_result($tb_cekPN,"phase");
			$sacek=odbc_result($tb_cekPN,"sub_acc");
			$cccek=odbc_result($tb_cekPN,"cccode");
			$carcek=odbc_result($tb_cekPN,"carline");
			$makercek=odbc_result($tb_cekPN,"carmaker");
			$qtycek=odbc_result($tb_cekPN,"qty");
			
		}
// echo $cekPN;		
if ($cek > 0 ) {
	$ny++;
	$errnya++;
	if (strlen($nik_nok) < 1) {
			$nik_nok = "Part No --> ".$kolC ." --> baris ". $i." --> Periode ". $period2." --> Qty ". $qtycek;
			} else {
			$nik_nok = $nik_nok ."\\n Part No --> ". $kolC ." --> baris ". $i." --> Periode ". $period2." --> Qty ". $qtycek;
		}
	}


if ($cek < 1 ) {
	
	$sql_updt="insert into bps_budget_stp(sect,part_no,part_dtl,uom,periode,term,price,expaired,id_proses,jns_budget,lp,pic_updt,tgl_updt,qty,kurs,curr,phase,cccode,part_nm,revisi,fungsi,carline,carmaker,sub_acc,lt_vp,lt_datang,lt_po,lt_pr,lt_Quo,urut,dept,sect_detail,category,sub_acc_desc) values('$sect_s','$kolC','$part_dtl','$uom','$period2','$term_s','".$price[$k]."','$expired','$kd_proses','NORMAL','$lp','$pic','$tgl','$qty[$k]','$kurs','$curr','$kolJ','$kolK','$part_nm','$for_review','$kolF','$kolL','$kolM','$kolG','$lt_vp','$lt_datang','$lt_po','$lt_pr','$lt_quo','$j','$dept','$sect_det','$kategori','$kolBQ')";	
 //echo $sql_updt;	
	$hasilstp = odbc_exec($koneksi_lp,$sql_updt);	
	if(!$hasilstp){
		echo "<br>Error ".$i.$sql_updt."<br>".$sql_updt;
		print(odbc_error());
		$jmler;
	}else{ $jmrow++; }
	
} else {
	
	// echo "<script>alert('Sheet : $j ; Baris : $i ; Part No : $kolC ; Periode : $period2 ;  Data Kembar! ');</script>";
	 // echo "<script>'Data Kembar! :\\n $nik_nok';</script>";
	// echo "<br>Sheet : ".$j."; Baris : ".$i."; Part No : ".$kolC."<br>";
	// echo "<script>alert(' Sheet : $j ; Baris : $i ; Part No : $kolC ;  $jmrow Data Berhasil Diupload, $jmlk Data Gagal ($jmlerr Tidak ada di Master Part. $jmlerid Tidak ada kode proses nya)');</script>";
	
} 
// echo "<br>Sheet : ".$j."; Baris : ".$i."; Part No : ".$kolC."<br>";
	// $sql_updt="insert into bps_budget_stp(sect,part_no,part_dtl,uom,periode,term,price,expaired,id_proses,jns_budget,lp,pic_updt,tgl_updt,qty,kurs,curr,phase,cccode,part_nm,revisi,fungsi,carline,carmaker,sub_acc,lt_vp,lt_datang,lt_po,lt_pr,lt_Quo,urut,dept,sect_detail) values('$sect_s','$kolC','$part_dtl','$uom','$period2','$term_s','".$price[$k]."','$expired','$kd_proses','NORMAL','$lp','$pic','$tgl','$qty[$k]','$kurs','$curr','$kolJ','$kolK','$part_nm','$for_review','$kolF','$kolL','$kolM','$kolG','$lt_vp','$lt_datang','$lt_po','$lt_pr','$lt_quo','$j','$dept','$sect_det')";	
 //echo $sql_updt;	
	// $hasilstp = odbc_exec($koneksi_lp,$sql_updt);	
	//echo "<br>lht ".$i.$sql_updt;
	// if(!$hasilstp){
		// echo "<br>Error ".$i.$sql_updt."<br>".$sql_updt;
		// print(odbc_error());
		// $jmler;
	// }else{ $jmrow++; }
	
	}
	
	
	
}else{
	
//	$sql_updt="insert into bps_budget_stp(sect,part_no,part_dtl,uom,periode,term,price,expaired,id_proses,jns_budget,lp,pic_updt,tgl_updt,qty,kurs,curr,phase,cccode,part_nm,revisi,fungsi,carline,carmaker,sub_acc) values('$sect_s','$kolC','$part_dtl','$uom','$period2','$term_s','".$price[$k]."','$expired','$kd_proses','NORMAL','$lp','$pic','$tgl','$qty[$k]','$kurs','$curr','$kolJ','$kolK','$part_nm','$for_review','$kolF','$kolL','$kolM','$kolG')";	
	
	$errnya++;
	echo "<script>alert('Sheet : $kategori ; Baris : $i ; Part No : $kolC ;  Part No Tidak Ada di Master Part!');</script>";
	echo "<br>lht ".$i.$sql_updt;
	echo $sql_updt;
	if($cekp<=0){
		$jmlerr++;
	}else{
		$jmlerid++;
		
	}
	
}


		} //end if kolC/F
		else {
			$errnya++;
			echo "<script>alert('Sheet : $kategori ; Baris : $i ; Part No : $kolC ;  Ada kolom yang kosong!!');</script>";
			 
		}
		
		}
	} // end $i
} //end $j

//upload sheet ke 5 ( summary )
$baris = $data->rowcount($sheet_index=5);
				for ($i=9; $i<=$fixedbaris; $i++){
				

$kolA=$data->val($i,1,5);//PHASE 
$kolB=$data->val($i,2,5);//ACC NO / SUB_ACC
$kolC=$data->val($i,3,5);//ACC DES
$kolD=$data->val($i,4,5);//PN
$kolE=$data->val($i,5,5);//STP QTY
//SAMPAI
$kolP=$data->val($i,16,5);//STP QTY
$kolQ=$data->val($i,17,5);//STP AMT
//SAMPAI
$kolAB=$data->val($i,28,5);//STP AMT
$kolAC=$data->val($i,29,5);//TOTAL QTY
$kolAD=$data->val($i,30,5);//TOTAL AMT TERM NOW
$kolAF=$data->val($i,32,5);//TOTAL QTY
$kolAG=$data->val($i,33,5);//TOTAL AMT TERM SEBELUM
$kolAH=$data->val($i,34,5);//PERCENTAGE QTY
$kolAI=$data->val($i,35,5);//PERCENTAGE AMT
$kolAJ=$data->val($i,36,5);//REMARK

			if( $kolAJ!="" && $kolA!="" ){

	$sql_updt_r="update bps_budget_stp set remark='$kolAJ' where part_nm='$kolD' and category='$kolA' and revisi='$for_review' and sect='$sect_s' and term='$term_s' ";	
	
	$hasilstp_r = odbc_exec($koneksi_lp,$sql_updt_r);	
	//echo "<br>lht ".$i.$sql_updt;
	if(!$hasilstp_r){
		echo "<br>Error ".$i.$sql_updt_r."<br>".$sql_updt_r;
		print(odbc_error());
		$jmler;
	}else{ $jmrow++; }
		
echo $sql_updt_r;
		} //end if kolAJ sheet 5 summary
		
	} // end $i 
// end sheet 5 summary	

//upload sheet ke 6 ( analisys )
$baris = $data->rowcount($sheet_index=6);
				for ($i=14; $i<=$fixedbaris; $i++){
				

$kolA=$data->val($i,1,6);//PHASE 
$kolB=$data->val($i,2,6);//PN
$kolC=$data->val($i,3,6);//CTG
$kolD=$data->val($i,4,6);//TERM SEBELUM
$kolE=$data->val($i,5,6);//TERM ACT
$kolF=$data->val($i,6,6);//NARIYUKI
$kolG=$data->val($i,7,6);//COSH PUSH
$kolH=$data->val($i,8,6);//COST REDUCTION
$kolI=$data->val($i,9,6);//OTHER
$kolJ=$data->val($i,10,6);//STP AMOUNT
$kolK=$data->val($i,11,6);//PERCENTAGE
$kolL=$data->val($i,12,6);//REMARK


			if( $kolA!="" && $kolB!="" ){

	$sql_updt_a="update bps_budget_stp set ctg='$kolC',cost_push='$kolG',cost_reduction='$kolH',cost_other='$kolI' where category='$kolA' and part_nm='$kolB' and revisi='$for_review' and sect='$sect_s' and term='$term_s' ";	
	
	$hasilstp_a = odbc_exec($koneksi_lp,$sql_updt_a);	
	//echo "<br>lht ".$i.$sql_updt;
	if(!$hasilstp_a){
		echo "<br>Error ".$i.$sql_updt_a."<br>".$sql_updt_a;
		print(odbc_error());
		$jmler;
	}else{ $jmrow++; }
		
echo $sql_updt_a;
		} //end if kolAJ sheet 6 analisys
		
	} // end $i 
// end sheet 6 analisys			
		
unlink($_FILES['file_s']['name']);
	$jmlk=0;
$jmlk=$jmler+$jmlerr+$jmlerid;

if($errnya>0){
	
	$sql_updt_a="delete from bps_budget_stp where revisi='$for_review' and sect='$sect_s' and term='$term_s' ";	
	$hasilstp_a = odbc_exec($koneksi_lp,$sql_updt_a);	
	
	echo "<script>alert('Anda Harus Upload Ulang, $jmlk Data Salah ($jmlerr Tidak ada di Master Part. $jmlerid Tidak ada kode proses nya), \\n Data Kembar $ny \\n $nik_nok ');</script>";
}else{
	echo "<script>alert('$jmrow Data Berhasil Diupload, $jmlk Data Gagal ($jmlerr Tidak ada di Master Part. $jmlerid Tidak ada kode proses nya), \\n Data Kembar $ny \\n $nik_nok ');</script>";
	
}

echo "<br>Data Kembar ".$ny."\\n".$nik_nok."<br>";
$sq_acc="select *,(qty * price) as amount from bps_budget_stp where tgl_updt=getdate() and pic_updt='$pic'";


	} // end array
else{ echo "<script>alert('Format file harus XLS'); window.location = '?page=form/inpt_budFA.php'</script>"; }

} //end isset


//End Upload STP

// Start Upload STP Remark

if(isset($_POST['upld_s_r']) ){
					require_once "excel_reader2.php";
	
		$file_name = $_FILES['file_s']['name']; //nama file (tanpa path)
		$tmp_name  = $_FILES['file_s']['tmp_name']; //nama local temp file di server
		$file_size = $_FILES['file_s']['size']; //ukuran file (dalam bytes)
		$file_type = $_FILES['file_s']['type']; //tipe filenya (langsung detect MIMEnya)
		$fp1 = fopen($tmp_name, 'r'); // open file (read-only, binary)
		$fp = fopen($tmp_name, 'r');		
		$pecah=explode(".",$file_name);
		$ekstensi=$pecah[1];
		$extensionList=array("xls","XLS");
		if(in_array($ekstensi,$extensionList)){		
			$target = basename($_FILES['file_s']['name']) ;
			move_uploaded_file($_FILES['file_s']['tmp_name'], $target);			 
			$data = new Spreadsheet_Excel_Reader($_FILES['file_s']['name'],false);  
			$baris = $data->rowcount($sheet_index=0);
			$fixedbaris = $baris;	 
			$term_s=$_POST['term_s'];
			$term=$_POST['term_s'];
			$year=$term-82+2022;
			$year2=$year+1;
			$term_sum=$term_s-1;
			
			$period=date("Ym",strtotime($year."-07-01"));
			$periode=date("Y-m-01",strtotime($year."-07-01"));
				
			$sect_s=$_POST['sect_s'];
			$sect_ss=explode("-",$_POST['sect_s']);
			$dept=$sect_ss[0];
			$sect_det=$sect_ss[1];
			
			$for_review=$_POST['for_review'];
			$pic=$_SESSION['nama'];
			$tgl=date("Y-m-d H:i:s");
			$jmrow=0;
			
			$jmlerr=0;
			$jmlerid=0;
			$jmler=0;
				$sql_del_taki=" where sect='$sect_s' and revisi='$for_review'";
			//echo $sql_del;
				$hilang_taki=odbc_exec($koneksi_lp,"Delete from bps_budget_stp_taki".$sql_del_taki);
			
	/*		for($j=1;$j<=3;$j++){
				$baris = $data->rowcount($sheet_index=1);
				$fixedbaris = $baris;
				
				$kategori="PLAN BUDGET SAMI-JF";
				
				if($j==1){
					$kategori="SUMMARY";
				}else if($j==2){
					$kategori="ANALYSIS";
				}/*else if($j==3){
					$kategori="IMPROVEMENT";
				} *
				echo $j."fixedbaris".$fixedbaris."<br>";	*/		
				for ($i=9; $i<=$fixedbaris; $i++){		

$kolA=$data->val($i,1,1);//PHASE 
$kolB=$data->val($i,2,1);//ACC NO / SUB_ACC
$kolC=$data->val($i,3,1);//ACC DES
$kolD=$data->val($i,4,1);//PN
$kolE=$data->val($i,5,1);//STP QTY
//SAMPAI
$kolP=$data->val($i,16,1);//STP QTY
$kolQ=$data->val($i,17,1);//STP AMT
//SAMPAI
$kolAB=$data->val($i,28,1);//STP AMT
$kolAC=$data->val($i,29,1);//TOTAL QTY
$kolAD=$data->val($i,30,1);//TOTAL AMT TERM NOW
$kolAF=$data->val($i,32,1);//TOTAL QTY
$kolAG=$data->val($i,33,1);//TOTAL AMT TERM SEBELUM
$kolAH=$data->val($i,34,1);//PERCENTAGE QTY
$kolAI=$data->val($i,35,1);//PERCENTAGE AMT
$kolAJ=$data->val($i,36,1);//REMARK

			if( $kolAJ!="" ){

	$sql_updt_r="update bps_budget_stp set remark='$kolAJ' where part_nm='$kolD' and category='$kolA' and revisi='$for_review' and sect='$sect_s' and term='$term_s' ";	
	
	$hasilstp_r = odbc_exec($koneksi_lp,$sql_updt_r);	
	//echo "<br>lht ".$i.$sql_updt;
	if(!$hasilstp_r){
		echo "<br>Error ".$i.$sql_updt_r."<br>".$sql_updt_r;
		print(odbc_error());
		$jmler;
	}else{ $jmrow++; }
		
echo $sql_updt_r;
		} //end if kolAJ
		
	} // end $i
//} //end $j


//upload sheet ke 6 ( analisys )
				for ($i=14; $i<=$fixedbaris; $i++){
				
$kolA=$data->val($i,1,2);//PHASE 
$kolB=$data->val($i,2,2);//PN
$kolC=$data->val($i,3,2);//CTG
$kolD=$data->val($i,4,2);//TERM SEBELUM
$kolE=$data->val($i,5,2);//TERM ACT
//$kolF=$data->val($i,6,2);//NARIYUKI
//$kolG=$data->val($i,7,2);//COSH PUSH
//$kolH=$data->val($i,8,2);//COST REDUCTION
//$kolI=$data->val($i,9,2);//OTHER
$kolJ=$data->val($i,10,2);//STP AMOUNT
$kolK=$data->val($i,11,2);//PERCENTAGE
$kolL=$data->val($i,12,2);//REMARK

$kolF=str_replace(",","",$data->val($i,6,2));//COSH PUSH
$kolF=str_replace("*","",$kolF);//QTY 7
$kolF=str_replace(" ","",$kolF);//QTY 7
$kolF=str_replace("?","",$kolF);//QTY 7
$kolF=str_replace("-","",$kolF);//QTY 6
$kolF=str_replace("(","-",$kolF);//QTY 6
$kolF=str_replace(")","",$kolF);//QTY 6
$kolG=str_replace(",","",$data->val($i,7,2));//COSH PUSH
$kolG=str_replace("*","",$kolG);//QTY 7
$kolG=str_replace(" ","",$kolG);//QTY 7
$kolG=str_replace("?","",$kolG);//QTY 7
$kolG=str_replace("-","",$kolG);//QTY 6
$kolG=str_replace("(","-",$kolG);//QTY 6
$kolG=str_replace(")","",$kolG);//QTY 6
$kolH=str_replace(",","",$data->val($i,8,2));//COSH REDUCTION
$kolH=str_replace("*","",$kolH);//QTY 7
$kolH=str_replace(" ","",$kolH);//QTY 7
$kolH=str_replace("?","",$kolH);//QTY 7
$kolH=str_replace("-","",$kolH);//QTY 6
$kolH=str_replace("(","-",$kolH);//QTY 6
$kolH=str_replace(")","",$kolH);//QTY 6
$kolI=str_replace(",","",$data->val($i,9,2));//COSH OTHER
$kolI=str_replace("*","",$kolI);//QTY 7
$kolI=str_replace(" ","",$kolI);//QTY 7
$kolI=str_replace("?","",$kolI);//QTY 7
$kolI=str_replace("-","",$kolI);//QTY 6
$kolI=str_replace("(","-",$kolI);//QTY 6
$kolI=str_replace(")","",$kolI);//QTY 6

if($kolF=="" || $kolF=="-"){
	$kolF="0";
}
if($kolG=="" || $kolG=="-"){
	$kolG="0";
}
if($kolH=="" || $kolH=="-"){
	$kolH="0";
}
if($kolI=="" || $kolI=="-"){
	$kolI="0";
}

			if( $kolA!="" && $kolB!="" ){

	$sql_updt_a="update bps_budget_stp set ctg='$kolC',cost_push='$kolG',cost_reduction='$kolH',cost_other='$kolI',nariyuki='$kolF' where category='$kolA' and part_nm='$kolB' and revisi='$for_review' and sect='$sect_s' and term='$term_s' ";	
	
	$hasilstp_a = odbc_exec($koneksi_lp,$sql_updt_a);	
	//echo "<br>lht ".$i.$sql_updt;
	if(!$hasilstp_a){
		echo "<br>Error ".$i.$sql_updt_a."<br>".$sql_updt_a;
		print(odbc_error());
		$jmler;
	}else{ $jmrow++; }
		
echo $sql_updt_a;
		} //end if kolAJ sheet 6 analisys
		
	} // end $i 

	
	//start upload sheet TAKI graph
	for ($i=9; $i<=$fixedbaris; $i++){
				
$kolA=$data->val($i,1,3);//no
$kolB=$data->val($i,2,3);//item
//$kolC=$data->val($i,3,3);//amount
$kolD=$data->val($i,4,3);//remark
$kolE=$data->val($i,5,3);//keterangan
$kolF=$data->val($i,6,3);//term
$kolG=$data->val($i,7,3);//measure
//$kolH=$data->val($i,8,2);//COST REDUCTION
//$kolI=$data->val($i,9,2);//OTHER
//$kolJ=$data->val($i,10,2);//STP AMOUNT
//$kolK=$data->val($i,11,2);//PERCENTAGE
//$kolL=$data->val($i,12,2);//REMARK

$kolC=str_replace(",","",$data->val($i,3,3));//amount
$kolC=str_replace("*","",$kolC);//QTY 7
$kolC=str_replace(" ","",$kolC);//QTY 7
$kolC=str_replace("?","",$kolC);//QTY 7
$kolC=str_replace("-","",$kolC);//QTY 6
$kolC=str_replace("(","-",$kolC);//QTY 6
$kolC=str_replace(")","",$kolC);//QTY 6


if($kolC=="" || $kolC=="-"){
	$kolC="0";
}

			if( $kolB!="" ){

	$sql_updt_t="insert into bps_budget_stp_taki(sect,item,amount,remark,keterangan,term,measure,dept,sect_detail,revisi,pic_updt,tgl_updt,term_budget) values('$sect_s','$kolB','$kolC','$kolD','$kolE','$kolF','$kolG','$dept','$sect_det','$for_review','$pic','$tgl','$term_s')";		
	
	$hasilstp_t = odbc_exec($koneksi_lp,$sql_updt_t);	
	//echo "<br>lht ".$i.$sql_updt;
	if(!$hasilstp_t){
		echo "<br>Error ".$i.$sql_updt_t."<br>".$sql_updt_t;
		print(odbc_error());
		$jmler;
	}else{ $jmrow++; }
		
echo $sql_updt_t;
		} //end if kolAJ sheet taki
		
	} // end $i 	
	
	
// end sheet 6 analisys			
		
unlink($_FILES['file_s']['name']);
	$jmlk=0;
$jmlk=$jmler+$jmlerr+$jmlerid;
echo "<script>alert('$jmrow Data Berhasil Diupload, $jmlk Data Gagal ($jmlerr Tidak ada di Plan Budget STP)');</script>";

}else{ echo "<script>alert('Format file harus XLS'); window.location = '?page=form/inpt_budFA.php'</script>"; }

}

//End Upload STP Remark

//input ManHour Budget
if(isset($_POST['sv_mh_s']) ) {
	$term_mh_s=$_POST['term_mh_s'];
	$mh_s=$_POST['mh_s'];
	$mh_act=$_POST['mh_act'];
	$mh_p=$_POST['mh_p'];
	$pic_update=$_SESSION['nama'];
	
	$rev_mh=0;
	$cekPN="select max(revisi) as rev from bps_stp_mh where term='$term_mh'' ";
	$tb_cekPN=odbc_exec($koneksi_lp,$cekPN);
	
		while(odbc_fetch_array($tb_cekPN)) {
			
			$rev_mh=odbc_result($tb_cekPN,"rev");
			
			
		}
		
		$rev_mh=$rev_mh+1;
		
	//$sql_del_mh="delete from bps_budget_stp_mh where term='$term_mh_s' ";	
	//$del_mh = odbc_exec($koneksi_lp,$sql_del_mh);
	
	$sq_inpt_mh="insert into bps_budget_stp_mh(term,mh_s,mh_act,mh_p,pic_updt,tgl_updt,revisi) values('$term_mh_s','$mh_s','$mh_act','$mh_p','$pic_update',getdate(),'$rev_mh' )";	
	echo $sq_inpt_mh;
	$tb_add_mh=odbc_exec($koneksi_lp,$sq_inpt_mh);
	
	//echo "<script>alert('Sukses Menambah Data!'); window.location = '?page=form/inpt_budFA.php'</script>";
	echo "<script>alert('Sukses Menambah Data!');";
}
//end input ManHour Budget


if(isset($_POST['upld2']) ){
	require_once "excel_reader2.php";
	//	echo "<script>alert('upload data');</script>"; 
		$file_name = $_FILES['file']['name']; //nama file (tanpa path)
		$tmp_name  = $_FILES['file']['tmp_name']; //nama local temp file di server
		$file_size = $_FILES['file']['size']; //ukuran file (dalam bytes)
		$file_type = $_FILES['file']['type']; //tipe filenya (langsung detect MIMEnya)
		$fp1 = fopen($tmp_name, 'r'); // open file (read-only, binary)
		$fp = fopen($tmp_name, 'r');		
		$pecah=explode(".",$file_name);
		$ekstensi=$pecah[1];
		$extensionList=array("xls","XLS");
		if(in_array($ekstensi,$extensionList)){		
			$target = basename($_FILES['file']['name']) ;
			move_uploaded_file($_FILES['file']['tmp_name'], $target);			 
			$data = new Spreadsheet_Excel_Reader($_FILES['file']['name'],false);  
			$baris = $data->rowcount($sheet_index=0);
			$fixedbaris = $baris;	 
			$term_u=$_POST['term_u'];
			$sect_u=$data->val(1,8)."-".$data->val(2,8); //$sect_u=$_POST['sect_u'];
			$jns_bud_u=$_POST['jns_bud_u'];
			$pic=$_SESSION['nama'];
			$jmrow=0;
			if($sect_u = $_POST['sect_u']){
				$sql_del=" where term='$term_u' and sect='$sect_u'";
			//echo $sql_del;
				$hilang=odbc_exec($koneksi_lp,"Delete from bps_budget".$sql_del);
				$hilangFA=odbc_exec($koneksi_lp,"Delete from bps_budget_FA".$sql_del);
				for ($i=12; $i<=$fixedbaris; $i++){		
$kolC=$data->val($i,3);//PART NO
$kolD=$data->val($i,4);//PART NAME
$kolE=$data->val($i,5);//DETAIL PART
$kolF=$data->val($i,6);//FUNGSI
$kolG=$data->val($i,8);//ACCOUNT
$kolI=$data->val($i,10);//PHASE
$kolJ=$data->val($i,11);//Cv code/cccode
$kolM=$data->val($i,14);//UOM
$kolN=$data->val($i,15);//Currency

$kolBQ=$data->val($i,70);//SUB ACCOUNT
$kolBR=$data->val($i,71);//SUB ACCOUNT
$kolBS=$data->val($i,72);//KODE PROSES
$kolBT=$data->val($i,73);//PURCHASING
$kolBU=$data->val($i,74);//PENAWARAN
$kolBV=$data->val($i,75);//PR
$kolBW=$data->val($i,76);//PO
$kolBX=$data->val($i,77);//KEDATANGAN

$KP=15;$KQ=39;$KC=78;

for ($n=1; $n<=12; $n++){
	$KQ++;$KC++;$KP++;		
/*if($n<7){	
$prcx=str_replace(",","",$data->val($i,15));//PRICE JUL-DEC
}else{
$prcx=str_replace(",","",$data->val($i,16));//PRICE JAN-JUN
}
*/
//$prcx=number_format($prcy,10,".","");
$prcx=str_replace(",","",$data->val($i,$KP));//PRICE ORI
$qty=str_replace(",","",$data->val($i,$KQ));//QTY
$peri=$data->val(11,$KQ);//PERIODE
$experied=date("Y-m-d",strtotime($peri."28"));
$ctrln=$data->val($i,$KC);//CONTROL NO	

if($kolC!="" and ($qty!="" and $qty>0))
{
	$prc=number_format(str_replace("*","",$prcx),10,".","");
	$sql_updt="insert into bps_budget(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode,lt_Quo,lt_pr,lt_po,lt_datang,lt_vp,fungsi)
	values('$sect_u','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$ctrln','$kolBS','$kolBR','$kolG','$kolBQ','$kolC','$kolD','$kolE','$peri','$qty','$kolM','$kolN','$prc','$kolI','$kolJ','$kolBT','$kolBU','$kolBV','$kolBW','$kolBX','$kolF')";

	$sql_updtFA="insert into bps_budget_FA(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode,fungsi)
	values('$sect_u','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$ctrln','$kolBS','$kolBR','$kolG','$kolBQ','$kolC','$kolD','$kolE','$peri','$qty','$kolM','$kolN','$prc','$kolI','$kolJ','$kolF')";
	$hasil = odbc_exec($koneksi_lp,$sql_updt);	
	$hasilFA = odbc_exec($koneksi_lp,$sql_updtFA);
// echo "<br>lht ".$i.$sql_updt;
// echo "<br>lht ".$i.$sql_updtFA;
	if(!$hasilFA){
		echo "<br>Error ".$i.$sql_updt."<br>".$sql_updtFA;
		print(odbc_error());
	}else{ $jmrow++; }
}

}


}		
unlink($_FILES['file']['name']);	
echo "<script>alert('$jmrow DATA SELESAI DI UPLOAD ');</script>";
$sq_acc="select *,(qty * price) as amount from bps_budget_FA where tgl_updt=getdate() and pic_updt='$pic'";

}
else{		
	unlink($_FILES['file']['name']);	
	echo "<script>alert('GAGAL Upload, Dept - Sect Tidak sesuai');</script>";}

}else{ echo "<script>alert('Format file harus XLS'); window.location = '?page=form/inpt_budFA.php'</script>"; }

}


if(isset($_POST['upldnew']) ){
	require_once "excel_reader2.php";
	//	echo "<script>alert('upload data');</script>"; 
		$file_name = $_FILES['file']['name']; //nama file (tanpa path)
		$tmp_name  = $_FILES['file']['tmp_name']; //nama local temp file di server
		$file_size = $_FILES['file']['size']; //ukuran file (dalam bytes)
		$file_type = $_FILES['file']['type']; //tipe filenya (langsung detect MIMEnya)
		$fp1 = fopen($tmp_name, 'r'); // open file (read-only, binary)
		$fp = fopen($tmp_name, 'r');		
		$pecah=explode(".",$file_name);
		$ekstensi=$pecah[1];
		$extensionList=array("xls","XLS");
		if(in_array($ekstensi,$extensionList)){		
			$target = basename($_FILES['file']['name']) ;
			move_uploaded_file($_FILES['file']['tmp_name'], $target);			 
			$data = new Spreadsheet_Excel_Reader($_FILES['file']['name'],false);  
			$baris = $data->rowcount($sheet_index=0);
			$fixedbaris = $baris;	 
			$term_u=$_POST['term_u'];
			//$sect_u=$data->val(1,8)."-".$data->val(2,8); //$sect_u=$_POST['sect_u'];
			$jns_bud_u=$_POST['jns_bud_u'];
			$pic=$_SESSION['nama'];
			$jmrow=0;
			if($sect_u = "SAMI-ALL"){
				$sql_del=" where term='$term_u'";
			//echo $sql_del;
				$hilang=odbc_exec($koneksi_lp,"Delete from bps_budget".$sql_del);
				$hilangFA=odbc_exec($koneksi_lp,"Delete from bps_budget_FA".$sql_del);
				for ($i=12; $i<=$fixedbaris; $i++){
$kolA=$data->val($i,1);//sect	
$kolC=$data->val($i,3);//PART NO
$kolD=$data->val($i,4);//PART NAME
$kolE=$data->val($i,5);//DETAIL PART
$kolF=$data->val($i,6);//FUNGSI
$kolG=$data->val($i,8);//ACCOUNT
$kolI=$data->val($i,10);//PHASE
$kolJ=$data->val($i,11);//Cv code/cccode
$kolM=$data->val($i,14);//UOM
$kolN=$data->val($i,15);//Currency

$kolBQ=$data->val($i,70);//SUB ACCOUNT
$kolBR=$data->val($i,71);//KODE PROSES
$kolBS=$data->val($i,72);//PURCHASING
$kolBT=$data->val($i,73);//PENAWARAN
$kolBU=$data->val($i,74);//PR
$kolBV=$data->val($i,75);//PO
$kolBW=$data->val($i,76);//KEDATANGAN
$kolBX=$data->val($i,77);//VP

$KP=15;$KQ=39;$KC=78;

for ($n=1; $n<=12; $n++){
	$KQ++;$KC++;$KP++;		
/*if($n<7){	
$prcx=str_replace(",","",$data->val($i,15));//PRICE JUL-DEC
}else{
$prcx=str_replace(",","",$data->val($i,16));//PRICE JAN-JUN
}
*/
//$prcx=number_format($prcy,10,".","");
$prcx=str_replace(",","",$data->val($i,$KP));//PRICE ORI
$qty=str_replace(",","",$data->val($i,$KQ));//QTY
$peri=$data->val(11,$KQ);//PERIODE
$experied=date("Y-m-d",strtotime($peri."28"));
$ctrln=$data->val($i,$KC);//CONTROL NO	

if($kolC!="" and ($qty!="" and $qty>0))
{
	$prc=number_format(str_replace("*","",$prcx),10,".","");
	$sql_updt="insert into bps_budget(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode,lt_Quo,lt_pr,lt_po,lt_datang,lt_vp,fungsi)
	values('$kolA','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$ctrln','$kolBS','$kolBR','$kolG','$kolBQ','$kolC','$kolD','$kolE','$peri','$qty','$kolM','$kolN','$prc','$kolI','$kolJ','$kolBT','$kolBU','$kolBV','$kolBW','$kolBX','$kolF')";

	$sql_updtFA="insert into bps_budget_FA(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode,fungsi)
	values('$kolA','$term_u','$jns_bud_u','$pic',getdate(),'$experied','$ctrln','$kolBS','$kolBR','$kolG','$kolBQ','$kolC','$kolD','$kolE','$peri','$qty','$kolM','$kolN','$prc','$kolI','$kolJ','$kolF')";
	$hasil = odbc_exec($koneksi_lp,$sql_updt);	
	$hasilFA = odbc_exec($koneksi_lp,$sql_updtFA);
//echo "<br>lht ".$i.$sql_updt;
	if(!$hasilFA){
		echo "<br>Error ".$i.$sql_updt."<br>".$sql_updtFA;
		print(odbc_error());
	}else{ $jmrow++; }
}

}


}		
unlink($_FILES['file']['name']);	
echo "<script>alert('$jmrow DATA SELESAI DI UPLOAD ');</script>";
$sq_acc="select *,(qty * price) as amount from bps_budget_FA where tgl_updt=getdate() and pic_updt='$pic'";

}
else{		
	unlink($_FILES['file']['name']);	
	echo "<script>alert('GAGAL Upload, Dept - Sect Tidak sesuai');</script>";}

}else{ echo "<script>alert('Format file harus XLS'); window.location = '?page=form/inpt_budFA.php'</script>"; }

}
?>



<?php 
if(isset($_POST['upld3']) ){
	require_once "excel_reader2.php";				
									$file_name = $_FILES['file']['name'];// nama file (tanpa path)								
									$tmp_name  = $_FILES['file']['tmp_name']; //nama local temp file di server								
									$file_size = $_FILES['file']['size']; //ukuran file (dalam bytes)								
									$file_type = $_FILES['file']['type']; //tipe filenya (langsung detect MIMEnya)		
									$fp1 = fopen($tmp_name, 'r'); // open file (read-only, binary)
									$fp = fopen($tmp_name, 'r');		
									$pecah=explode(".",$file_name);
									$ekstensi=$pecah[1];
									$extensionList=array("xls","XLS");
									if(in_array($ekstensi,$extensionList)){		
										$target = basename($_FILES['file']['name']) ;
										move_uploaded_file($_FILES['file']['tmp_name'], $target);			 
										$data = new Spreadsheet_Excel_Reader($_FILES['file']['name'],false);  
										$baris = $data->rowcount($sheet_index=0);
										$fixedbaris = $baris;	 
										$term_u=$_POST['term_u'];
		// $sect_u=$data->val(1,8)."-".$data->val(2,8);
										$sect_u= $_SESSION["area"];
		// $jns_bud_u=$_POST['jns_bud_u'];
										$pic=$_SESSION['nama'];
										$jmrow=0;
		// if($sect_u = $_POST['sect_u'])
		// {
										$sql_del=" where term='$term_u' and sect='$sect_u'";
			//echo $sql_del;
			// $hilang=odbc_exec($koneksi_lp,"Delete from bps_budget".$sql_del);	
			// $hilangFA=odbc_exec($koneksi_lp,"Delete from bps_budget_FA".$sql_del);
										for ($i=12; $i<=$fixedbaris; $i++){
			$kolA=$data->val($i,1); //PART_NO
			$kolB=$data->val($i,2); //PART  NAME
			$kolC=$data->val($i,3); //DETAIL PART
			$kolD=$data->val($i,4); //SECTION CATEGORY
			$kolE=$data->val($i,5); //ACC NO.
			$kolF=$data->val($i,6); //ACCOUNT DESCRIPTION
			$kolG=$data->val($i,7); //CATEGORY
			$kolH=$data->val($i,8); //COST CENTER
			$kolI=$data->val($i,9); //CARLINE
			$kolJ=$data->val($i,10); //CARMAKER
			$kolZ=$data->val($i,26); //SUB ACCOUNT
			$kolAA=$data->val($i,27); //KODE PROSES
			$kolAB=$data->val($i,28); //PURCHASING
			$kolAC=$data->val($i,29); //KODE LEAD TIME
			$no_doc=$term_u."-".$sect_u;

			$KP=14;$K=10;$KC=77;
			for ($n=1; $n<=12; $n++)
			{
				$KP++;$K++;$KC++;
				// $prcx=str_replace(",","",$data->val($i,$KP));//PRICE ORI
				$qty=str_replace(",","",$data->val($i,$K));//QTY
				$peri=$data->val(11,$K);//PERIODE
				$experied=date("Y-m-d",strtotime($peri."28"));
				// $ctrln=$data->val($i,$KC);//CONTROL NO
				$cr_dt=odbc_exec($koneksi_lp, "SELECT count(*) as jm FROM bps_budget_temp $sql_del ");
				$jm=odbc_result($cr_dt, "jm")+1;
				$ctrln=$term_u."-".$jm;//CONTROL NO
				if($kolA!="" and ($qty!="" and $qty>0))
				{
					$periode_price='price'.$n;
					$cr_prc=odbc_exec($koneksi_lp,"select isnull(price,0) as price,uom,curr,lp from mstr_part_ver where part_no='$kolA' and prc='$periode_price' ") ;
					$prcx=odbc_result($cr_prc,'price') ;
					$uom=odbc_result($cr_prc,'uom') ;
					$curr=odbc_result($cr_prc,'curr') ;
					$lp=odbc_result($cr_prc,'lp') ;

					$prc=number_format(str_replace("*","",$prcx),10,".","");
					$sql_updtFA="insert into bps_budget_temp(sect,term,jns_budget,pic_updt,tgl_updt,expaired,no_ctrl,lp,id_proses,account,sub_acc,part_no,part_nm,part_dtl,periode,qty,uom,curr,price,phase,cccode)
					values('$sect_u','$term_u','NORMAL','$pic',getdate(),'$experied','$ctrln','$kolAB','$kolAA','$kolE','$kolZ','$kolA','$kolB','$kolC','$peri','$qty','$uom','$curr',$prc,'$kolI','$kolJ')";

					// $hasil = odbc_exec($koneksi_lp,$sql_updt);	
					$hasilFA = odbc_exec($koneksi_lp,$sql_updtFA);
//$amoun_USD

//echo "<script>window.close();</script>";
					//echo "<script>alert('DATA BERHASIL DISIMPAN DENGAN NO PR $pr_no');</script>";

//echo "<br>lht ".$i.$sql_updt;
					if(!$hasilFA){
						echo "<br>Error ".$i.$sql_updt."<br>".$sql_updtFA;
						print(odbc_error());
					}else{ 
						$jmrow++; 
					}
				}
			}
		}
		unlink($_FILES['file']['name']);	
		echo "<script>alert('$jmrow DATA SELESAI DI UPLOAD ');</script>";
		$sq_acc="select *,(qty * price) as amount from bps_budget_temp where tgl_updt=getdate() and pic_updt='$pic'";
	}
	else{ 
		echo "<script>alert('Format file harus XLS'); window.location = '?page=form/inpt_budFA.php'</script>";
	}
}
?> 
<div class="row clearfix">
	<div class="card">
		<div class="row clearfix">				
			<div class="header">
				<h2>Record<small>Plan Budget</small></h2>
			</div>
			<div class="body">
				<form action="" id="frmcari" method="post"  enctype="multipart/form-data">
					
					
					
					<div class="col-md-2">	
						<div class="form-group">
							<div class="input-group">
								<select class="selectpicker" style="width: 50%;"  name="term_cr" id="term_cr"  required>
									<option selected="selected" value="">---Pilih Term---</option>
									<?php echo $opt_term; ?>

								</select>
							</div>
						</div>
					</div>

					<div class="col-md-2">	
						<div class="form-group">
							<div class="input-group">
								<select class="selectpicker" style="width: 100%;"  name="sect_cr" id="sect_cr"  required>
									<option selected="selected" value="">---Pilih Section---</option>
									<option value="all">Semua Section</option>
									<?php 
									echo $opt_sect;
									?>

								</select>
							</div>
						</div>
					</div>
					<div class="col-sm-2">	
						<div class="form-group">
							<div class="input-group">
								<select class="selectpicker" style="width: 100%;"  name="cmd_cari" id="cmd_cari" >
									<option selected="selected" value="">---Pilih Kolom---</option>
									<option value="no_ctrl">CONTROL NO</option>
									<option value="lp">PURCHASING</option>
									<option value="id_proses">PROCESS CODE</option>
									<option value="account">ACCOUNT</option>
									<option value="sub_acc">SUB ACCOUNT</option>
									<option value="part_no">PART NO</option>
									<option value="part_nm">PART NAME</option>
									<option value="part_dtl">DETAIL PART</option>
									<option value="periode">PERIODE</option>
									<option value="phase">PHASE</option>
									<option value="cccode">CC CODE</option>
									<option value="qty">QTY</option>
									<option value="uom">UOM</option>
									<option value="price">PRICE</option>
									<option value="curr">CURRENCY</option>
									<option value="expaired">EXPAIRED DATE</option>
								</select>
							</div>
						</div>
					</div>

					<div class="col-sm-4">	
						<div class="form-group">
							<div class="form-line">
								<input type="text"  class="form-control" data-role="tagsinput" id="txt_cari" name="txt_cari" placeholder="Detail Pencarian">
							</div> 
						</div>
					</div>

					<div class="col-sm-2">
						<button type="submit" name="cr_b" id="cr_b" class="btn bg-purple btn-circle-lg waves-effect waves-circle waves-float"><i class="material-icons">search</i> </button>
						<button type="reset" name="reset" id="reset" class="btn bg-red btn-circle-lg waves-effect waves-circle waves-float"><i class="material-icons">clear</i> </button>
					</div>
				</form>
			</div>	
		</div>	   
		<div class="row clearfix">
			<div class="body">
				<div class="table-responsive">
					<table id="example" class="table table-bordered table-striped table-hover dataTable js-exportable tabel2">
						<thead>
							<tr>	
								<th>SECTION</th>
								<th>TERM</th>
								<th>TYPE</th>
								<th>CONTROL NO</th>
								<th>PURCHASING</th>
								<th>PROCESS CODE</th>
								<th>ACCOUNT</th>
								<th>SUB ACC</th>
								<th>PART NO</th>
								<th>PART NAME</th>
								<th>PART DETAIL</th>
								<th>PART DESCRIPTION</th>
								<th>PERIODE</th>
								<th>QTY</th>
								<th>UOM</th>
								<th>CURR</th>
								<th>PRICE</th>
								<th>AMOUNT</th>
								<th>PHASE</th>
								<th>CC CODE</th>
								<th>EXPAIRED DATE</th>
								<!--th>VP</th>
								<th>KEDATANGAN</th>
								<th>PO</th>
								<th>PR</th>
								<th>PENAWARAN</th-->

							</tr>
						</thead>

						<tbody>
							<?php
							if(isset($_POST['cr_b']) ){	
								$term_cr=$_POST['term_cr'];
								$sect_cr=$_POST['sect_cr'];
								if($sect_cr=="all"){$whr0="";}else{$whr0=" and sect='$sect_cr'";}
								//$cmd_tbl=$_POST['cmd_tbl'];
								//if($cmd_tbl=="real"){$tbl="";}else{$tbl="_temp";}
								$cmd_cari=$_POST['cmd_cari'];
								$txt_cari=str_replace(" ","",$_POST['txt_cari']);
								if($txt_cari==""){$whr=""; }else{
									$whr=" and replace($cmd_cari,' ','') like '%$txt_cari%'";}
									$sq_acc="select *,(qty * price) as amount from bps_budget_FA where term='$term_cr' $whr0 $whr";
								}
								if(isset($_POST['smpn']) or isset($_POST['upld']) or isset($_POST['cr_b'])){
									$tb_acc=odbc_exec($koneksi_lp,$sq_acc);
									$row=0;
									while($baris1=odbc_fetch_array($tb_acc)){ $row++;
										$expaired=odbc_result($tb_acc,"expaired");
										$curr=odbc_result($tb_acc,"curr");
										$price=odbc_result($tb_acc,"price");
										if($curr=="USD"){$harga=number_format($price,18);}else{$harga=$price;}
										
										$qty=odbc_result($tb_acc,"qty");
										/*$lt_vp=Odbc_result($tb_acc,"lt_vp");
										$lt_datang=odbc_result($tb_acc,"lt_datang");
										$lt_po=odbc_result($tb_acc,"lt_po");
										$lt_pr=odbc_result($tb_acc,"lt_pr");
										$lt_Quo=odbc_result($tb_acc,"lt_Quo");
										$lt_datang_cumm=$lt_datang+$lt_vp;
										$lt_po_cumm=$lt_po+$lt_datang_cumm;
										$lt_pr_cumm=$lt_pr+$lt_po_cumm;
										$lt_Quo_cumm=$lt_Quo+$lt_pr_cumm;
										*/				?>	
										<tr  onclick="javascript:pilih(this);">
											<td><?php echo odbc_result($tb_acc,"sect"); ?></td>
											<td><?php echo odbc_result($tb_acc,"term"); ?></td>
											<td><?php echo odbc_result($tb_acc,"jns_budget"); ?></td>
											<td><?php echo odbc_result($tb_acc,"no_ctrl"); ?></td>
											<td><?php echo odbc_result($tb_acc,"lp"); ?></td>
											<td><?php echo odbc_result($tb_acc,"id_proses"); ?></td>
											<td><?php echo odbc_result($tb_acc,"account"); ?></td>
											<td><?php echo odbc_result($tb_acc,"sub_acc"); ?></td>
											<td><?php echo odbc_result($tb_acc,"part_no"); ?></td>
											<td><?php echo odbc_result($tb_acc,"part_nm"); ?></td>
											<td><?php echo odbc_result($tb_acc,"part_dtl"); ?></td>
											<td><?php echo odbc_result($tb_acc,"part_desc"); ?></td>
											<td><?php echo odbc_result($tb_acc,"periode"); ?></td>
											<td><?php echo $qty;?></td>
											<td><?php echo odbc_result($tb_acc,"uom"); ?></td>
											<td><?php echo odbc_result($tb_acc,"curr"); ?></td>
											<td><?php echo $harga; ?></td>
											<td><?php echo $qty*$price; ?></td>
											<td><?php echo odbc_result($tb_acc,"phase"); ?></td>
											<td><?php echo odbc_result($tb_acc,"cccode"); ?></td>
											<td><?php echo date("Y-m-d",strtotime($expaired)); ?></td>
										<!--td><?php// echo "($lt_vp) ".date('Y-m-d', strtotime($expaired. " - $lt_vp days")); ?></td>
										<td><?php// echo "($lt_datang) ".date('Y-m-d', strtotime($expaired. " - $lt_datang_cumm days")); ?></td>
										<td><?php// echo "($lt_po) ".date('Y-m-d', strtotime($expaired. " - $lt_po_cumm days")); ?></td>
										<td><?php// echo "($lt_pr) ".date('Y-m-d', strtotime($expaired. " - $lt_pr_cumm days")); ?></td>
										<td><?php// echo "($lt_Quo)) ".date('Y-m-d', strtotime($expaired. " - $lt_Quo_cumm days")); ?></td-->
									</tr>	
									<?php 
								}
							}
							?>	
						</tbody>
						<tfoot>
							<tr>	
							</tr>
						</tfoot>
					</table>
				</div>
			</div>
		</div>
	</div>
</div>


<div class="row clearfix">
	<div class="card">
		<div class="row clearfix">				
			<div class="header">
				<h2>Record<small>MH</small></h2>
			</div>
			<div class="body">
				<form action="" id="frmcari" method="post"  enctype="multipart/form-data">
					
					
					
					<div class="col-md-2">	
						<div class="form-group">
							<div class="input-group">
								<select class="selectpicker" style="width: 50%;"  name="term_crMH" id="term_crMH"  required>
									<option selected="selected" value="">---Pilih Term---</option>
									<?php echo $opt_term; ?>

								</select>
							</div>
						</div>
					</div>

					
					

					<div class="col-sm-2">
						<button type="submit" name="cr_bMH" id="cr_bMH" class="btn bg-purple btn-circle-lg waves-effect waves-circle waves-float"><i class="material-icons">search</i> </button>
						
					</div>
				</form>
			</div>	
		</div>	   
		<div class="row clearfix">
			<div class="body">
				<div class="table-responsive">
					<table id="example" class="table table-bordered table-striped table-hover dataTable js-exportable tabel2">
						<thead>
							<tr>	
								
								<th>TERM</th>
								<th>MH PLAN TERM SEBELUMNYA</th>
								<th>MH ACTUAL TERM SEBELUMNYA</th>
								<th>MH PLAN TERM INI</th>
								
								<th>REVISI</th>
								<th>TGL UPDATE</th>
								<th>PIC UPDATE</th>
								

							</tr>
						</thead>

						<tbody>
							<?php
							if(isset($_POST['cr_bMH']) ){	
								$term_crMH=$_POST['term_crMH'];
								
									$sq_acc="select * from bps_budget_stp_mh where term='$term_crMH' ORDER BY revisi DESC";
								}else{
								$sq_acc="select * from bps_budget_stp_mh where term='XXXX' ORDER BY revisi DESC";
								}
								//echo $sq_acc;
									$tb_acc=odbc_exec($koneksi_lp,$sq_acc);
									$row=0;
									while($baris1=odbc_fetch_array($tb_acc)){ $row++;
										
										?>	
										<tr>
											<td><?php echo odbc_result($tb_acc,"TERM"); ?></td>
											<td><?php echo number_format(odbc_result($tb_acc,"mh_s"), 5, '.', ''); ?></td>
											<td><?php echo number_format(odbc_result($tb_acc,"mh_act"), 5, '.', ''); ?></td>
											<td><?php echo number_format(odbc_result($tb_acc,"mh_p"), 5, '.', ''); ?></td>
											<td><?php echo odbc_result($tb_acc,"revisi"); ?></td>
											<td><?php echo odbc_result($tb_acc,"tgl_updt"); ?></td>
											<td><?php echo odbc_result($tb_acc,"tgl_updt"); ?></td>
											
									</tr>	
									<?php 
								}
							
							?>	
						</tbody>
						<tfoot>
							<tr>	
							</tr>
						</tfoot>
					</table>
				</div>
			</div>
		</div>
	</div>
</div>




</div>
</section>

<script type="text/javascript">
	$(document).ready(function()
	{
		$('.periode').bootstrapMaterialDatePicker({
			format: 'YYYYMM', minDate : new Date(),
			clearButton: true,
			weekStart: 1,
			time: false
		});	
	});
</script>	
<script>

		
	function pilih(row){
		var kd_pel0=row.cells[0].innerHTML;
		var kd_pel1=row.cells[1].innerHTML;
		var kd_pel2=row.cells[2].innerHTML;
		var kd_pel3=row.cells[3].innerHTML;
		var kd_pel4=row.cells[4].innerHTML;
		var kd_pel5=row.cells[5].innerHTML;
		var kd_pel6=row.cells[6].innerHTML;
		var kd_pel7=row.cells[7].innerHTML;
		var kd_pel8=row.cells[8].innerHTML;
		var kd_pel9=row.cells[9].innerHTML;
		var kd_pel10=row.cells[10].innerHTML;
		var kd_pel11=row.cells[11].innerHTML;
		var kd_pel12=row.cells[12].innerHTML;
		var kd_pel13=row.cells[13].innerHTML;
		var kd_pel14=row.cells[14].innerHTML;
		var kd_pel15=row.cells[15].innerHTML;
		var kd_pel16=row.cells[16].innerHTML;
		var kd_pel17=row.cells[17].innerHTML;
		var kd_pel18=row.cells[18].innerHTML;
		var kd_pel19=row.cells[19].innerHTML;
		var kd_pel20=row.cells[20].innerHTML;
//document.form1.sect.value=kd_pel0;
//document.form1.term.value=kd_pel1;
//document.form1.jns_bud.value=kd_pel2;
document.form1.no_ctrl.value=kd_pel3;
//document.form1.lp.value=kd_pel4;
document.form1.id_proses.value=kd_pel5;
document.form1.account.value=kd_pel6;
document.form1.sub_acc.value=kd_pel7;
document.form1.part_no.value=kd_pel8;
document.form1.part_nm.value=kd_pel9;
document.form1.part_dtl.value=kd_pel10;
document.form1.part_desc.value=kd_pel11;
document.form1.periode.value=kd_pel12;
document.form1.qty.value=kd_pel13;
document.form1.uom.value=kd_pel14;
document.form1.curr.value=kd_pel15;
document.form1.price.value=kd_pel16;

//document.form1.phase.value=kd_pel18;
document.form1.cccode.value=kd_pel19;
}
</script>			