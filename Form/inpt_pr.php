...
<script>
	...
	function pil_add(){
		...
		if(lstctrl==""){alert('ANDA BELUM MEMILIH DATA');
	}else{
		jQuery.ajax({
...
function sendEmailApi($jns_doc, $no_doc, $location){
	...
	if ($location == 'TF') {
		$url = "http://10.62.124.17/sami-local/send-email-pr/$jns_doc/$no_doc/$location";
	} else {
		$url = "http://10.62.191.24:90/sami-local/send-email-pr/$jns_doc/$no_doc/$location";
	}
...
if(isset($_POST['smpn_pr']) ){
	$plhperi=$_POST['plhperi'];
	$pr_date=$_POST["pr_date"];
	$bln=date("Ym",strtotime($pr_date));
	$nopr=0;
	$qry_nopr="select max(RIGHT(pr_no,3)) as jj from bps_pr where sect='$sect' and convert(nvarchar(6),pr_date,112)='$bln' and equipment_no is null";
	//periode='$per_tmp'";
	...
	foreach ($_POST['plh'] as $_boxValue2)
	{
		...
		while($bar_crdtquo=odbc_fetch_array($tb_crdtquo)){
			$supkod=str_replace(' ', '',odbc_result($tb_crdtquo,"kode_supp"));
...
//echo "<script>alert('$i - $np2[6] - $np2[2] - $np2[3] - $np2[5] - $np2[0] - $np2[1] - $noctrl - $lsctrl[$i]');</script>";
	...
	$qry_adpr="INSERT into bps_pr(pr_no,remark,no_ctrl,pr_date,qty,price,curr,term,periode,sect,cccode,pic_updt,tgl_updt) 
	select pr_no,'$rmk_pr' as remark,no_ctrl,pr_date,qty_act as qty, price_tot as 
	price,curr,term,periode,'$sect' as sect,cccode,pic_updt,getdate() as tgl_updt from
	bps_tmpPR where pr_no='$pr_no'";
		// echo $qry_adpr;
...
//if($qty_add!="" or $prc_add!=""){$dokaprv="'PR','ADD'";}else{$dokaprv="'PR'";}
	...
	SELECT distinct nama as pic_plan,email as email_plan,'$pr_no' as no_doc,getdate() as tgl_prepaire,
	'PR' as jns_doc,sect,initial,approve,no_aprv,right('000000'+nik,6)[nik]  FROM bps_setApprove where jns_dok in('PR') 
...
//$amoun_USD
	...
	if($_SESSION['lok']=="JF" and  $_SESSION["area"]=="PGA-IT"){
		
		
	}else{
			$squpdtapp="update bps_approve set pic_act='$pic',tgl_updt=getdate(),status='CLOSE' where no_doc='$pr_no' and jns_doc='PR' and sect='$sect' and approve='REQUEST'";
			// $tb_accPR=odbc_exec($koneksi_lp,$squpdtapp);
		
	}
	
	sendEmailApi('PR', $pr_no, $_SESSION['lok']);
...
if(isset($_POST['delpr']) ){	
	...
	$tb_del1=odbc_exec($koneksi_lp,"delete from bps_approve where jns_doc='PR' and no_doc='$prdel'");

	$tb_delpo=odbc_exec($koneksi_lp,"delete from bps_podtl where pr_no='$prdel'");
}
...
<section class="content">
	<div class="container-fluid">
		...
		<div class="row clearfix">	
			...
			if (($_SESSION["area"] == 'LOG-EXIM') or ($_SESSION["area"] == 'LOG-WHS') or  ($_SESSION["area"] == 'PGA-GS')) {
                $previousPeriod = date('Ym', strtotime('-2 month'));
                $PERIODEEE = "periode >= '$previousPeriod'";
            } else {
                $PERIODEEE = "periode >= convert(nvarchar(6), getdate(), 112)";
            }
			...
			//$tb_peri=odbc_exec($koneksi_lp,"select distinct periode from bps_budget where periode>=convert(nvarchar(6),getdate(),112) order by periode asc");
										//$tb_peri=odbc_exec($koneksi_lp,"select distinct periode from bps_budget where periode>=convert(nvarchar(6),getdate(),112) order by periode asc");
										$tb_peri=odbc_exec($koneksi_lp,"select distinct periode from bps_budget where $PERIODEEE order by periode asc");
		...
		if(isset($_POST['cr_b']) ){	
			...
			$cr_fcs=odbc_exec($koneksi_lp, "select isnull(sum(qty_fcs),0) as qty_fcs FROM bps_budget where sect='$sect' and periode='$peri'") ;
			$qty_fcs=odbc_result($cr_fcs, "qty_fcs");
		
		//	echo $qrycrdt."xxx". "select isnull(sum(qty_fcs),0) as qty_fcs FROM bps_budget where sect='$sect' and periode='$peri'";
			...
			// else if ($qty_fcs==0){
			// 	echo "<script>alert('SILAKAN INPUT FORECASE TERLEBIH DAHULU');</script>";
			// }
									...
									<div class="body">
										<div class="table-wrapper-scroll-y my-custom-scrollbar">
											<table id="dtVerticalScroll_vvvv" class="table table-striped table-bordered " cellspacing="0" width="100%">	
												...
												<tbody>
													<?php
													$sq_acc="select term,periode,phase,lp,no_ctrl,part_no,part_nm,part_dtl,part_desc,uom,qty,qty_fcs,qty_add,curr,price,price_add,exp_pr, dbo.cr_proseslp('quo',id_proses) as penawaran ,account, act_bud, act_qty,(select top 1 isnull(price,0) from bps_Quotation where part_no=b.part_no and lp_rekom='YES' and Exp_Quo>=b.expaired order by tgl_updt desc) as price_quo,cccode,expaired,(select count (*) as jml from bps_budget_add where no_ctrl=b.no_ctrl and exists (select * from bps_tmpPR where no_ctrl=b.no_ctrl and periode!=b.periode) ) as add_periode 
													...
													while($baris1=odbc_fetch_array($tb_acc)){
														...
														$Qplan_fcs=odbc_result($tb_acc,"qty_fcs");
														...
														$add_periode=odbc_result($tb_acc,"add_periode");
														$term_p=odbc_result($tb_acc,"term");
														...
														else if($prcadd+$prcplan>$price_quot and $price_quot>0) {
															$tot_prc=$price_quot;
														}
														else {
															$tot_prc=$prcplan+$prcadd;
														}
														...
														if($Qadd==0){
															$mount_quo=$price_quot;
														}
														else{
															$mount_quo=$price_quot*$Qadd;
														}
														$cccode=odbc_result($tb_acc,"cccode");
														$cr_car=odbc_exec($koneksi_lp,"select cv_code from lp_cv where cost_center_code='$cccode' and term='$term_p' ");
														if($cccode==''){
															$cv_code='000';
														...
														if($sis_bud>0){
															...
															$plh="on";$rmx_x="";
															// $sis_bud<$tot_prc														
															if($tot_prc<$price_quot){
																$plh="off";
																$ket="Penawaran > Plan SPT";
															}else
															if($amount_pln-$act_bud < $sis_bud){
																$plh="off";
																$ket="Price kurang";
															}else
															if($act_qty>($Qadd+$Qplan)){
																$plh="off";
																$ket="Qty kurang";
															}else
																//$rmx_x="Harga Kurang";}
															if($ketquo=="YES" and $mount_quo<=0){
																$plh="off";
																$ket="Belum ada Penawaran";
															}else
																//$rmx_x="Harga Kurang";}
																if($add_periode>0 ){
																	if($Qadd>=$Qplan){
															...
															*/	//$mount_quo$rmx_x=$rmx_x." Penawaran Belum";}
															...
															<tr>				
																<td >
																	<?php if($plh=="on"){ ?>
																		<div class="switch" ><label><input type="checkbox" name="plh[]" id="plh" value="<?php echo $no_ctrl.'|'.$tot_qty.'|'.$tot_prc.'|'.$amount_pln.'|'.$sis_bud.'|'.$sis_bud1.'|'.$amount.'|'.$price_quot.'|'.$phase.'|'.$part_no.'|'.$exp_bud."|".$cv_code."|".$account; ?>" onclick="dipilih(this.form);"><span class="lever"></span></label></div>
																	<?php }else{ echo '<i class="material-icons">clear</i>';} ?>
																</td>
																	<!-- <td><select class="selectpicker" style="width: 100%;"  name="bc" id="bc" >
																		<option selected="selected" value="">---Pilih Kolom---</option>
																	...
																	<td><?php echo round($Qplan,3); ?></td>
																	<td><?php echo $Qadd; ?></td>
																	<td><?php echo $tot_qty; ?></td>
																	<td><?php echo odbc_result($tb_acc,"curr"); ?></td>
																	<td><?php echo number_format($prcplan,3,".",","); ?></td>
																	<td><?php echo number_format($prcadd,3,".",","); ?></td>
																	<td><?php echo number_format($prcplan+$prcadd,3,".",","); ?></td>
																	<td><?php echo number_format($amount_pln,3,".",","); ?></td>
																	<td><?php echo number_format($act_qty,3,".",",");?></td>
																	<td><?php echo number_format($tot_prc,3,".",",");?></td>
																	<td><?php echo number_format($amount_pln-$act_bud,3,".",","); ?></td>
																	<td><?php echo $ketquo; ?></td>
																	<td><?php echo number_format($price_quot,3,".",","); ?></td>
									...
									<div class="table-responsive">
										<table id="example" class="table table-bordered table-striped table-hover dataTable js-exportable tabel2">
											...
											<tbody>
												...
												from bps_pr a left join bps_approve b on a.PR_NO=b.no_doc 
												and a.sect=b.sect where request is null and a.sect='$sect' and b.status is null and equipment_no is null
												...
												while($bar_pr=odbc_fetch_array($tb_pr)){ $i++;
													...
													?>	
													<tr onclick="javascript:pilihan(this);">
														<td><?= $prno ; ?></td>
														<!-- <div class="form-line">
														...
														<td><button type="button" class="btn bg-green waves-effect" onclick="open_child('Exp_pdf/print_pr.php?nomor=<?php echo $i;?>&nopr=<?php echo $prno;?>','Print PR <?php echo $prno;?>','800','500'); return false;"><i class="material-icons">print</i></button>			
															<a href="Exp_xls/exp_PR.php?pr_no=<?php echo $prno; ?>&sec=<?php echo $sect;?>&periode=<?php echo $periode_pr; ?>" _target="blank" class="btn bg-blue waves-effect">
																<i  class="material-icons">file_download</i>
															</a>
															<!-- <?php if ($status!='FINISH' || $adm1=='user'){?> -->
															<button type="button" class="btn bg-orange waves-effect" onclick="open_child('select/rev_pr.php?pic=<?php echo $pic;?>&sec=<?php echo $sect;?>&sesi=<?php echo $lok;?>&nopr=<?php echo $prno;?>','Edit PR <?php echo $prno;?>','800','500'); return false;"><i class="material-icons">edit</i></button><br/>
															<?php 
															$cr_partlist="";
															 ?>
															...
															<!-- <a href="#" onClick="deletepr()" class="btn bg-red waves-effect" ><i  class="material-icons">delete</i></a> -->
															<!-- <?php } ?> -->
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<form action="" id="frmdel" name="frmdel" method="post"  enctype="multipart/form-data">
						<div class="modal-body">
							SEMENTARA , HAPUS PR DARI MENU REVISI PR YA 
							<input type="text" readonly class="form-control" data-role="tagsinput" id="prdel" name="prdel" placeholder="PR NO" required>
						</div>
						<div class="modal-footer">
