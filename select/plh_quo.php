...
<?php }else { ?>
  <script>
    function pilih(row){
    //var kd_pel=row.cells[<?php echo $kol; ?>].innerHTML;
    //window.opener.parent.document.getElementById("<?php echo $nmobj; ?>").value = kd_pel;
    var kd_pel3=row.cells[3].innerHTML;
    var kd_pel=row.cells[4].innerHTML;
    
    var nomx=document.getElementById("nom").value;
    
  //window.opener.parent.document.getElementById("cccode").value = kd_pel;
  var els=window.opener.parent.document.getElementsByName("cccode[]");  
  els[nomx].value = kd_pel;
