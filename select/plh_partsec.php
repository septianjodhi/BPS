...
//$part_no=$_GET['pno'];	
 		$part_nm=$_GET['pnd'];
 		if($part_nm==''){$partnm="";}else{$partnm=" and part_nm like '%$part_nm%'";}
 		$lp=$_GET['lp'];
 		$part_kat=$_GET['part_kat'];
 		if($part_kat==''){$partkat="";}else{$partkat=" and part_kat='$part_kat' ";}
 		$periode=substr($_GET['p'],-2);
 		// $sql1="select * from bps_part where lp='$lp' $partnm $partkat";
    $sql1="SELECT * from bps_part where lp='$lp' and status_part='1' $partnm $partkat";
 		echo $sql1;
 		if($periode<=7){$kp="9";}else{$kp="8";}
 		?>

 		<form id="form1" name="form1" method="post">
 			<div class="block-header"><h2>Cari Part No</h2>
 			</div>
 				...
 				<tbody>
 					...
 					while($baris1=odbc_fetch_array($tb_area)){ $row++;
 						...
 						<tr onclick="javascript:pilih(this);">
 							<td><?php echo $row; ?></td>
 							<td><?php echo odbc_result($tb_area,"part_grp"); ?></td>
 							...
 							<td><?php echo odbc_result($tb_area,"price1"); ?></td>
 							<td><?php echo odbc_result($tb_area,"price2"); ?></td>	
 						</tr>
            <?php 
          } 
          ?>
...
<script>
  function pilih(row){
      	...
      	window.opener.parent.document.getElementById("part_no").value = kd_pel4;
		...
		//window.opener.parent.document.getElementById("price").value = kd_pel<?php echo $kp; ?>;
		window.close();
	}
</script>